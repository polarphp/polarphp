...
struct NonPOD
{
   ...
   friend HashCode hash_value(const NonPOD &obj)
   {
...
using polar::basic::hash_combine_range;
using polar::basic::hash_combine;
using polar::basic::NonPOD;
using polar::basic::StringRef;
using polar::basic::LargeTestInteger;
...
TEST(HashingTest, testHashValueStdPair)
{
...
TEST(HashingTest, testHashValueStdString)
{
...
HashCode hash_value(HashableDummy dummy) { return dummy.value; }

} // anonymous namespace

TEST(HashingTest, testHashCombineRangeBasicTest)
{
   // Leave this uninitialized in the hope that valgrind will catch bad reads.
   int dummy;
   HashCode dummy_hash = hash_combine_range(&dummy, &dummy);
   EXPECT_NE(HashCode(0), dummy_hash);

   const int arr1[] = { 1, 2, 3 };
   HashCode arr1_hash = hash_combine_range(begin(arr1), end(arr1));
   ...
   HashCode arr2_hash = hash_combine_range(begin(arr2), end(arr2));
   EXPECT_NE(dummy_hash, arr2_hash);
   EXPECT_NE(arr1_hash, arr2_hash);

   const int arr3[] = { 1, 1, 2, 3 };
   HashCode arr3_hash = hash_combine_range(begin(arr3), end(arr3));
   EXPECT_NE(dummy_hash, arr3_hash);
   EXPECT_NE(arr1_hash, arr3_hash);

   const int arr4[] = { 1, 2, 3, 3 };
   HashCode arr4_hash = hash_combine_range(begin(arr4), end(arr4));
   EXPECT_NE(dummy_hash, arr4_hash);
   EXPECT_NE(arr1_hash, arr4_hash);

   const size_t arr5[] = { 1, 2, 3 };
   const HashableDummy d_arr5[] = { {1}, {2}, {3} };
   HashCode arr5_hash = hash_combine_range(begin(arr5), end(arr5));
   HashCode d_arr5_hash = hash_combine_range(begin(d_arr5), end(d_arr5));
   EXPECT_EQ(arr5_hash, d_arr5_hash);
}

TEST(HashingTest, testHashCombineRangeLengthDiff)
{
   ...
   for (unsigned Idx = 1, Size = all_one_c.size(); Idx < Size; ++Idx) {
      HashCode code = hash_combine_range(&all_one_c[0], &all_one_c[0] + Idx);
   ...
   for (unsigned Idx = 1, Size = all_zero_c.size(); Idx < Size; ++Idx) {
      HashCode code = hash_combine_range(&all_zero_c[0], &all_zero_c[0] + Idx);
      std::map<size_t, size_t>::iterator
            I = code_to_size.insert(std::make_pair(code, Idx)).first;
      EXPECT_EQ(Idx, I->second);
   }
   ...
   for (unsigned Idx = 1, Size = all_one_int.size(); Idx < Size; ++Idx) {
      HashCode code = hash_combine_range(&all_one_int[0], &all_one_int[0] + Idx);
      std::map<size_t, size_t>::iterator
            I = code_to_size.insert(std::make_pair(code, Idx)).first;
      EXPECT_EQ(Idx, I->second);
   }
   ...
   for (unsigned Idx = 1, Size = all_zero_int.size(); Idx < Size; ++Idx) {
      HashCode code = hash_combine_range(&all_zero_int[0], &all_zero_int[0] + Idx);
      std::map<size_t, size_t>::iterator
            I = code_to_size.insert(std::make_pair(code, Idx)).first;
      EXPECT_EQ(Idx, I->second);
   }
...
TEST(HashingTest, testHashCombineRangeGoldenTest)
{
...
#error This test only supports 64-bit and 32-bit systems.
   ...
   for (unsigned i = 0; i < sizeof(golden_data)/sizeof(*golden_data); ++i) {
      ...
      HashCode hash = hash_combine_range(str.begin(), str.end());
...
TEST(HashingTest, testHashCombineBasicTest) {

...
TEST(Endian, testReadBitAligned)
{
...
TEST(Endian, testWriteBitAligned)
{
...
TEST(Endian, testPackedEndianSpecificIntegral)
{
   // These are 5 bytes so we can be sure at least one of the reads is UNALIGNED.
   ...
   little32_t *littleValue =
         reinterpret_cast<little32_t *>(little + 1);

   EXPECT_EQ(*bigValue, *littleValue);
}

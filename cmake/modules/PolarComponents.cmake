...
#
# *NOTE* In the following for brevity, a "polarphp Component" will just be called a
# component.
#
...
# -------------------------
#
# The set of "defined" polarphp components are as follows:
#
...
#   'lib/polarphp/clang'.  This is useful when polarphp compiler is installed in
#   isolation.
# * clang-resource-dir-symlink -- install a symlink to the Clang resource
#   directory (which contains builtin headers) under 'lib/polarphp/clang'.  This is
#   useful when Clang and polarphp are installed side-by-side.
# * stdlib -- the polarphp standard library.
# * stdlib-experimental -- the Swift standard library module for experimental
#   APIs.
# * sdk-overlay -- the polarphp SDK overlay.
...
set(_POLAR_DEFINED_COMPONENTS
   "autolink-driver;compiler;clang-builtin-headers;clang-resource-dir-symlink;clang-builtin-headers-in-clang-resource-dir;stdlib;stdlib-experimental;sdk-overlay;parser-lib;editor-integration;tools;testsuite-tools;toolchain-tools;toolchain-dev-tools;dev;license;sourcekit-xpc-service;sourcekit-inproc;polarphp-remote-mirror;polarphp-remote-mirror-headers")

# The default install components include all of the defined components, except
# for the following exceptions.
set(_POLAR_DEFAULT_COMPONENTS "${_POLAR_DEFINED_COMPONENTS}")
# 'dev' takes up a lot of disk space and isn't part of a normal toolchain.
list(REMOVE_ITEM _POLAR_DEFAULT_COMPONENTS "dev")
# These clang header options conflict with 'clang-builtin-headers'.
list(REMOVE_ITEM _POLAR_DEFAULT_COMPONENTS "clang-resource-dir-symlink")
list(REMOVE_ITEM _POLAR_DEFAULT_COMPONENTS "clang-builtin-headers-in-clang-resource-dir")
# This conflicts with LLVM itself when doing unified builds.
list(REMOVE_ITEM _POLAR_DEFAULT_COMPONENTS "toolchain-dev-tools")
# The sourcekit install variants are currently mutually exclusive.
if(${CMAKE_SYSTEM_NAME} MATCHES "Darwin")
   list(REMOVE_ITEM _POLAR_DEFAULT_COMPONENTS "sourcekit-inproc")
else()
   list(REMOVE_ITEM _POLAR_DEFAULT_COMPONENTS "sourcekit-xpc-service")
endif()
list(REMOVE_ITEM _POLAR_DEFAULT_COMPONENTS "stdlib-experimental")

macro(polar_configure_components)
   # Set the POLAR_INSTALL_COMPONENTS variable to the default value if it is not passed in via -D
   set(POLAR_INSTALL_COMPONENTS "${_POLAR_DEFAULT_COMPONENTS}" CACHE STRING
      "A semicolon-separated list of components to install from the set ${_POLAR_DEFINED_COMPONENTS}")

   foreach(component ${_POLAR_DEFINED_COMPONENTS})
      ...
      set(POLAR_INSTALL_${var_name_piece} FALSE)
   endforeach()
   ...
   add_custom_target(polarphp-components)
   add_custom_target(install-polarphp-components)

   foreach(component ${POLAR_INSTALL_COMPONENTS})
      ...
      string(TOUPPER "${component}" var_name_piece)
      string(REPLACE "-" "_" var_name_piece "${var_name_piece}")
      if(NOT POLAR_INSTALL_EXCLUDE_${var_name_piece})
         set(POLAR_INSTALL_${var_name_piece} TRUE)
         add_dependencies(polarphp-components ${component})
         add_dependencies(install-polarphp-components install-${component})
      endif()
   endforeach()
endmacro()
...
# polar_install_in_component(<same parameters as install()>)
#
...
function(polar_install_in_component)
   cmake_parse_arguments(
      ...
      "" # multi-value args
      ${ARGN})
   polar_precondition(ARG_COMPONENT MESSAGE "Component name is required")
   polar_is_installing_component("${ARG_COMPONENT}" is_installing)
...
function(polar_install_symlink_component component)
   cmake_parse_arguments(
      ARG # prefix
      "" # options
   ...
   polar_precondition(INSTALL_SYMLINK

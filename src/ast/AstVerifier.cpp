...
#undef FUNCTION_LIKE
   ...
   void verifyChecked(InterfaceMetatypeToObjectExpr *E) {
      PrettyStackTraceExpr debugStack(Ctx,
                                      "verifying InterfaceMetatypeToObject", E);

      auto srcTy = checkMetatypeType(E->getSubExpr()->getType(),
                                     "source of InterfaceMetatypeToObject");
      if (E->getSubExpr()->getType()->is<ExistentialMetatypeType>()) {
         Out << "InterfaceMetatypeToObject with existential "
                "metatype:\n";
         E->dump(Out);
         Out << "\n";
         abort();
      ...
      if (!srcTy->isExistentialType()) {
         Out << "InterfaceMetatypeToObject with non-existential metatype:\n";
         E->dump(Out);
         Out << "\n";
         abort();
      }
      ...
      if (layout.explicitSuperclass ||
          !layout.isObjC() ||
          layout.getInterfaces().size() != 1) {
         Out << "InterfaceMetatypeToObject with non-ObjC-protocol metatype:\n";
         E->dump(Out);
         Out << "\n";
         abort();
      }
      ...
      if (!E->getType()->getClassOrBoundGenericClass()) {
         Out << "InterfaceMetatypeToObject does not produce class:\n";
         E->dump(Out);
         Out << "\n";
         abort();
      }
   ...
   void verifyChecked(ErasureExpr *E) {
      ...
      while (auto meta = erasedTy->getAs<ExistentialMetatypeType>()) {
         ...
         if (auto concreteMeta = concreteTy->getAs<MetatypeType>()) {
            ...
            // shouldn't be here, since (P & Q).Interface.Type doesn't exist as
            // a type.
            ...
            // from (P & Q).Type.Interface to P.Type.Type), then this is invalid,
      ...
      if (knownConcreteErasure != ConcreteErasureOnly
         ...
         if (erasedLayout.requiresClass()) {
            ...
            if (concreteTy->mayHaveSuperclass()) {
               ...
               canBeClass = concreteLayout.getKind() == ExistentialLayout::Kind::Class
                            && !concreteLayout.containsNonObjCInterface;
            } else {
               canBeClass = false;
            }
         ...
         for (auto proto : erasedLayout.getInterfaces()) {
            if (std::find_if(conformances.begin(), conformances.end(),
                             [&](InterfaceConformanceRef ref) -> bool {
   ...
   void verifyChecked(AnyHashableErasureExpr *E) {
      ...
      auto hashableDecl = Ctx.getInterface(KnownInterfaceKind::Hashable);
      if (!hashableDecl) {
   ...
   /// type.
   void verifyConformance(Type type, InterfaceConformanceRef conformance) {
   ...
   void verifyConformance(Decl *decl, InterfaceConformance *conformance) {
      ...
      switch (conformance->getState()) {
         ...
         case InterfaceConformanceState::Checking:
            ...
            Out << " has a protocol conformance that is still being checked "
                << conformance->getInterface()->getName().str() << "\n";
            abort();
      }
...
//      }
   ...
   void verifyChecked(ConstructorDecl *CD) {
      ...
      if (!isa<ClassDecl>(ND) && !isa<StructDecl>(ND) && !isa<EnumDecl>(ND) &&
          !isa<InterfaceDecl>(ND) && !CD->isInvalid()) {
...
//            }
   ...
   void verifyParsed(AssociatedTypeDecl *ATD) {
      ...
      if (!isa<NominalTypeDecl>(DC) ||
          !isa<InterfaceDecl>(cast<NominalTypeDecl>(DC))) {
...
fail:
   ...
   void checkSameOrSubType(Type T0, Type T1, const char *what) {
      ...
      // Interface subtyping.
      if (auto Proto0 = T0->getAs<InterfaceType>())
         if (auto Proto1 = T1->getAs<InterfaceType>())
...
bool should_verify(const Decl *D, const AstContext &Context) {
#if !(defined(NDEBUG) || defined(SWIFT_DISABLE_AST_VERIFIER))
   if (const auto *ED = dyn_cast<ExtensionDecl>(D)) {
      return should_verify(ED->getExtendedNominal(), Context);
   }

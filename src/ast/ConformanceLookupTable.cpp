...
InterfaceDecl *ConformanceLookupTable::ConformanceEntry::getInterface() const {
   if (auto protocol = Conformance.dyn_cast<InterfaceDecl *>())
      return protocol;

   return Conformance.get<InterfaceConformance *>()->getInterface();
}
...
void ConformanceLookupTable::ConformanceEntry::dump(raw_ostream &os,
   ...
   os << " protocol=";
   getInterface()->dumpRef(os);

   if (Loc.isValid()) {
      os << " loc=";
      Loc.print(os, getInterface()->getAstContext().SourceMgr);
   }
...
namespace {
using ConformanceConstructionInfo = std::pair<SourceLoc, InterfaceDecl *>;
}
...
void ConformanceLookupTable::forEachInStage(ConformanceStage stage,
   ...
   // Interface extensions do not contribute protocol conformances. This
   ...
   while (auto next = lastProcessed.getPointer()
      ...
      if (loader.first) {
         ...
         for (auto conf : conformances) {
            protocols.push_back({SourceLoc(), conf->getInterface()});
         }
...
void ConformanceLookupTable::inheritConformances(ClassDecl *classDecl,
   ...
   llvm::SmallPtrSet<InterfaceDecl *, 4> protocols;
   auto addInheritedConformance = [&](ConformanceEntry *entry) {
      auto protocol = entry->getInterface();
      ...
      (void) addInterface(protocol, getSuperclassLoc(),
   ...
   switch (stage) {
      case ConformanceStage::RecordedExplicit:
         ...
         forEachInStage(
            stage, nominal,
            [&](NominalTypeDecl *nominal) {
               addInheritedInterfaces(nominal,
            ...
            [&](ExtensionDecl *ext,
               ...
               for (auto locAndProto : protos)
                  addInterface(locAndProto.second, locAndProto.first, source);
            });
         break;
...
void ConformanceLookupTable::loadAllConformances(
   DeclContext *dc,
   ArrayRef<InterfaceConformance *> conformances) {
   ...
   for (auto conformance : conformances) {
      registerInterfaceConformance(conformance);
   }
}

bool ConformanceLookupTable::addInterface(InterfaceDecl *protocol, SourceLoc loc,
...
void ConformanceLookupTable::addInheritedInterfaces(
   ...
   for (const auto &found :
      ...
      if (auto proto = dyn_cast<InterfaceDecl>(found.second))
         addInterface(proto, found.first, source);
   }
}
...
void ConformanceLookupTable::expandImpliedConformances(NominalTypeDecl *nominal,
   ...
   for (unsigned i = 0; i != AllConformances[dc].size(); ++i) {
      ...
      InterfaceDecl *conformingInterface = conformanceEntry->getInterface();
      ...
      if (conformingInterface->isSpecificInterface(
         ...
         if (auto bridgedNSError
            = ctx.getInterface(KnownInterfaceKind::BridgedNSError)) {
            addInterface(bridgedNSError, SourceLoc(),
                         ConformanceSource::forImplied(conformanceEntry));
         }
      }

      addInheritedInterfaces(conformingInterface,
                             ConformanceSource::forImplied(conformanceEntry));
   }
}
...
ConformanceLookupTable::Ordering ConformanceLookupTable::compareConformances(
   ...
   auto lhsExplicitInterface = lhsExplicit->getInterface();
   auto rhsExplicit = rhs->getDeclaredConformance();
   auto rhsExplicitInterface = rhsExplicit->getInterface();
   if (lhsExplicitInterface != rhsExplicitInterface) {
      ...
      for (auto rhsInterface : rhsExplicitInterface->getAllInterfaces()) {
         if (rhsInterface == lhsExplicitInterface) {
            return Ordering::Before;
         }
      }
      ...
      for (auto lhsInterface : lhsExplicitInterface->getAllInterfaces()) {
         if (lhsInterface == rhsExplicitInterface) {
...
bool ConformanceLookupTable::resolveConformances(InterfaceDecl *protocol) {
...
DeclContext *ConformanceLookupTable::getConformingContext(
   NominalTypeDecl *nominal,
   ConformanceEntry *entry) {
   InterfaceDecl *protocol = entry->getInterface();
...
ConformanceLookupTable::getConformance(NominalTypeDecl *nominal,
   ...
   if (entry->getKind() == ConformanceEntryKind::Inherited) {
      ...
      auto normalConf =
         ctx.getConformance(conformingType, protocol, conformanceLoc,
                            conformingDC, InterfaceConformanceState::Incomplete);
      ...
      NormalInterfaceConformance *implyingConf = nullptr;
      ...
      if (impliedEntry->getKind() == ConformanceEntryKind::Synthesized) {
         auto *impliedProto = impliedEntry->getInterface();

         // Find a SynthesizedInterfaceAttr corresponding to the protocol.
         for (auto attr : conformingNominal->getAttrs()
            .getAttributes<SynthesizedInterfaceAttr>()) {
            auto otherProto = ctx.getInterface(attr->getInterfaceKind());
   ...
   return entry->Conformance.get<InterfaceConformance *>();
}

void ConformanceLookupTable::addSynthesizedConformance(NominalTypeDecl *nominal,
                                                       InterfaceDecl *protocol) {
   addInterface(protocol, nominal->getLoc(),
                ConformanceSource::forSynthesized(nominal));
}

void ConformanceLookupTable::registerInterfaceConformance(
   ...
   for (auto entry : dcConformances) {
      if (entry->getInterface() == protocol) {
   ...
   auto inherited = dyn_cast<InheritedInterfaceConformance>(conformance);
   ConformanceSource source
...
void ConformanceLookupTable::lookupConformances(
   ...
   for (const auto &potential : potentialConformances) {
      resolveConformances(potential->getInterface());
   }
   ...
   potentialConformances.erase(
      std::remove_if(potentialConformances.begin(),
                     ...
                     [&](ConformanceEntry *entry) {
                        ...
                        // Record the protocol.
                        if (protocols)
                           protocols->push_back(entry->getInterface());
   ...
   if (diagnostics) {
      ...
      if (knownDiags != AllSupersededDiagnostics.end()) {
         for (const auto *entry : knownDiags->second) {
            ...
            diagnostics->push_back({entry->getInterface(),
                                    entry->getDeclaredLoc(),
                                    entry->getKind(),
                                    entry->getDeclaredConformance()->getInterface(),
...
void ConformanceLookupTable::getAllInterfaces(
   NominalTypeDecl *nominal,
   SmallVectorImpl<InterfaceDecl *> &scratch) {
...
int ConformanceLookupTable::compareInterfaceConformances(
   InterfaceConformance *const *lhsPtr,
   InterfaceConformance *const *rhsPtr) {
   InterfaceConformance *lhs = *lhsPtr;
   InterfaceConformance *rhs = *rhsPtr;

   // If the two conformances are normal conformances with locations,
   // sort by location.
   if (auto lhsNormal = dyn_cast<NormalInterfaceConformance>(lhs)) {
      if (auto rhsNormal = dyn_cast<NormalInterfaceConformance>(rhs)) {
   ...
   InterfaceDecl *lhsProto = lhs->getInterface();
   InterfaceDecl *rhsProto = rhs->getInterface();
...
void ConformanceLookupTable::getAllConformances(
   NominalTypeDecl *nominal,
   bool sorted,
   SmallVectorImpl<InterfaceConformance *> &scratch) {
   ...
   if (sorted) {
      llvm::array_pod_sort(scratch.begin(), scratch.end(),
                           &compareInterfaceConformances);
   }
}

void ConformanceLookupTable::getImplicitInterfaces(
   NominalTypeDecl *nominal,
   SmallVectorImpl<InterfaceDecl *> &protocols) {
   for (auto conformance : AllConformances[nominal]) {
      if (conformance->getKind() == ConformanceEntryKind::Synthesized) {
         protocols.push_back(conformance->getInterface());
      }
   }
}

ArrayRef<ValueDecl *>
ConformanceLookupTable::getSatisfiedInterfaceRequirementsForMember(
   ...
   SmallVector<InterfaceConformance *, 4> result;

...
void simple_display(llvm::raw_ostream &out,
   ...
   Flag possibleFlags[] = {
      {UnqualifiedLookupFlags::AllowInterfaceMembers, "AllowInterfaceMembers"},
...
static void recordShadowedDeclsAfterSignatureMatch(
   ...
   for (unsigned firstIdx : indices(decls)) {
      ...
      for (unsigned secondIdx : range(firstIdx + 1, decls.size())) {
         ...
         if ((bool) firstDecl->getDeclContext()->getSelfInterfaceDecl() !=
             (bool) secondDecl->getDeclContext()->getSelfInterfaceDecl()) {
            if (firstDecl->getDeclContext()->getSelfInterfaceDecl()) {
               shadowed.insert(firstDecl);
               break;
            } else {
               shadowed.insert(secondDecl);
         ...
         if (isa<InterfaceDecl>(firstDecl->getDeclContext()) ||
             isa<InterfaceDecl>(secondDecl->getDeclContext()))
            continue;
...
SelfBoundsFromWhereClauseRequest::evaluate(
   ...
   auto *protoDecl = dyn_cast_or_null<InterfaceDecl>(typeDecl);
...
static void extractDirectlyReferencedNominalTypes(
   ...
   if (auto compositionTy = type->getAs<InterfaceCompositionType>()) {
...
bool DeclContext::lookupQualified(ArrayRef<NominalTypeDecl *> typeDecls,
   ...
   auto &ctx = getAstContext();
   bool wantInterfaceMembers = (options & NL_InterfaceMembers);
   while (!stack.empty()) {
      ...
      bool currentIsInterface = isa<InterfaceDecl>(current);
      ...
      // step.
      if (!wantInterfaceMembers && !currentIsInterface)
         continue;

      SmallVector<InterfaceDecl *, 4> protocols;

      if (auto *protoDecl = dyn_cast<InterfaceDecl>(current)) {
         ...
         for (auto inheritedProto : protoDecl->getInheritedInterfaces()) {
            addNominalType(inheritedProto);
         }
      } else {
         // Collect the protocols to which the nominal type conforms.
         for (auto proto : current->getAllInterfaces()) {
...
directReferencesForUnqualifiedTypeLookup(DeclName name,
   ...
   UnqualifiedLookupOptions options =
      ...
      UnqualifiedLookupFlags::AllowInterfaceMembers;
...
static DirectlyReferencedTypeDecls directReferencesForType(Type type) {
   ...
   if (type->isExistentialType()) {
      ...
      // Interfaces.
      for (auto protocolTy : layout.getInterfaces())
...
SuperclassDeclRequest::evaluate(Evaluator &evaluator,
   ...
   // Interfaces may get their superclass bound from a `where Self : Superclass`
   // clause.
   if (auto *proto = dyn_cast<InterfaceDecl>(subject)) {
...
CustomAttrNominalRequest::evaluate(Evaluator &evaluator,
   ...
   if (nominals.size() == 1 && !isa<InterfaceDecl>(nominals.front()))
...
getDirectlyInheritedNominalTypeDecls(
   ...
   if (protoDecl->wasDeserialized()) {
      ...
      for (auto &req : protoDecl->getRequirementSignature()) {
         ...
         result.emplace_back(
            loc, req.getSecondType()->castTo<InterfaceType>()->getDecl());
      }
      return result;
   }

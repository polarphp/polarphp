...
//      Printer << suffix << " ";
   ...
   void printAccess(const ValueDecl *D) {
      ...
      if (!Options.PrintAccess || isa<InterfaceDecl>(D->getDeclContext()))
         return;
...
public:
   ...
   void printInheritedFromRequirementSignature(InterfaceDecl *proto,
                                               Decl *attachingTo);

   void printWhereClauseFromRequirementSignature(InterfaceDecl *proto,
                                                 Decl *attachingTo);
...
public:
   ...
   bool visit(Decl *D) {
      #ifdef POLAR_BUILD_ONLY_SYNTAXPARSERLIB
      ...
      POLAR_DEFER{CurrentType = OldType;};
...
bestRequirementPrintLocation(InterfaceDecl *proto, const Requirement &req) {
   auto protoSelf = proto->getInterfaceSelfType();
   ...
   auto findRelevantDeclAndDirectUse = [&](Type outerType) {
      ...
      (void) outerType.findIf([&](Type t) {
         if (t->isEqual(protoSelf)) {
            ...
            if (assocType && assocType->getInterface() == proto) {
...
void PrintAst::printInheritedFromRequirementSignature(InterfaceDecl *proto,
                                                      Decl *attachingTo) {
   printGenericSignature(
      GenericSignature::get({proto->getInterfaceSelfType()},
...
void PrintAst::printWhereClauseFromRequirementSignature(InterfaceDecl *proto,
                                                        Decl *attachingTo) {
...
bool ShouldPrintChecker::shouldPrint(const Decl *D,
   ...
   if (Options.SkipPrivateStdlibDecls &&
       D->isPrivateStdlibDecl(!Options.SkipUnderscoredStdlibInterfaces))
      return false;
   ...
   if (Options.SkipEmptyExtensionDecls && isa<ExtensionDecl>(D)) {
      ...
      SmallVector<TypeLoc, 8> InterfacesToPrint;
      getInheritedForPrinting(Ext, Options, InterfacesToPrint);
      if (InterfacesToPrint.empty()) {
   ...
   if (Options.SkipOverrides) {
      if (auto *VD = dyn_cast<ValueDecl>(D)) {
         ...
         if (!VD->getSatisfiedInterfaceRequirements().empty()) return false;
         ...
         if (auto clangDecl = VD->getClangDecl()) {
            ...
            if (isa<clang::ObjCInterfaceDecl>(clangDecl->getDeclContext()) &&
...
void PrintAst::printAccessors(const AbstractStorageDecl *ASD) {
   ...
   if (impl.isSimpleStored()) {
      // ...unless we're printing for PIL, which expects a { get set? } on
      //    trivial properties
      if (Options.PrintForPIL) {
   ...
   bool inInterface = isa<InterfaceDecl>(ASD->getDeclContext());
   if ((inInterface && !Options.PrintAccessorBodiesInInterfaces) ||
       PrintAbstract) {
      ...
      if (!inInterface && !Options.PrintGetSetOnRWProperties &&
   ...
   auto shouldHideModifyAccessor = [&] {
      ...
      return isa<InterfaceDecl>(ASD->getDeclContext());
   };
...
void PrintAst::printMembersOfDecl(Decl *D, bool needComma,
   ...
   if (auto Ext = dyn_cast<ExtensionDecl>(D)) {
      ...
      if (Options.PrintExtensionFromConformingInterfaces) {
         for (auto Conf : NTD->getAllConformances()) {
            for (auto Ext : Conf->getInterface()->getExtensions()) {
               if (Options.printExtensionContentAsMembers(Ext))
                  AddDeclFunc(Ext->getMembers());
            }
         }
...
void PrintAst::visitInterfaceDecl(InterfaceDecl *decl) {
//   printDocumentationComment(decl);
//   printAttributes(decl);
//   printAccess(decl);
//
...
//      // already-compiled PIL modules. The requirement signature is available in
...
//   if (Options.PrintForPIL && decl->hasStorage() &&
...
//         // conforming type.
//         auto *proto = dyn_cast<InterfaceDecl>(decl->getDeclContext());
...
void PrintAst::visitConstructorDecl(ConstructorDecl *decl) {
   ...
   if ((decl->getInitKind() == CtorInitializerKind::Convenience ||
      ...
      // Interface extension initializers are modeled as convenience initializers,
      ...
      if (isClassContext) {
         ...
         assert(decl->getDeclContext()->getExtendedInterfaceDecl() &&
...
void Decl::print(raw_ostream &os) const {
   ...
   // FIXME: Move all places where PIL printing is happening to explicit options.
...
public:
   ...
   void printQualifiedType(T *Ty) {
      ...
      // know we're printing a type member so it escapes `Type` and `Interface`.
   ...
   void visit(Type T) {
      ...
      POLAR_DEFER{Printer.printTypePost(TypeLoc::withoutLoc(T));};
...
#undef ASTPRINTER_PRINT_BUILTINTYPE
   ...
   void visitPILTokenType(PILTokenType *T) {
//      Printer << BUILTIN_TYPE_NAME_PILTOKEN;
   }

   void visitTypeAliasType(TypeAliasType *T) {
      if (Options.PrintForPIL || Options.PrintTypeAliasUnderlyingType) {
      ...
      if (T->hasRepresentation()) {
         switch (T->getRepresentation()) {
            ...
            case MetatypeRepresentation::ObjC:
               Printer << "@objc_metatype ";
               break;
         }
      }
      printWithParensIfNotSimple(T->getInstanceType());

      // We spell normal metatypes of existential types as .Interface.
   ...
   void printFunctionExtInfo(AnyFunctionType::ExtInfo info) {
      ...
      if (Options.PrintFunctionRepresentationAttrs &&
          ...
          info.getPILRepresentation() != PILFunctionType::Representation::Thick) {
         Printer.callPrintStructurePre(PrintStructureKind::BuiltinAttribute);
         Printer.printAttrName("@convention");
         Printer << "(";
         // TODO: coalesce into a single convention attribute.
         switch (info.getPILRepresentation()) {
   ...
   void printFunctionExtInfo(PILFunctionType::ExtInfo info,
      ...
      if (Options.PrintFunctionRepresentationAttrs &&
          !Options.excludeAttrKind(TAK_convention) &&
          info.getRepresentation() != PILFunctionType::Representation::Thick) {
         Printer.callPrintStructurePre(PrintStructureKind::BuiltinAttribute);
         Printer.printAttrName("@convention");
         Printer << "(";
...
//      Printer.printStructurePost(PrintStructureKind::FunctionReturnType);
   ...
   void printPILCoroutineKind(PILCoroutineKind kind) {
      switch (kind) {
   ...
   void visitPILFunctionType(PILFunctionType *T) {
      printPILCoroutineKind(T->getCoroutineKind());
      printFunctionExtInfo(T->getExtInfo(),
   ...
   void visitPILBlockStorageType(PILBlockStorageType *T) {
      Printer << "@block_storage ";
      printWithParensIfNotSimple(T->getCaptureType());
   }

   void visitPILBoxType(PILBoxType *T) {
      {
   ...
   void visitInterfaceCompositionType(InterfaceCompositionType *T) {
...
//      visit(T->getObjectType());
   ...
   void visitGenericTypeParamType(GenericTypeParamType *T) {
      if (T->getDecl() == nullptr) {
         ...
         // When printing PIL types, use a generic environment to map them from
      ...
      if (Name.empty())
         ...
         if (T->getDecl() &&
             T->getDecl()->getDeclContext()->getSelfInterfaceDecl()) {
...
void PILParameterInfo::dump() const {
   print(llvm::errs());
   llvm::errs() << '\n';
}

void PILParameterInfo::print(raw_ostream &OS, const PrintOptions &Opts) const {
   StreamPrinter Printer(OS);
   print(Printer, Opts);
}

void PILParameterInfo::print(AstPrinter &Printer,
...
void PILResultInfo::dump() const {
   print(llvm::errs());
   llvm::errs() << '\n';
}

void PILResultInfo::print(raw_ostream &OS, const PrintOptions &Opts) const {
   StreamPrinter Printer(OS);
   print(Printer, Opts);
}

void PILResultInfo::print(AstPrinter &Printer, const PrintOptions &Opts) const {
...
void InterfaceConformance::printName(llvm::raw_ostream &os,
                                     const PrintOptions &PO) const {
   ...
   switch (getKind()) {
      case InterfaceConformanceKind::Normal: {
         auto normal = cast<NormalInterfaceConformance>(this);
         os << normal->getInterface()->getName()
            << " module " << normal->getDeclContext()->getParentModule()->getName();
         break;
      }
      case InterfaceConformanceKind::Self: {
         auto self = cast<SelfInterfaceConformance>(this);
         os << self->getInterface()->getName()
      ...
      case InterfaceConformanceKind::Inherited: {
         auto inherited = cast<InheritedInterfaceConformance>(this);
         os << "inherit (";
...
void getInheritedForPrinting(const Decl *decl, const PrintOptions &options,
   ...
   for (auto attr : decl->getAttrs().getAttributes<SynthesizedInterfaceAttr>()) {
      if (auto *proto = ctx.getInterface(attr->getInterfaceKind())) {
         if (!options.shouldPrint(proto))
            continue;
         if (attr->getInterfaceKind() == KnownInterfaceKind::RawRepresentable &&
             isa<EnumDecl>(decl) &&

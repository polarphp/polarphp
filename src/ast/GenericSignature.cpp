   ...
   for (unsigned idx : indices(canonicalRequirements)) {
      ...
      switch (reqt.getKind()) {
         ...
         case RequirementKind::Conformance:
            assert(reqt.getFirstType()->isTypeParameter() &&
                   "Left-hand side must be a type parameter");
            assert(isa<InterfaceType>(reqt.getSecondType().getPointer()) &&
      ...
      auto prevProto =
         prevReqt.getSecondType()->castTo<InterfaceType>()->getDecl();
...
GenericSignatureImpl::getConformsTo(Type type) {
   ...
   InterfaceType::canonicalizeInterfaces(result);

   return result;
}

bool GenericSignatureImpl::conformsToInterface(Type type, InterfaceDecl *proto) {
...
bool GenericSignatureImpl::isRequirementSatisfied(Requirement requirement) {
   ...
   switch (requirement.getKind()) {
      case RequirementKind::Conformance: {
         auto protocolType = requirement.getSecondType()->castTo<InterfaceType>();
         auto protocol = protocolType->getDecl();

         if (canFirstType->isTypeParameter())
            return conformsToInterface(canFirstType, protocol);
         else
...
/// requirements in it.
static bool hasNonCanonicalSelfInterfaceRequirement(
   ...
   for (; source; source = source->parent) {
      ...
      // build one now.
      auto inProto = source->getInterfaceDecl();
...
getBestRequirementSource(GenericSignatureBuilder &builder,
                         ArrayRef<GSBConstraint<InterfaceDecl *>> constraints) {
   ...
   for (const auto &constraint : constraints) {
      ...
      bool isNonCanonical =
         hasNonCanonicalSelfInterfaceRequirement(source, constraint.value);
...
void GenericSignatureImpl::buildConformanceAccessPath(
   ...
   if (source->isInterfaceRequirement()) {
      ...
      auto inInterface = source->getInterfaceDecl();
      buildConformanceAccessPath(path, reqs, source->parent, inInterface, rootType,
                                 requirementSignatureProto);
      assert(path.back().second == inInterface &&
      ...
      if (source->usesRequirementSignature) {
         ...
         subjectType = inInterface->getGenericSignature()
            ->getCanonicalTypeInContext(subjectType);

         assert(hasConformanceInSignature(inInterface->getRequirementSignature(),
      ...
      auto inProtoSig = inInterface->getGenericSignature();
      ...
      buildConformanceAccessPath(path, inInterface->getRequirementSignature(),
   ...
   if (source->kind == RequirementSource::Superclass ||
      ...
      auto conformance = source->getInterfaceConformance();
      (void) conformance;
...
GenericSignatureImpl::getConformanceAccessPath(Type type, InterfaceDecl *protocol) {

...
static_assert(std::is_trivially_destructible_v<LoadedFile>,
...
ModuleDecl::lookupExistentialConformance(Type type, InterfaceDecl *protocol) {
   AstContext &ctx = getAstContext();
   ...
   if (!layout.isObjC()) {
      ...
      if (protocol->requiresSelfConformanceWitnessTable() &&
          ...
          type->castTo<InterfaceType>()->getDecl() == protocol)
         return InterfaceConformanceRef(ctx.getSelfConformance(protocol));
...
InterfaceConformanceRef ModuleDecl::lookupConformance(Type type,
   ...
   if (auto archetype = type->getAs<ArchetypeType>()) {
      ...
      if (auto super = archetype->getSuperclass()) {
         if (auto inheritedConformance = lookupConformance(super, protocol)) {
            return InterfaceConformanceRef(ctx.getInheritedConformance(
      ...
      for (auto ap : archetype->getConformsTo()) {
         if (ap == protocol || ap->inheritsFrom(protocol))
            return InterfaceConformanceRef(protocol);
      }
   ...
   if (auto inherited = dyn_cast<InheritedInterfaceConformance>(conformance)) {
      ...
      return InterfaceConformanceRef(conformance);
   }
...
ModuleDecl::ReverseFullNameIterator::printForward(raw_ostream &out,
   ...
   interleave(llvm::reverse(elements),
...
bool ModuleDecl::isOnoneSupportModule() const {
   return !getParent() && getName().str() == POLAR_ONONE_SUPPORT;
}
...
void SourceFile::print(AstPrinter &Printer, const PrintOptions &PO) {
   std::set<DeclKind> MajorDeclKinds = {DeclKind::Class, DeclKind::Enum,
                                        DeclKind::Extension, DeclKind::Interface, DeclKind::Struct};
   for (auto decl : Decls) {
...
bool SourceFile::hasTestableOrPrivateImport(
   AccessLevel accessLevel, const ValueDecl *ofDecl,
...
// See polarphp/basic/Statistic.h for declaration: this enables tracing SourceFiles, is

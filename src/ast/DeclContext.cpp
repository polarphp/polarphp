...
InterfaceDecl *DeclContext::getSelfInterfaceDecl() const {
  return dyn_cast_or_null<InterfaceDecl>(getSelfTypeDecl());
}

InterfaceDecl *DeclContext::getExtendedInterfaceDecl() const {
  if (auto decl = const_cast<Decl*>(getAsDecl()))
    if (auto ED = dyn_cast<ExtensionDecl>(decl))
      return dyn_cast_or_null<InterfaceDecl>(ED->getExtendedNominal());
  return nullptr;
}

GenericTypeParamType *DeclContext::getInterfaceSelfType() const {
  assert(getSelfInterfaceDecl() && "not a protocol");

  GenericParamList *genericParams;
  if (auto proto = dyn_cast<InterfaceDecl>(this)) {
...
DeclContext *DeclContext::getParentForLookup() const {
  if (isa<InterfaceDecl>(this) || isa<ExtensionDecl>(this)) {
  ...
  if (isa<NominalTypeDecl>(this)) {
    ...
    // skip the protocol.
    if (isa<InterfaceDecl>(getParent()))
      return getModuleScopeContext();
  }
  return getParent();
}
...
ResilienceExpansionRequest::evaluate(Evaluator &evaluator,
...
bool DeclContext::hasValueSemantics() const {
  if (getExtendedInterfaceDecl())
    return !isClassConstrainedInterfaceExtension();
  return !getDeclaredInterfaceType()->hasReferenceSemantics();
}

bool DeclContext::isClassConstrainedInterfaceExtension() const {
  if (getExtendedInterfaceDecl()) {

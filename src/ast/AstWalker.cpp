                                    ...
                                    Pattern *, /*TypeRepr*/ bool>
{
  ...
  bool visitNominalTypeDecl(NominalTypeDecl *NTD) {
    ...
    // Visit requirements
    if (auto *Interface = dyn_cast<InterfaceDecl>(NTD)) {
      if (auto *WhereClause = Interface->getTrailingWhereClause()) {
        for (auto &Req: WhereClause->getRequirements()) {
          if (doIt(Req))
            return true;
        }
...
bool Traversal::visitInterfaceTypeRepr(InterfaceTypeRepr *T) {
  return doIt(T->getBase());
}
...
bool Traversal::visitPILBoxTypeRepr(PILBoxTypeRepr *T) {

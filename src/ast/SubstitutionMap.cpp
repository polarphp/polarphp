...
MutableArrayRef <InterfaceConformanceRef>
SubstitutionMap::getConformancesBuffer() {
   return storage ? storage->getConformances()
                  : MutableArrayRef<InterfaceConformanceRef>();
}

ArrayRef <InterfaceConformanceRef> SubstitutionMap::getConformances() const {
   return storage ? storage->getConformances()
...
SubstitutionMap SubstitutionMap::get(GenericSignature genericSig,
   ...
   for (const auto &req : genericSig->getRequirements()) {
      ...
      auto protoType = req.getSecondType()->castTo<InterfaceType>();
...
SubstitutionMap::lookupConformance(CanType type, InterfaceDecl *proto) const {
   if (empty())
   ...
   auto getInitialConformance =
      [&](Type type, InterfaceDecl *proto) -> InterfaceConformanceRef {
         ...
         for (const auto &req : getGenericSignature()->getRequirements()) {
            ...
            if (req.getFirstType()->isEqual(type) &&
                req.getSecondType()->castTo<InterfaceType>()->getDecl() == proto) {
   ...
   if (!genericSig->conformsToInterface(type, proto))
      return InterfaceConformanceRef::forInvalid();
   ...
   for (const auto &step : accessPath) {
      ...
      if (normal->getSignatureConformances().empty()) {
         ...
         // to use that.
         if (normal->getState() == InterfaceConformanceState::CheckingTypeWitnesses)
            return InterfaceConformanceRef::forInvalid();
      }
...
SubstitutionMap
SubstitutionMap::getInterfaceSubstitutions(InterfaceDecl *protocol,
                                          Type selfType,
                                          InterfaceConformanceRef conformance) {
   return get(protocol->getGenericSignature(),
              llvm::makeArrayRef<Type>(selfType),
              llvm::makeArrayRef<InterfaceConformanceRef>(conformance));
}
...
SubstitutionMap::combineSubstitutionMaps(SubstitutionMap firstSubMap,
   ...
   return get(
      ...
      [&](CanType type, Type substType, InterfaceDecl *proto) {
...
void SubstitutionMap::verify() const {
   ...
   for (const auto &req : getGenericSignature()->getRequirements()) {
      ...
      if (substType->isExistentialType()) {
         assert(isa<SelfInterfaceConformance>(conformance.getConcrete()) &&

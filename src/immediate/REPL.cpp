...
class REPLEnvironment {
  ...
  llvm::SmallPtrSet<polar::ModuleDecl *, 8> ImportedModules;
...
private:
  ...
  bool executeSwiftSource(llvm::StringRef Line, const ProcessCmdLine &CmdLine) {
    ...
    // PILGen the module and produce PIL diagnostics.
    std::unique_ptr<PILModule> sil;
    
    if (!CI.getASTContext().hadError()) {
      // We don't want anything to get stripped, so pretend we're doing a
      // non-whole-module generation.
      sil = performPILGeneration(*M->getFiles().front(), CI.getPILTypes(),
                                 CI.getPILOptions());
      runPILDiagnosticPasses(*sil);
      runPILOwnershipEliminatorPass(*sil);
      runPILLoweringPasses(*sil);
    }
...
void polar::runREPL(CompilerInstance &CI, const ProcessCmdLine &CmdLine,
                    bool ParseStdlib) {

...
///
/// Helper for swift::atomically_writing_to_file.
///
...
try_to_open_temporary_file(Optional<llvm::raw_fd_ostream> &openedStream,
                           const StringRef outputPath)
{
   ...
   std::error_code errorCode = fs::createUniqueFile(tempPath, fd, tempPath, perms);
      ...
      const llvm::function_ref<void(llvm::raw_pwrite_stream &)> action)
{
   ...
   llvm::ErrorOr<bool> canUseTemporary = can_use_temporary_for_write(outputPath);
   if (std::error_code error = canUseTemporary.getError()) {
      return error;
   }
   Optional<std::string> temporaryPath;
   {
      Optional<llvm::raw_fd_ostream> ostream;
      if (canUseTemporary.get()) {
         temporaryPath = try_to_open_temporary_file(ostream, outputPath);
         if (!temporaryPath) {
            assert(!ostream.hasValue());
      ...
      if (!ostream.hasValue()) {
         ...
         ostream.emplace(outputPath, error, fs::F_None);
         if (error) {
            return error;
         }
      }
      action(ostream.getValue());
      // In addition to scoping the use of 'ostream', ending the scope here also
   ...
   return move_file_if_different(temporaryPath.getValue(), outputPath);
}
...
vfs::get_file_or_stdin(llvm::vfs::FileSystem &fs,

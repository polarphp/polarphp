...
llvm::CallingConv::ID irgen::expandCallingConv(IRGenModule &IGM,
                                               PILFunctionTypeRepresentation convention) {
   switch (convention) {
...
public:
   ...
   SignatureExpansion(IRGenModule &IGM, CanPILFunctionType fnType)
...
private:
   void expand(PILParameterInfo param);
   llvm::Type *addIndirectResult();

   PILFunctionConventions getPILFuncConventions() const {
      return PILFunctionConventions(FnType, IGM.getPILModule());
   }
...
llvm::Type *SignatureExpansion::addIndirectResult() {
   auto resultType = getPILFuncConventions().getPILResultType();
...
void SignatureExpansion::expandResult() {
   ...
   auto fnConv = getPILFuncConventions();

   // Disable the use of sret if we have multiple indirect results.
   if (fnConv.getNumIndirectPILResults() > 1)
      CanUseSRet = false;
...
class YieldSchema {
   ...
   bool IsIndirect;
public:
   YieldSchema(IRGenModule &IGM, PILFunctionConventions fnConv,
               PILYieldInfo yield)
      : YieldTy(fnConv.getPILType(yield)),
...
// TODO: Direct to Indirect result conversion could be handled in a PIL
...
llvm::Type *SignatureExpansion::expandDirectResult() {
   ...
   llvm_unreachable("Not a valid PILFunctionLanguage.");
}
...
void SignatureExpansion::expandExternalSignatureTypes() {
   assert(FnType->getLanguage() == PILFunctionLanguage::C);

   // Convert the PIL result type to a Clang type.
   auto clangResultTy =
      IGM.getClangType(FnType->getFormalCSemanticResult(IGM.getPILModule()));
   ...
   if (FnType->getRepresentation() == PILFunctionTypeRepresentation::Block) {
   ...
   for (auto i : indices(paramTys).slice(firstParamToLowerNormally)) {
      ...
      switch (AI.getKind()) {
         ...
         case clang::CodeGen::ABIArgInfo::Indirect: {
            ...
            auto paramTy = getPILFuncConventions().getPILType(param);
...
void SignatureExpansion::expand(PILParameterInfo param) {
   auto paramPILType = getPILFuncConventions().getPILType(param);
   auto &ti = IGM.getTypeInfo(paramPILType);
   switch (auto conv = param.getConvention()) {
      ...
      case ParameterConvention::Indirect_In_Guaranteed:
         ...
         addPointerParameter(
            IGM.getStorageType(getPILFuncConventions().getPILType(param)));
         return;
...
bool irgen::hasSelfContextParameter(CanPILFunctionType fnType) {
   if (!fnType->hasSelfParam())
      return false;

   PILParameterInfo param = fnType->getSelfParameter();
...
/// Expand the abstract parameters of a PIL function type into the physical
...
void SignatureExpansion::expandParameters() {
   ...
   if (FnType->hasErrorResult()) {
      ...
      llvm::Type *errorType = IGM.getStorageType(
         getPILFuncConventions().getPILType(FnType->getErrorResult()));
...
/// Expand the result and parameter types of a PIL function into the
...
Signature SignatureExpansion::getSignature() {
   ...
   assert((ForeignInfo.ClangInfo != nullptr) ==
          (FnType->getLanguage() == PILFunctionLanguage::C) &&
   ...
   if (FnType->getLanguage() == PILFunctionLanguage::C) {
...
void CallEmission::emitToUnmappedExplosion(Explosion &out) {
   ...
   PILFunctionConventions fnConv(getCallee().getOrigFunctionType(),
                                 IGF.getPILModule());
...
//   }
   ...
   // Specially handle noreturn c function which would return a 'Never' PIL result
   // type.
   if (origFnType->getLanguage() == PILFunctionLanguage::C &&
       origFnType->isNoReturnFunction(IGF.getPILModule())) {
   ...
   auto resultType = fnConv.getPILResultType();
...
void CallEmission::emitToUnmappedMemory(Address result) {
   ...
   PILFunctionConventions FnConv(CurCallee.getSubstFunctionType(),
                                 IGF.getPILModule());
   addIndirectResultAttributes(IGF.IGM, CurCallee.getMutableAttributes(),
                               0, FnConv.getNumIndirectPILResults() <= 1);
#ifndef NDEBUG
...
void CallEmission::emitToMemory(Address addr,
   ...
   // indirect results here, just the possibility of a direct-in-PIL
   // result that's actually being passed indirectly.
   //
   // TODO: PIL address lowering should be able to handle such cases earlier.
   auto origResultType = origFnType->getDirectFormalResultsType(IGF.IGM.getPILModule())
      .getAstType();
   auto substResultType = substFnType->getDirectFormalResultsType(IGF.IGM.getPILModule())
      .getAstType();
...
void CallEmission::emitYieldsToExplosion(Explosion &out) {
   ...
   if (!rawReturnValues.empty()) {
      ...
      Address indirectBuffer(indirectPointer, Alignment(layout->getAlignment().value()));
   ...
   PILFunctionConventions fnConv(substCoroType, IGF.getPILModule());
...
void CallEmission::emitToExplosion(Explosion &out, bool isOutlined) {
   ...
   PILFunctionConventions fnConv(getCallee().getSubstFunctionType(),
                                 IGF.getPILModule());
   PILType substResultType = fnConv.getPILResultType();
   ...
   auto isNoReturnCFunction =
      origFnType->getLanguage() == PILFunctionLanguage::C &&
      origFnType->isNoReturnFunction(IGF.getPILModule());
...
Callee::Callee(CalleeInfo &&info, const FunctionPointer &fn,
   ...
   assert((Fn.getForeignInfo().ClangInfo != nullptr) ==
          (Info.OrigFnType->getLanguage() == PILFunctionLanguage::C));
...
void CallEmission::setFromCallee() {
   ...
   if (fnType->hasErrorResult()) {
      ...
      PILFunctionConventions fnConv(fnType, IGF.getPILModule());
      Address errorResultSlot = IGF.getErrorResultSlot(fnConv.getPILErrorType());
...
static void emitCoerceAndExpand(IRGenFunction &IGF, Explosion &in,
   ...
   Alignment coercionTyAlignment = Alignment(coercionTyLayout->getAlignment().value());
   auto alloca = cast<llvm::AllocaInst>(temporary.getAddress());
   if (alloca->getAlignment() < coercionTyAlignment.getValue()) {
      alloca->setAlignment(llvm::Align(coercionTyAlignment.getValue()));
...
static void emitDirectExternalArgument(IRGenFunction &IGF, PILType argType,
...
emitClangExpandedArgument(IRGenFunction &IGF, Explosion &in, Explosion &out,
                          clang::CanQualType clangType, PILType swiftType,
...
static void externalizeArguments(IRGenFunction &IGF, const Callee &callee,
   ...
   auto silConv = PILFunctionConventions(fnType, IGF.IGM.silConv);
   auto params = fnType->getParameters();
   ...
   if (callee.getRepresentation() == PILFunctionTypeRepresentation::ObjCMethod) {
   ...
   for (unsigned i = firstParam, e = FI.arg_size(); i != e; ++i) {
      ...
      // swiftcall function pointers through PIL as C functions anyway.
      ...
      PILType paramType = silConv.getPILType(params[i - firstParam]);
      switch (AI.getKind()) {
         case clang::CodeGen::ABIArgInfo::Extend: {
         ...
         case clang::CodeGen::ABIArgInfo::Direct: {
            ...
            if (silConv.isPILIndirect(params[i - firstParam])) {
               assert(paramType.isAddress() && "PIL type is not an address?");
         ...
         case clang::CodeGen::ABIArgInfo::Indirect: {
            ...
            if (AI.getIndirectByVal()) {
               ...
               if (ABIAlign > addr.getAlignment()) {
                  ...
                  AS->setAlignment(llvm::Align(ABIAlign.getQuantity()));
...
bool irgen::addNativeArgument(IRGenFunction &IGF,
                              ...
                              PILParameterInfo origParamInfo, Explosion &out,
                              bool isOutlined) {
   // Addresses consist of a single pointer argument.
   if (IGF.IGM.silConv.isPILIndirect(origParamInfo)) {
      out.add(in.claimNext());
      return false;
   }
   auto paramType = IGF.IGM.silConv.getPILType(origParamInfo, fnTy);
   auto &ti = cast<LoadableTypeInfo>(IGF.getTypeInfo(paramType));
...
static void emitDirectForeignParameter(IRGenFunction &IGF, Explosion &in,
                                       const clang::CodeGen::ABIArgInfo &AI,
                                       Explosion &out, PILType paramType,
                                       const LoadableTypeInfo &paramTI) {
...
void irgen::emitForeignParameter(IRGenFunction &IGF, Explosion &params,
                                 ForeignFunctionInfo foreignInfo,
                                 unsigned foreignParamIndex, PILType paramTy,
                                 const LoadableTypeInfo &paramTI,
...
llvm::Value *irgen::emitYield(IRGenFunction &IGF,
                              CanPILFunctionType coroutineType,
...
void CallEmission::setArgs(Explosion &original, bool isOutlined,
   ...
   PILFunctionConventions fnConv(origCalleeType, IGF.getPILModule());

   // Pass along the indirect result pointers.
   original.transferInto(adjusted, fnConv.getNumIndirectPILResults());
...
Address IRGenFunction::getErrorResultSlot(PILType errorType) {
   if (!ErrorResultSlot) {
...
Explosion IRGenFunction::coerceValueTo(PILType fromTy, Explosion &from,
                                       PILType toTy) {
   if (fromTy == toTy)
...
void IRGenFunction::emitScalarReturn(PILType returnResultType,
                                     PILType funcResultType, Explosion &result,
...
Callee irgen::getPolarphpFunctionPointerCallee(
...
FunctionPointer::getExplosionValue(IRGenFunction &IGF,
                                   CanPILFunctionType fnType) const {

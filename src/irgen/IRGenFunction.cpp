...
PILModule &IRGenFunction::getPILModule() const {
   return IGM.getPILModule();
}

lowering::TypeConverter &IRGenFunction::getPILTypes() const {
   return IGM.getPILTypes();
}
...
Atomicity IRGenFunction::getDefaultAtomicity() {
   return getPILModule().isDefaultAtomic() ? Atomicity::Atomic : Atomicity::NonAtomic;
}

...
void TypeInfo::assign(IRGenFunction &IGF, Address dest, Address src,
                      IsTake_t isTake, PILType T, bool isOutlined) const {
   if (isTake) {
...
LoadedRef LoadableTypeInfo::loadRefcountedPtr(IRGenFunction &IGF,
   ...
   llvm::report_fatal_error("loadRefcountedPtr: Invalid PIL in IRGen");
}
...
llvm::Value *FixedTypeInfo::getSize(IRGenFunction &IGF, PILType T) const {
...
llvm::Value *FixedTypeInfo::getStride(IRGenFunction &IGF, PILType T) const {
   return FixedTypeInfo::getStaticStride(IGF.IGM);
}
llvm::Value *FixedTypeInfo::getIsPOD(IRGenFunction &IGF, PILType T) const {
   return llvm::ConstantInt::get(IGF.IGM.Int1Ty,
                                 isPOD(ResilienceExpansion::Maximal) == IsPOD);
}
llvm::Value *FixedTypeInfo::getIsBitwiseTakable(IRGenFunction &IGF, PILType T) const {
   return llvm::ConstantInt::get(IGF.IGM.Int1Ty,
...
public:
   ...
   void destroy(IRGenFunction &IGF, Address address, PILType T,
                bool isOutlined) const override {
      /* nop */
   }
...
TypeConverter::TypeConverter(IRGenModule &IGM)
   ...
   if (path.empty()) {
      ...
      // one.
      auto platformName = get_platform_name_for_triple(Triple);
      auto archName = polar::get_major_architecture_name(Triple);
...
/// have yet undergone PIL type lowering.  The type can serve as its own
/// abstraction pattern.
...
/// Get the fragile type information for the given type, which may not
/// have yet undergone PIL type lowering.
const TypeInfo &
...
/// to have undergone PIL type lowering (or be one of the types for
...
const TypeInfo &IRGenFunction::getTypeInfo(PILType T) {
   return IGM.getTypeInfo(T);
}

/// Return the PIL-lowering of the given type.
PILType IRGenModule::getLoweredType(AbstractionPattern orig, Type subst) const {
   return getPILTypes().getLoweredType(
      orig, subst, TypeExpansionContext::maximalResilienceExpansionOnly());
}

/// Return the PIL-lowering of the given type.
PILType IRGenModule::getLoweredType(Type subst) const {
   return getPILTypes().getLoweredType(
      subst, TypeExpansionContext::maximalResilienceExpansionOnly());
}

/// Return the PIL-lowering of the given type.
const lowering::TypeLowering &IRGenModule::getTypeLowering(PILType type) const {
   return getPILTypes().getTypeLowering(
      type, TypeExpansionContext::maximalResilienceExpansionOnly());
}

bool IRGenModule::isTypeABIAccessible(PILType type) const {
   return getPILModule().isTypeABIAccessible(
      type, TypeExpansionContext::maximalResilienceExpansionOnly());
}
...
llvm::PointerType *IRGenModule::getStoragePointerType(PILType T) {
...
llvm::Type *IRGenModule::getStorageType(PILType T) {
...
/// yet undergone PIL type lowering.  The type can serve as its own
/// abstraction pattern.
...
/// to have undergone PIL type lowering (or be one of the types for
/// which that lowering is the identity function).
const TypeInfo &IRGenModule::getTypeInfo(PILType T) {
...
CanType TypeConverter::getExemplarType(CanType contextTy) {
   // FIXME: A generic PILFunctionType should not contain any nondependent
   // archetypes.
   if (isa<PILFunctionType>(contextTy)
       && cast<PILFunctionType>(contextTy)->isPolymorphic()) {
      return contextTy;
   } else {
...
const TypeInfo *TypeConverter::getTypeEntry(CanType canonicalTy) {
   ...
   if (contextTy->hasTypeParameter()) {
      // The type we got should be lowered, so lower it like a PILType.
      contextTy = getGenericEnvironment()->mapTypeIntoContext(
         IGM.getPILModule(),
         PILType::getPrimitiveAddressType(contextTy)).getAstType();
   }
...
#include "polarphp/ast/TypeNodesDef.h"
      ...
      case TypeKind::GenericFunction:
         llvm_unreachable("AST FunctionTypes should be lowered by PILGen");
      case TypeKind::PILFunction:
         return convertFunctionType(cast<PILFunctionType>(ty));
      case TypeKind::Interface:
         return convertInterfaceType(cast<InterfaceType>(ty));
      case TypeKind::InterfaceComposition:
         return convertInterfaceCompositionType(cast<InterfaceCompositionType>(ty));
...
#include "polarphp/ast/ReferenceStorageDef.h"
      case TypeKind::PILBlockStorage: {
         return convertBlockStorageType(cast<PILBlockStorageType>(ty));
         case TypeKind::PILBox:
            return convertBoxType(cast<PILBoxType>(ty));
         case TypeKind::PILToken:
            llvm_unreachable("should not be asking for representation of a PILToken");
      }
   }
...
public:
   ...
   virtual void destroy(IRGenFunction &IGF, Address address, PILType T,
                        bool isOutlined) const override {
      llvm_unreachable("TypeConverter::Mode::Legacy is not for real values");
   }
};
...
/// protocol composition type.  Interface composition types are
...
llvm::StructType *
IRGenModule::createNominalType(InterfaceCompositionType *type) {
   IRGenMangler Mangler;
   std::string typeName = Mangler.mangleInterfaceForLLVMTypeName(type);
   return llvm::StructType::create(getLLVMContext(), StringRef(typeName));
}
...
PILType irgen::getSingletonAggregateFieldType(IRGenModule &IGM, PILType t,

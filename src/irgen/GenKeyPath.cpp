...
#include "polarphp/abi/KeyPath.h"
...
getAccessorForComputedComponent(IRGenModule &IGM,
   ...
   if (requirements.empty() &&
       !is_available_externally(accessor->getLinkage()) &&
       &IGM == IGM.IRGen.getGenModule(accessor)) {
      return IGM.getAddrOfPILFunction(accessor, NotForDefinition);
   }
   auto accessorFn = IGM.getAddrOfPILFunction(accessor, NotForDefinition);
...
getLayoutFunctionForComputedComponent(IRGenModule &IGM,
...

   {
      ...
      for (auto &index : component.getSubscriptIndices()) {
         auto ty = genericEnv
                   ? genericEnv->mapTypeIntoContext(IGM.getPILModule(), index.LoweredType)
                   : index.LoweredType;
...
getWitnessTableForComputedComponent(IRGenModule &IGM,
   ...
   for (auto &component : component.getSubscriptIndices()) {
      auto ty = genericEnv
                ? genericEnv->mapTypeIntoContext(IGM.getPILModule(), component.LoweredType)
   ...
   if (isTrivial) {
      ...
      // Generate a destructor for this set of indices.
      {
         ...
         for (auto &component : component.getSubscriptIndices()) {
            auto ty = genericEnv
                      ? genericEnv->mapTypeIntoContext(IGM.getPILModule(),
                                                       component.LoweredType)
                      : component.LoweredType;
            auto &ti = IGM.getTypeInfo(ty);
            if (offset) {
   ...
   switch (auto kind = component.getKind()) {
      case KeyPathPatternComponent::Kind::StoredProperty: {
         ...
         if (loweredClassTy.getClassOrBoundGenericClass()) {
            ...
            auto loweredBaseContextTy =
               PILType::getPrimitiveObjectType(loweredClassTy.getAstType());
            if (!loweredClassTy.getAstType()->hasArchetype())
               loweredBaseContextTy = PILType::getPrimitiveObjectType(
      ...
      case KeyPathPatternComponent::Kind::SettableProperty: {
                  ...
                  [&](GenericRequirement reqt) {
                     ...
                     if (!reqt.Interface) {
                        ...
                        auto conformance = subs.lookupConformance(
                           reqt.TypeParameter->getCanonicalType(), reqt.Interface);
         ...
         switch (id.getKind()) {
            case KeyPathPatternComponent::ComputedPropertyId::Function: {
               ...
               auto idRef = IGM.getAddrOfLLVMVariableOrGOTEquivalent(
                  LinkEntity::forPILFunction(id.getFunction(), false));
         ...
         if (!isInstantiableOnce) {
            ...
            // arguments in the component. Thunk the PIL-level accessors to give the
      ...
      case KeyPathPatternComponent::Kind::TupleElement:
         ...
         PILType loweredTy = IGM.getLoweredType(AbstractionPattern::getOpaque(),
                                                baseTy);
...
void IRGenModule::emitPILProperty(PILProperty *prop) {
   if (prop->isTrivial()) {
      ...
      if (!TheTrivialPropertyDescriptor) {
         ...
         ConstantStructBuilder fields = builder.beginStruct();
         fields.addInt32(
            _PolarphpKeyPathComponentHeader_TrivialPropertyDescriptorMarker);

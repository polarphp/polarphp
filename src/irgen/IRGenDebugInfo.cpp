...
class IRGenDebugInfoImpl : public IRGenDebugInfo {
   ...
   /// Various caches.
   /// @{
   llvm::DenseMap<const PILDebugScope *, llvm::TrackingMDNodeRef> ScopeCache;
   llvm::DenseMap<const PILDebugScope *, llvm::TrackingMDNodeRef> InlinedAtCache;
   llvm::DenseMap<const void *, PILLocation::DebugLoc> DebugLocCache;
...
public:
   ...
   void setCurrentLoc(IRBuilder &Builder, const PILDebugScope *DS,
                      PILLocation Loc);
   ...
   void pushLoc();
   void popLoc();
   void setInlinedTrapLocation(IRBuilder &Builder, const PILDebugScope *Scope);
   void setEntryPointLoc(IRBuilder &Builder);
   llvm::DIScope *getEntryPointFn();
   llvm::DIScope *getOrCreateScope(const PILDebugScope *DS);
   void emitImport(ImportDecl *D);
   llvm::DISubprogram *emitFunction(const PILDebugScope *DS, llvm::Function *Fn,
                                    PILFunctionTypeRepresentation Rep,
                                    PILType Ty, DeclContext *DeclCtx = nullptr);
   llvm::DISubprogram *emitFunction(PILFunction &PILFn, llvm::Function *Fn);
   void emitArtificialFunction(IRBuilder &Builder, llvm::Function *Fn,
                               PILType PILTy);
   void emitVariableDeclaration(IRBuilder &Builder,
                                ArrayRef<llvm::Value *> Storage,
                                DebugTypeInfo Ty, const PILDebugScope *DS,
                                ValueDecl *VarDecl, PILDebugVariable VarInfo,
                                IndirectionKind = DirectValue,
                                ArtificialKind = RealValue);
   void emitDbgIntrinsic(IRBuilder &Builder, llvm::Value *Storage,
                         llvm::DILocalVariable *Var, llvm::DIExpression *Expr,
                         unsigned Line, unsigned Col, llvm::DILocalScope *Scope,
                         const PILDebugScope *DS);
   ...
   PILLocation::DebugLoc decodeSourceLoc(SourceLoc SL);
...
private:
   ...
   using DebugLoc = PILLocation::DebugLoc;
   ...
   /// Use the Polarphp SM to figure out the actual line/column of a SourceLoc.
   ...
   DebugLoc getStartLocation(Optional<PILLocation> OptLoc) {
      if (!OptLoc)
         return {};
   ...
   static bool isExplicitClosure(const PILFunction *PILFn) {
      if (PILFn && PILFn->hasLocation())
         if (Expr *E = PILFn->getLocation().getAsAstNode<Expr>())
            if (isa<ClosureExpr>(E))
               return true;
      return false;
   }

   llvm::MDNode *createInlinedAt(const PILDebugScope *DS) {
      ...
      auto Scope = getOrCreateScope(CS->Parent.dyn_cast<const PILDebugScope *>());
   ...
   static bool parentScopesAreSane(const PILDebugScope *DS) {
      auto *Parent = DS;
      while ((Parent = Parent->Parent.dyn_cast<const PILDebugScope *>())) {
   ...
   bool lineEntryIsSane(PILLocation::DebugLoc DL, const PILDebugScope *DS);

#endif

   llvm::DIFile *getOrCreateFile(StringRef Filename) {
      if (Filename.empty())
         Filename = PILLocation::getCompilerGeneratedDebugLoc().Filename;
   ...
   StringRef getName(PILLocation L) {
      if (L.isNull())
         return StringRef();
   ...
   static CanPILFunctionType getFunctionType(PILType PILTy) {
      if (!PILTy)
         return CanPILFunctionType();

      auto FnTy = PILTy.getAs<PILFunctionType>();
      if (!FnTy) {
         LLVM_DEBUG(llvm::dbgs() << "Unexpected function type: ";
                       PILTy.print(llvm::dbgs());
                       llvm::dbgs() << "\n");
         return CanPILFunctionType();
      }
   ...
   llvm::DIScope *getOrCreateContext(DeclContext *DC) {
      ...
      if (isa<FuncDecl>(DC))
         if (auto *Decl = IGM.getPILModule().lookUpFunction(PILDeclRef(
            cast<AbstractFunctionDecl>(DC), PILDeclRef::Kind::Func)))
   ...
   // This is different from PILFunctionType::getAllResultsType() in some subtle
   // ways.
   static PILType getResultTypeForDebugInfo(IRGenModule &IGM,
                                            CanPILFunctionType fnTy) {
      if (fnTy->getNumResults() == 1) {
         return fnTy->getResults()[0].getPILStorageType(IGM.getPILModule(), fnTy);
      } else if (!fnTy->getNumIndirectFormalResults()) {
         return fnTy->getDirectFormalResultsType(IGM.getPILModule());
      } else {
         SmallVector<TupleTypeElt, 4> eltTys;
         for (auto &result : fnTy->getResults()) {
            eltTys.push_back(result.getReturnValueType(IGM.getPILModule(), fnTy));
         }
   ...
   llvm::DITypeRefArray createParameterTypes(PILType PILTy) {
      if (!PILTy)
         return nullptr;
      return createParameterTypes(PILTy.castTo<PILFunctionType>());
   }

   llvm::DITypeRefArray createParameterTypes(CanPILFunctionType FnTy) {
      ...
      for (auto Param : FnTy->getParameters())
         createParameterType(Parameters, IGM.silConv.getPILType(Param, FnTy));

      return DBuilder.getOrCreateTypeArray(Parameters);
   }

   /// FIXME: replace this condition with something more sane.
   static bool isAllocatingConstructor(PILFunctionTypeRepresentation Rep,
                                       DeclContext *DeclCtx) {
      return Rep != PILFunctionTypeRepresentation::Method && DeclCtx &&
   ...
   StringRef getMangledName(DebugTypeInfo DbgTy) {
      ...
      if (!Opts.DisableRoundTripDebugTypes
         ...
         Type Reconstructed = demangling::getTypeForMangling(Ctx, Result);
         if (!Reconstructed) {
      ...
      for (VarDecl *VD : D->getStoredProperties()) {
         ...
         auto DbgTy = DebugTypeInfo::getFromTypeInfo(
            ...
            IGM.getTypeInfoForUnlowered(
               IGM.getPILTypes().getAbstractionPattern(VD), memberTy));
   ...
   llvm::DIType *createFunctionPointer(DebugTypeInfo DbgTy, llvm::DIScope *Scope,
      ...
      CanPILFunctionType FunTy;
      TypeBase *BaseTy = DbgTy.getType();
      if (auto *PILFnTy = dyn_cast<PILFunctionType>(BaseTy))
         FunTy = CanPILFunctionType(PILFnTy);
         // FIXME: Handling of generic parameters in PIL type lowering is in flux.
         ...
         FunTy = IGM.getLoweredType(nongenericTy).castTo<PILFunctionType>();
      } else
         FunTy = IGM.getLoweredType(BaseTy).castTo<PILFunctionType>();
      auto Params = createParameterTypes(FunTy);

      auto FnTy = DBuilder.createSubroutineType(Params, Flags);
      llvm::DIType *DITy;
      if (FunTy->getRepresentation() == PILFunctionType::Representation::Thick) {
   ...
   llvm::DIType *createType(DebugTypeInfo DbgTy, StringRef MangledName,
      ...
      switch (BaseTy->getKind()) {
         ...
         case TypeKind::Interface: {
            auto *InterfaceTy = BaseTy->castTo<InterfaceType>();
            auto *Decl = InterfaceTy->getDecl();
         ...
         case TypeKind::InterfaceComposition: {
            ...
            // FIXME: emit types
            // auto InterfaceCompositionTy = BaseTy->castTo<InterfaceCompositionType>();
            return createOpaqueStruct(Scope, Decl ? Decl->getNameStr() : MangledName,
                                      File, L.Line, SizeInBits, AlignInBits, Flags,
                                      MangledName);
         }
         ...
         case TypeKind::NestedArchetype: {
            ...
            SmallVector<llvm::Metadata *, 4> Interfaces;
            for (auto *InterfaceDecl : Archetype->getConformsTo()) {
               auto PTy = IGM.getLoweredType(InterfaceDecl->getInterfaceType())
                  .getAstType();
               auto PDbgTy = DebugTypeInfo::getFromTypeInfo(
                  InterfaceDecl->getInterfaceType(), IGM.getTypeInfoForLowered(PTy));
            ...
            auto DITy = DBuilder.createStructType(
               ...
               DerivedFrom, DBuilder.getOrCreateArray(Interfaces),
               llvm::dwarf::DW_LANG_Swift, nullptr, MangledName);
...
#include "polarphp/ast/ReferenceStorageDef.h"
   ...
   static bool canMangle(TypeBase *Ty) {
      switch (Ty->getKind()) {
         ...
         case TypeKind::PILBlockStorage: // Not supported at all.
         case TypeKind::PILBox:
            return false;
         default:
            return true;
...
IRGenDebugInfoImpl::IRGenDebugInfoImpl(const IRGenOptions &Opts,
   ...
   std::string Producer = version::retrieve_polarphp_full_version(
      IGM.Context.LangOpts.EffectiveLanguageVersion);
   StringRef Flags = Opts.DebugFlags;
   unsigned Major, Minor;
   std::tie(Major, Minor) = version::retrieve_polarphp_numeric_version();
...
#ifndef NDEBUG
bool IRGenDebugInfoImpl::lineEntryIsSane(PILLocation::DebugLoc DL,
                                         const PILDebugScope *DS) {
...
void IRGenDebugInfoImpl::setCurrentLoc(IRBuilder &Builder,
   ...
   PILFunction *Fn = DS->getInlinedFunction();
   if (Fn && (Fn->isThunk() || Fn->isTransparent())) {
      L = PILLocation::getCompilerGeneratedDebugLoc();
...
void IRGenDebugInfoImpl::addFailureMessageToCurrentLoc(IRBuilder &Builder,
   ...
   // TrapSc is the PIL debug scope which corresponds to TrapSP in the LLVM debug
   // info.
   RegularLocation ALoc = RegularLocation::getAutoGeneratedLocation();
   const PILDebugScope *TrapSc = new (IGM.getPILModule()) PILDebugScope(ALoc);
...
void IRGenDebugInfoImpl::setInlinedTrapLocation(IRBuilder &Builder,
   ...
   const PILDebugScope *TheLastScope = LastScope;
...
llvm::DIScope *IRGenDebugInfoImpl::getEntryPointFn() {
   ...
   if (!EntryPointFn) {
      EntryPointFn = DBuilder.createReplaceableCompositeType(
         llvm::dwarf::DW_TAG_subroutine_type, POLAR_ENTRY_POINT_FUNCTION,
         MainFile, MainFile, 0);
   }
   return EntryPointFn;
}

llvm::DIScope *IRGenDebugInfoImpl::getOrCreateScope(const PILDebugScope *DS) {
   if (DS == nullptr)
      return MainFile;
   ...
   if (auto *PILFn = DS->Parent.dyn_cast<PILFunction *>()) {
      auto *FnScope = PILFn->getDebugScope();
      // FIXME: This is a bug in the PIL deserialization.
      if (!FnScope)
         PILFn->setDebugScope(DS);
      ...
      if (!PILFn->getName().empty() && !PILFn->isZombie())
         Fn = IGM.getAddrOfPILFunction(PILFn, NotForDefinition);
      auto *SP = emitFunction(*PILFn, Fn);
   ...
   auto *ParentScope = DS->Parent.get<const PILDebugScope *>();
...
llvm::DISubprogram *IRGenDebugInfoImpl::emitFunction(PILFunction &PILFn,
                                                     llvm::Function *Fn) {
   auto *DS = PILFn.getDebugScope();
   assert(DS && "PIL function has no debug scope");
   (void)DS;
   return emitFunction(PILFn.getDebugScope(), Fn, PILFn.getRepresentation(),
                       PILFn.getLoweredType(), PILFn.getDeclContext());
}

llvm::DISubprogram *
IRGenDebugInfoImpl::emitFunction(const PILDebugScope *DS, llvm::Function *Fn,
                                 PILFunctionTypeRepresentation Rep,
                                 PILType PILTy, DeclContext *DeclCtx) {
   ...
   // PIL function, hence the dyn_cast.
   auto *PILFn = DS ? DS->Parent.dyn_cast<PILFunction *>() : nullptr;

   StringRef LinkageName;
   if (Fn)
      LinkageName = Fn->getName();
   else if (DS)
      LinkageName = PILFn->getName();
   else
   ...
   PILLocation::DebugLoc L;
   if (!DS || (PILFn && (PILFn->isBare() || PILFn->isThunk() ||
                         PILFn->isTransparent()))) {
   ...
   if (PILFn && PILFn->getDeclContext())
      Scope = getOrCreateContext(PILFn->getDeclContext()->getParent());

   // We know that main always comes from MainFile.
   if (LinkageName == POLAR_ENTRY_POINT_FUNCTION) {
      File = MainFile;
      Line = 1;
      Name = LinkageName;
   }

   CanPILFunctionType FnTy = getFunctionType(PILTy);
   auto Params = Opts.DebugInfoLevel > IRGenDebugInfoLevel::LineTables
                 ? createParameterTypes(PILTy)
                 : nullptr;
   ...
   if ((Name.empty() && LinkageName != POLAR_ENTRY_POINT_FUNCTION &&
        !isExplicitClosure(PILFn)) ||
   ...
   if (FnTy &&
       FnTy->getRepresentation() == PILFunctionType::Representation::Block)
   ...
   if (FnTy)
      if (auto ErrorInfo = FnTy->getOptionalErrorResult()) {
         auto DTI = DebugTypeInfo::getFromTypeInfo(
            ...
            IGM.getTypeInfo(IGM.silConv.getPILType(*ErrorInfo, FnTy)));
...
void IRGenDebugInfoImpl::emitArtificialFunction(IRBuilder &Builder,
                                                ...
                                                PILType PILTy) {
   RegularLocation ALoc = RegularLocation::getAutoGeneratedLocation();
   const PILDebugScope *Scope = new (IGM.getPILModule()) PILDebugScope(ALoc);
   emitFunction(Scope, Fn, PILFunctionTypeRepresentation::Thin, PILTy);
...
void IRGenDebugInfoImpl::emitVariableDeclaration(
   ...
   const PILDebugScope *DS, ValueDecl *VarDecl, PILDebugVariable VarInfo,
...
void IRGenDebugInfoImpl::emitDbgIntrinsic(
   ...
   llvm::DILocalScope *Scope, const PILDebugScope *DS) {
...
PILLocation::DebugLoc IRGenDebugInfoImpl::decodeSourceLoc(SourceLoc SL) {
   auto &Cached = DebugLocCache[SL.getOpaquePointerValue()];
   if (Cached.Filename.empty())
      Cached = sanitizeCodeViewDebugLoc(PILLocation::decode(SL, SM));
   return Cached;
}
...
void IRGenDebugInfo::setCurrentLoc(IRBuilder &Builder, const PILDebugScope *DS,
                                   PILLocation Loc) {
...
llvm::DIScope *IRGenDebugInfo::getOrCreateScope(const PILDebugScope *DS) {
...
llvm::DISubprogram *
IRGenDebugInfo::emitFunction(const PILDebugScope *DS, llvm::Function *Fn,
                             PILFunctionTypeRepresentation Rep, PILType Ty,
                             DeclContext *DeclCtx,
...
llvm::DISubprogram *IRGenDebugInfo::emitFunction(PILFunction &PILFn,
                                                 llvm::Function *Fn) {
   return static_cast<IRGenDebugInfoImpl *>(this)->emitFunction(PILFn, Fn);
}
...
PILLocation::DebugLoc IRGenDebugInfo::decodeSourceLoc(SourceLoc SL) {
...
ArtificialLocation::ArtificialLocation(const PILDebugScope *DS,

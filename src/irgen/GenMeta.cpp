...
#include "polarphp/abi/TypeIdentity.h"
...
#include "polarphp/irgen/internal/ClassMetadataVisitor.h"
#include "polarphp/irgen/internal/ConstantBuilder.h"
#include "polarphp/irgen/internal/EnumMetadataVisitor.h"
#include "polarphp/irgen/internal/FixedTypeInfo.h"
#include "polarphp/irgen/internal/ForeignClassMetadataVisitor.h"
...
#include "polarphp/irgen/internal/GenValueWitness.h"
...
#include "polarphp/irgen/internal/ScalarTypeInfo.h"
#include "polarphp/irgen/internal/GenInterface.h"
#include "polarphp/irgen/internal/StructMetadataVisitor.h"
...
class InterfaceDescriptorBuilder
   : public ContextDescriptorBuilderBase<InterfaceDescriptorBuilder> {
...
public:
   InterfaceDescriptorBuilder(IRGenModule &IGM, InterfaceDecl *Proto,
                              PILDefaultWitnessTable *defaultWitnesses)
   ...
   ContextDescriptorKind getContextKind() {
      return ContextDescriptorKind::Interface;
   }
   ...
   uint16_t getKindSpecificFlags() {
      InterfaceContextDescriptorFlags flags;
      flags.setClassConstraint(Proto->requiresClass()
                               ? InterfaceClassConstraint::Class
                               : InterfaceClassConstraint::Any);
      flags.setSpecialInterface(getSpecialInterfaceID(Proto));
   ...
   void emit() {
      ...
      auto addr = IGM.getAddrOfInterfaceDescriptor(Proto,
                                                   B.finishAndCreateFuture());
      auto var = cast<llvm::GlobalVariable>(addr);
   ...
   struct RequirementInfo {
      InterfaceRequirementFlags Flags;
      llvm::Constant *DefaultImpl;
   };

   /// Build the information which will go into a InterfaceRequirement entry.
   RequirementInfo getRequirementInfo(const WitnessTableEntry &entry) {
      using Flags = InterfaceRequirementFlags;
      if (entry.isBase()) {
         assert(entry.isOutOfLineBase());
         auto flags = Flags(Flags::Kind::BaseInterface);
      ...
      PILDeclRef func(entry.getFunction());
   ...
   void addRequirements() {
      auto &pi = IGM.getInterfaceInfo(Proto, InterfaceInfoKind::Full);
      ...
      if (pi.getNumWitnesses() > 0) {
         ...
         // table.
         auto address =
            B.getAddrOfCurrentPosition(IGM.InterfaceRequirementStructTy);
         ...
         IGM.defineInterfaceRequirementsBaseDescriptor(Proto, address);
      }
      ...
      for (auto &entry : pi.getWitnessEntries()) {
            ...
            if (entry.isFunction()) {
               ...
               auto *descriptor =
                  B.getAddrOfCurrentPosition(
                     IGM.InterfaceRequirementStructTy);
         ...
         auto reqt = B.beginStruct(IGM.InterfaceRequirementStructTy);
   ...
   llvm::Constant *findDefaultWitness(PILDeclRef func) {
      ...
      for (auto &entry : DefaultWitnesses->getEntries()) {
         if (!entry.isValid() || entry.getKind() != PILWitnessTable::Method ||
            ...
            continue;
         return IGM.getAddrOfPILFunction(entry.getMethodWitness().Witness,
                                         NotForDefinition);
      }
   ...
   llvm::Constant *findDefaultTypeWitness(AssociatedTypeDecl *assocType) {
      ...
      for (auto &entry : DefaultWitnesses->getEntries()) {
         if (!entry.isValid() ||
             entry.getKind() != PILWitnessTable::AssociatedType ||
   ...
   llvm::Constant *findDefaultAssociatedConformanceWitness(
      ...
      for (auto &entry : DefaultWitnesses->getEntries()) {
         if (!entry.isValid() ||
             entry.getKind() != PILWitnessTable::AssociatedTypeInterface ||
             entry.getAssociatedTypeInterfaceWitness().Interface != requirement ||
             entry.getAssociatedTypeInterfaceWitness().Requirement != association)
            continue;

         auto witness = entry.getAssociatedTypeInterfaceWitness().Witness;
   ...
   void defineDefaultAssociatedConformanceAccessFunction(
      ...
      if (conformance.isConcrete()) {
         auto conformanceI = &IGM.getConformanceInfo(associatedInterface,
   ...
   void addAssociatedTypeNames() {
      ...
      auto &pi = IGM.getInterfaceInfo(Proto,
...
class ClassContextDescriptorBuilder
   : public TypeContextDescriptorBuilderBase<ClassContextDescriptorBuilder,
      ClassDecl>,
     public PILVTableVisitor<ClassContextDescriptorBuilder>
{
   using super = TypeContextDescriptorBuilderBase;
   ...
   SmallVector<PILDeclRef, 8> VTableEntries;
   SmallVector<std::pair<PILDeclRef, PILDeclRef>, 8> OverrideTableEntries;

public:
   ClassContextDescriptorBuilder(IRGenModule &IGM, ClassDecl *Type,
                                 RequireMetadata_t requireMetadata)
      : super(IGM, Type, requireMetadata),
        VTable(IGM.getPILModule().lookUpVTable(getType())),
   ...
   void emitMethodDescriptor(PILDeclRef fn) {
...
//         flags = flags.withIsDynamic(true);
      ...
      if (auto entry = VTable->getEntry(IGM.getPILModule(), fn)) {
         assert(entry->TheKind == PILVTable::Entry::Kind::Normal);
         auto *implFn = IGM.getAddrOfPILFunction(entry->Implementation,
                                                 NotForDefinition);
   ...
   void emitMethodOverrideDescriptor(PILDeclRef baseRef, PILDeclRef declRef) {
      ...
      if (auto entry = VTable->getEntry(IGM.getPILModule(), baseRef)) {
         assert(entry->TheKind == PILVTable::Entry::Kind::Override);
         auto *implFn = IGM.getAddrOfPILFunction(entry->Implementation,
                                                 NotForDefinition);
         descriptor.addRelativeAddress(implFn);
      } else {
...
public:
   ...
   void addUnderlyingTypeAndConformances() {
      ...
      for (auto proto : opaqueType->getConformsTo()) {
         auto conformance = InterfaceConformanceRef(proto);
...
void irgen::emitLazyTypeMetadata(IRGenModule &IGM, NominalTypeDecl *type) {
   ...
   } else if (auto pd = dyn_cast<InterfaceDecl>(type)) {
      IGM.emitInterfaceDecl(pd);
   } else {
...
static void emitInitializeFieldOffsetVector(IRGenFunction &IGF,
   ...
   for (auto prop : storedProperties) {
      auto propTy = T.getFieldType(prop, IGF.getPILModule(),
...
getAddrOfDestructorFunction(IRGenModule &IGM, ClassDecl *classDecl) {
   auto dtorRef = PILDeclRef(classDecl->getDestructor(),
                             PILDeclRef::Kind::Deallocator);
   PILFunction *dtorFunc = IGM.getPILModule().lookUpFunction(dtorRef);
   if (!dtorFunc) return llvm::None;
   return IGM.getAddrOfPILFunction(dtorFunc, NotForDefinition);
}
...
public:
   ClassMetadataBuilderBase(IRGenModule &IGM, ClassDecl *theClass,
      ...
      : super(IGM, theClass), B(builder),
        ...
        VTable(IGM.getPILModule().lookUpVTable(theClass)) {}
...
public:
   ...
   void addSuperclass() {
      ...
      if (!Target->hasSuperclass()) {
         ...
         B.add(IGM.getAddrOfObjCClass(
            IGM.getObjCRuntimeBaseForPolarphpRootClass(Target),
            NotForDefinition));
         return;
      }
   ...
   void addMethod(PILDeclRef fn) {
      ...
      auto entry = VTable->getEntry(IGM.getPILModule(), fn);

      // The class is fragile. Emit a direct reference to the vtable entry.
      if (entry) {
         B.add(IGM.getAddrOfPILFunction(entry->Implementation, NotForDefinition));
         return;
      }
...
public:
   ...
   void addFieldOffset(VarDecl *var) {
      PILType baseType = PILType::getPrimitiveObjectType(
...
/// Given a lowered PIL type, load a value witness table that represents its
/// layout.
llvm::Value *
IRGenFunction::emitValueWitnessTableRef(PILType type,
...
public:
   ...
   void addFieldOffset(VarDecl *var) {
      ...
      PILType structType = getLoweredType();
...
public:
   ...
   PartialPattern buildExtraDataPattern() {
      ...
      struct Scanner : StructMetadataScanner<Scanner> {
         ...
         Scanner(IRGenModule &IGM, StructDecl *target, PILType type,
                 ConstantArrayBuilder &B)
         ...
         void addFieldOffset(VarDecl *field) {
            ...
            assert(IGM.getTypeInfo(
                  Type.getFieldType(field, IGM.getPILModule(),
                                    TypeExpansionContext::minimal()))
...
SpecialInterface irgen::getSpecialInterfaceID(InterfaceDecl *P) {
   auto known = P->getKnownInterfaceKind();
   if (!known)
      return SpecialInterface::None;
   switch (*known) {
      case KnownInterfaceKind::Error:
         return SpecialInterface::Error;

         // The other known protocols aren't special at runtime.
      case KnownInterfaceKind::Sequence:
      case KnownInterfaceKind::IteratorInterface:
      ...
      case KnownInterfaceKind::Comparable:
      case KnownInterfaceKind::SIMDScalar:
      case KnownInterfaceKind::ObjectiveCBridgeable:
      case KnownInterfaceKind::DestructorSafeContainer:
      case KnownInterfaceKind::SwiftNewtypeWrapper:
      case KnownInterfaceKind::ExpressibleByArrayLiteral:
      case KnownInterfaceKind::ExpressibleByBooleanLiteral:
      case KnownInterfaceKind::ExpressibleByDictionaryLiteral:
      case KnownInterfaceKind::ExpressibleByExtendedGraphemeClusterLiteral:
      case KnownInterfaceKind::ExpressibleByFloatLiteral:
      case KnownInterfaceKind::ExpressibleByIntegerLiteral:
      case KnownInterfaceKind::ExpressibleByStringInterpolation:
      case KnownInterfaceKind::ExpressibleByStringLiteral:
      case KnownInterfaceKind::ExpressibleByNilLiteral:
      case KnownInterfaceKind::ExpressibleByUnicodeScalarLiteral:
      case KnownInterfaceKind::ExpressibleByColorLiteral:
      case KnownInterfaceKind::ExpressibleByImageLiteral:
      case KnownInterfaceKind::ExpressibleByFileReferenceLiteral:
      case KnownInterfaceKind::ExpressibleByBuiltinBooleanLiteral:
      case KnownInterfaceKind::ExpressibleByBuiltinExtendedGraphemeClusterLiteral:
      case KnownInterfaceKind::ExpressibleByBuiltinFloatLiteral:
      case KnownInterfaceKind::ExpressibleByBuiltinIntegerLiteral:
      case KnownInterfaceKind::ExpressibleByBuiltinStringLiteral:
      case KnownInterfaceKind::ExpressibleByBuiltinUnicodeScalarLiteral:
      case KnownInterfaceKind::OptionSet:
      ...
      case KnownInterfaceKind::StringInterpolationInterface:
      case KnownInterfaceKind::Differentiable:
         return SpecialInterface::None;
   }

   llvm_unreachable("Not a valid KnownInterfaceKind.");
}
...
void IRGenModule::emitInterfaceDecl(InterfaceDecl *protocol) {
...
//         return;
//
//      getObjCInterfaceGlobalVars(protocol);
//      return;
//   }

   PILDefaultWitnessTable *defaultWitnesses = nullptr;
   if (isResilient(protocol, ResilienceExpansion::Minimal))
      defaultWitnesses = getPILModule().lookUpDefaultWitnessTable(protocol);

   {
      InterfaceDescriptorBuilder builder(*this, protocol, defaultWitnesses);
      builder.emit();
   }

   // Note that we emitted this protocol.
   SwiftInterfaces.push_back(protocol);
}
   ...
   for (auto &requirement : requirements) {
      ...
      switch (auto kind = requirement.getKind()) {
         ...
         case RequirementKind::Conformance: {
            auto protocol = requirement.getSecondType()->castTo<InterfaceType>()
               ->getDecl();
            bool needsWitnessTable =
               lowering::TypeConverter::interfaceRequiresWitnessTable(protocol);
            auto flags = GenericRequirementFlags(GenericRequirementKind::Interface,
               /*key argument*/needsWitnessTable,
               /*extra argument*/false);
            auto descriptorRef =
               IGM.getConstantReferenceForInterfaceDescriptor(protocol);
            addGenericRequirement(IGM, B, metadata, sig, flags,
                                  requirement.getFirstType(),
                                  [&]{

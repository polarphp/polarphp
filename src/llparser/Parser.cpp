...
//#include "polarphp/llparser/SyntaxParseActions.h"
...
static void getStringPartTokens(const Token &Tok,
                                ...
                                const SourceManager &SM, int BufID,
...
void tokenize(const LangOptions &LangOpts, const SourceManager &SM,
   ...
   Lexer L(LangOpts, SM, BufferID, Diags, LexerMode::Polarphp,
...
static void getStringPartTokens(const Token &Tok, const LangOptions &LangOpts,
   ...
   for (unsigned i = 0, e = Segments.size(); i != e; ++i) {
      ...
      if (Seg.Kind == Lexer::StringSegment::Literal) {
         ...
         std::vector<Token> NewTokens = tokenize(LangOpts, SM, BufID,
                                                 Offset, EndOffset,
            /*Diags=*/nullptr,
            /*KeepComments=*/true);
...
static LexerMode sourceFileKindToLexerMode(SourceFileKind kind) {
   switch (kind) {
      case SourceFileKind::Interface:
         return LexerMode::PolarphpInterface;
      case SourceFileKind::PIL:
         return LexerMode::PIL;
      case SourceFileKind::Library:
      case SourceFileKind::Main:
      case SourceFileKind::REPL:
         return LexerMode::Polarphp;
   }
   llvm_unreachable("covered switch");
}

Parser::Parser(unsigned BufferID, SourceFile &SF, PILParserTUStateBase *PIL,
               PersistentParserState *PersistentState,
   /* std::shared_ptr<SyntaxParseActions> SPActions, */
               bool DelayBodyParsing)
   : Parser(BufferID, SF, &SF.getAstContext().Diags, PIL, PersistentState,
...
Parser::Parser(unsigned BufferID, SourceFile &SF, DiagnosticEngine *LexerDiags,
   ...
   SF, PIL, PersistentState/*, std::move(SPActions)*/, DelayBodyParsing) {}
...
class TokenRecorder : public ConsumeTokenReceiver {
   ...
   void relexComment(CharSourceRange CommentRange,
      ...
      Lexer L(Ctx.LangOpts, Ctx.SourceMgr, BufferID, nullptr, LexerMode::Polarphp,
              HashbangMode::Disallowed,
...
//   Impl.SF->SyntaxParsingCache = SyntaxCache;
   Impl.TheParser.reset(new Parser(BufferID, *Impl.SF, /*PIL=*/nullptr,
...
ParserUnit::ParserUnit(SourceManager &SM, SourceFileKind SFKind, unsigned BufferID,
   ...
   Lex.reset(new Lexer(Impl.LangOpts, SM,
                       ...
                       TriviaRetentionMode::WithoutTrivia,
                       Offset, EndOffset));
   Impl.TheParser.reset(new Parser(std::move(Lex), *Impl.SF, /*PIL=*/nullptr,
...
ParsedDeclName parse_decl_name(StringRef name) {
...
DeclName ParsedDeclName::formDeclName(AstContext &ctx) const {
   return form_decl_name(ctx, BaseName, ArgumentLabels, IsFunctionName,
      /*IsInitializer=*/true);
}
...
DeclName parse_decl_name(AstContext &ctx, StringRef name) {
   return parse_decl_name(name).formDeclName(ctx);
}

...
//#include "polarphp/llparser/ParsedSyntaxBuilders.h"
...
//         ty = Result.getASTResult();
      ...
      case tok::kw_interface:
         if (startsWithLess(peekToken())) {
            ty = parseOldStyleInterfaceComposition();
            break;
         }
         LLVM_FALLTHROUGH;
      default: {
...
//            makeMetatypeTypeSyntax();
         ...
         if (peekToken().isContextualKeyword("Interface")) {
            consumeToken();
            SourceLoc protocolLoc = consumeToken(tok::identifier);
            ty = makeParserResult(ty,
                                  new(Context) InterfaceTypeRepr(ty.get(), protocolLoc));
            // @todo
//            makeMetatypeTypeSyntax();
            continue;
         }
...
ParserResult<TypeRepr> Parser::parsePILBoxType(GenericParamList *generics,
                                               const TypeAttributes &attrs,
                                               Optional<Scope> &GenericsScope) {
   auto LBraceLoc = consumeToken(tok::l_brace);

   SmallVector<PILBoxTypeRepr::Field, 4> Fields;
   if (!Tok.is(tok::r_brace)) {
      for (;;) {
         ...
         if (Tok.is(tok::kw_var)) {
            ...
            diagnose(Tok, diag::pil_box_expected_var_or_let);
   ...
   if (!Tok.is(tok::r_brace)) {
      diagnose(Tok, diag::pil_box_expected_r_brace);
      return makeParserError();
   }
   ...
   if (Tok.isContextualPunctuator("<")) {
      ...
      if (!Tok.isContextualPunctuator(">")) {
         diagnose(Tok, diag::pil_box_expected_r_angle);
         return makeParserError();
      }
   ...
   auto repr = PILBoxTypeRepr::create(Context, generics,
...
//                                           SyntaxContextKind::Type);
   ...
   // Parse generic parameters in PIL mode.
   ...
   // In PIL mode, parse box types { ... }.
   if (isInPILMode() && Tok.is(tok::l_brace)) {
      return parsePILBoxType(generics, attrs, GenericsScope);
   }
...
//                                       : SyntaxKind::MemberTypeIdentifier);
      ...
      if ((Tok.is(tok::period) || Tok.is(tok::period_prefix))) {
         ...
         if (!peekToken().isContextualKeyword("Type")
             && !peekToken().isContextualKeyword("Interface")) {
            consumeToken();
            continue;
         }
      } else if (Tok.is(tok::code_complete)) {
...
/// parseOldStyleInterfaceComposition
///   type-composition-deprecated:
///     'protocol' '<' '>'
...
ParserResult<TypeRepr> Parser::parseOldStyleInterfaceComposition() {
   assert(Tok.is(tok::kw_interface) && startsWithLess(peekToken()));
...
//                                           SyntaxContextKind::Type);
   ...
   SourceLoc InterfaceLoc = consumeToken();
   SourceLoc LAngleLoc = consumeStartingLess();

   // Parse the type-composition-list.
   ParserStatus Status;
   SmallVector<TypeRepr *, 4> Interfaces;
   bool IsEmpty = startsWithGreater(Tok);
   if (!IsEmpty) {
      do {
         // Parse the type-identifier.
         ParserResult<TypeRepr> Interface = parseTypeIdentifier();
         Status |= Interface;
         if (auto *ident =
            dyn_cast_or_null<IdentTypeRepr>(Interface.getPtrOrNull()))
            Interfaces.push_back(ident);
   ...
   auto composition = CompositionTypeRepr::create(
      Context, Interfaces, InterfaceLoc, {LAngleLoc, RAngleLoc});
   ...
   if (Status.isSuccess()) {
      ...
      diagnose(InterfaceLoc,
               IsEmpty ? diag::deprecated_any_composition :
               Interfaces.size() > 1 ? diag::deprecated_protocol_composition :
...
bool Parser::isImplicitlyUnwrappedOptionalToken(const Token &T) const {
   // A postfix '!' by itself, or a '!' in PIL mode, is obviously implicitly
   // unwrapped optional.
   if (T.is(tok::exclaim_postfix) || T.is(tok::pil_exclamation))
      return true;
   // A postfix or bound infix operator token that begins with '!' can be
   ...
   switch (Tok.getKind()) {
      ...
      case tok::kw_Any:
         if (!canParseTypeIdentifier())
            return false;
         break;
      case tok::kw_interface: // Deprecated composition syntax
      case tok::identifier:
...
bool Parser::canParseTypeIdentifierOrTypeComposition() {
   if (Tok.is(tok::kw_interface))
      return canParseOldStyleInterfaceComposition();
...
bool Parser::canParseOldStyleInterfaceComposition() {
   consumeToken(tok::kw_interface);

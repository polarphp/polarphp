...
#include "polarphp/ide/internal/CodeCompletionResultBuilder.h"
#include "polarphp/ide/internal/ExprContextAnalysis.h"
...
#include "polarphp/ide/CodeCompletionCache.h"
...
std::string polar::ide::removeCodeCompletionTokens(
...
CodeCompletionResult::getCodeCompletionDeclKind(const Decl *D) {
   switch (D->getKind()) {
      ...
      case DeclKind::Interface:
         return CodeCompletionDeclKind::Interface;
      case DeclKind::Var:
      case DeclKind::Param: {
      ...
      case ResultKind::Declaration:
         ...
         switch (getAssociatedDeclKind()) {
            ...
            case CodeCompletionDeclKind::EnumElement:
               ...
               break;
            case CodeCompletionDeclKind::Interface:
               Prefix.append("[Interface]");
               break;
            ...
            if (Sink.LastModule.first == CurrentModule.getOpaqueValue()) {
               ...
               if (auto *C = CurrentModule.dyn_cast<const clang::Module *>()) {
                  ...
                  ModuleName = copyString(
                     *Sink.Allocator,
                     CurrentModule.get<const polar::ModuleDecl *>()->getName().str());
               }
...
static KnownInterfaceKind
protocolForLiteralKind(CodeCompletionLiteralKind kind) {
   switch (kind) {
      case CodeCompletionLiteralKind::ArrayLiteral:
         return KnownInterfaceKind::ExpressibleByArrayLiteral;
      case CodeCompletionLiteralKind::BooleanLiteral:
         return KnownInterfaceKind::ExpressibleByBooleanLiteral;
      case CodeCompletionLiteralKind::ColorLiteral:
         return KnownInterfaceKind::ExpressibleByColorLiteral;
      case CodeCompletionLiteralKind::ImageLiteral:
         return KnownInterfaceKind::ExpressibleByImageLiteral;
      case CodeCompletionLiteralKind::DictionaryLiteral:
         return KnownInterfaceKind::ExpressibleByDictionaryLiteral;
      case CodeCompletionLiteralKind::IntegerLiteral:
         return KnownInterfaceKind::ExpressibleByIntegerLiteral;
      case CodeCompletionLiteralKind::NilLiteral:
         return KnownInterfaceKind::ExpressibleByNilLiteral;
      case CodeCompletionLiteralKind::StringLiteral:
         return KnownInterfaceKind::ExpressibleByUnicodeScalarLiteral;
      case CodeCompletionLiteralKind::Tuple:
...
class CompletionLookup final : public polar::VisibleDeclConsumer {
...
public:
   CompletionLookup(CodeCompletionResultSink &Sink,
      ...
      (void)polar::createTypeChecker(Ctx);
   ...
   void addImportModuleNames() {
      ...
      for (auto ModuleName : ModuleNames) {
         if (ModuleName.str().startswith("_") ||
             ...
             ModuleName.str() == POLAR_ONONE_SUPPORT)
            continue;
   ...
   SemanticContextKind getSemanticContext(const Decl *D,
      ...
      switch (Reason) {
         ...
         case DeclVisibilityKind::MemberOfInterfaceImplementedByCurrentNominal:
   ...
   Type eraseArchetypes(Type type, GenericSignature genericSig) {
      ...
      auto buildInterfaceComposition = [&](ArrayRef<InterfaceDecl *> protos) -> Type {
         SmallVector<Type, 2> types;
         for (auto proto : protos)
            types.push_back(proto->getDeclaredInterfaceType());
         return InterfaceCompositionType::get(Ctx, types,
      ...
      return type.transform([&](Type t) -> Type {
         ...
         if (auto *archetypeType = t->getAs<ArchetypeType>()) {
            ...
            if (!protos.empty())
               return buildInterfaceComposition(protos);
         }
   ...
   // Implement polar::VisibleDeclConsumer.
      ...
      switch (Kind) {
         ...
         case LookupKind::ImportFromModule:
            ...
            if (auto *GP = dyn_cast<GenericTypeParamDecl>(D)) {
               ...
               for (auto *protocol : GP->getConformingInterfaces())
      ...
      if (isa<TypeExpr>(ParsedExpr)) {
         if (auto *T = ExprType->getAs<AnyMetatypeType>()) {
            ...
            if (instanceTy->isAnyExistentialType()) {
               addKeyword("Interface", MetatypeType::get(instanceTy),
                          SemanticContextKind::CurrentNominal);
   ...
   void addTypeRelationFromInterface(CodeCompletionResultBuilder &builder,
                                     CodeCompletionLiteralKind kind,
                                     StringRef defaultTypeName) {
      // Check for matching ExpectedTypes.
      auto *P = Ctx.getInterface(protocolForLiteralKind(kind));
   ...
   void addPoundLiteralCompletions(bool needPound) {
      auto addFromProto = [&](StringRef name, CodeCompletionKeywordKind kwKind,
         ...
         builder.addTextChunk(name);
         addTypeRelationFromInterface(builder, literalKind, defaultTypeName);
      };
   ...
   void addValueLiteralCompletions() {
      ...
      auto addFromProto = [&](
         ...
         consumer(builder);
         addTypeRelationFromInterface(builder, kind, defaultTypeName);
      };
   ...
   struct FilteredDeclConsumer : public polar::VisibleDeclConsumer {
      polar::VisibleDeclConsumer &Consumer;
      DeclFilter Filter;
      FilteredDeclConsumer(polar::VisibleDeclConsumer &Consumer,
   ...
   void getTypeCompletions(Type BaseType) {
      ...
      if (BaseType->isAnyExistentialType()) {
         addKeyword("Interface", MetatypeType::get(BaseType));
...
class CompletionOverrideLookup : public polar::VisibleDeclConsumer {
   CodeCompletionResultSink &Sink;
   AstContext &Ctx;
   const DeclContext *CurrDeclContext;
...
public:
   ...
   bool missingOverride(DeclVisibilityKind Reason) {
      return !hasOverride && Reason == DeclVisibilityKind::MemberOfSuper &&
             !CurrDeclContext->getSelfInterfaceDecl();
   }
   ...
   void addAccessControl(const ValueDecl *VD,
      ...
      if (Access < AccessLevel::Public &&
          !isa<InterfaceDecl>(VD->getDeclContext())) {
            ...
            Conformance->getRootConformance()->forEachValueWitness(
               [&](ValueDecl *req, Witness witness) {
                  if (witness.getDecl() == VD)
                     Access = std::max(
                        Access, Conformance->getInterface()->getFormalAccess());
               });
         }
      }
   ...
   TypeLoc getOpaqueResultTypeLoc(const ValueDecl *VD, DeclVisibilityKind Reason,
                                  DynamicLookupInfo dynamicLookupInfo) {
      if (Reason !=
          DeclVisibilityKind::MemberOfInterfaceImplementedByCurrentNominal)
         return nullptr;
   ...
   void foundDecl(ValueDecl *D, DeclVisibilityKind Reason,
      ...
      if (auto *CD = dyn_cast<ConstructorDecl>(D)) {
         if (!isa<InterfaceDecl>(CD->getDeclContext()))
            return;
   ...
   void getOverrideCompletions(SourceLoc Loc) {
      if (!CurrDeclContext->isTypeContext())
         return;
      if (isa<InterfaceDecl>(CurrDeclContext))
         return;
...
//                                  : CompletionKind::KeyPathExprSwift;
   Kind = CompletionKind::KeyPathExprPHP;
   ParsedExpr = KPE;
   this->DotLoc = DotLoc;
   CurDeclContext = P.CurDeclContext;
}
...
//      case CompletionKind::KeyPathExprObjC:
      case CompletionKind::KeyPathExprPHP:
      ...
      case CompletionKind::AccessorBeginning: {
         ...
         if (var && !var->getDeclContext()->getSelfInterfaceDecl())
   ...
   for (auto &Request: Lookup.RequestedCachedResults) {
      ...
      auto handleImport = [&](ModuleDecl::ImportedModule Import) {
         ...
         if (!ModuleFilename.empty()) {
            ...
            using PairType = llvm::DenseSet<polar::ide::CodeCompletionCache::Key,
...
polar::ide::makeCodeCompletionCallbacksFactory(
   CodeCompletionContext &CompletionContext,
   CodeCompletionConsumer &Consumer) {
   return new CodeCompletionCallbacksFactoryImpl(CompletionContext, Consumer);
}

void polar::ide::lookupCodeCompletionResultsFromModule(
...
void polar::ide::copyCodeCompletionResults(CodeCompletionResultSink &targetSink,

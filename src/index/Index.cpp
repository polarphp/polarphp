...
#include "polarphp/index/Index.h"
...
#include "polarphp/ide/SourceEntityWalker.h"
...
using namespace polar::index;

static bool
printArtificialName(const polar::AbstractStorageDecl *ASD, AccessorKind AK, llvm::raw_ostream &OS) {
   switch (AK) {
...
static bool printDisplayName(const polar::ValueDecl *D, llvm::raw_ostream &OS) {
...
static bool isMemberwiseInit(polar::ValueDecl *D) {
...
class IndexPHPAstWalker : public SourceEntityWalker {
...
void IndexPHPAstWalker::visitDeclContext(DeclContext *Context) {
   IsModuleFile = false;
...
void IndexPHPAstWalker::visitModule(ModuleDecl &Mod) {
...
bool IndexPHPAstWalker::handleSourceOrModuleFile(SourceFileOrModule SFOrMod) {
...
bool IndexPHPAstWalker::visitImports(
...
bool IndexPHPAstWalker::handleWitnesses(Decl *D, SmallVectorImpl<IndexedWitness> &explicitWitnesses) {
   ...
   for (auto *conf : DC->getLocalConformances()) {
      ...
      auto normal = dyn_cast<NormalInterfaceConformance>(conf->getRootConformance());
      if (!normal)
         continue;
...
bool IndexPHPAstWalker::startEntity(Decl *D, IndexSymbol &Info, bool IsRef) {
   switch (IdxConsumer.startSourceEntity(Info)) {
      case polar::index::IndexDataConsumer::Abort:
         Cancelled = true;
         LLVM_FALLTHROUGH;
      case polar::index::IndexDataConsumer::Skip:
         return false;
      case polar::index::IndexDataConsumer::Continue: {
         SmallVector<IndexedWitness, 6> explicitWitnesses;
         if (!IsRef) {
...
bool IndexPHPAstWalker::startEntityDecl(ValueDecl *D) {
   ...
   for (auto Overriden: collectAllOverriddenDecls(D, /*IncludeInterfaceReqs=*/false)) {
...
bool IndexPHPAstWalker::reportRelatedRef(ValueDecl *D, SourceLoc Loc, bool isImplicit,
...
bool IndexPHPAstWalker::reportInheritedTypeRefs(ArrayRef<TypeLoc> Inherited, Decl *Inheritee) {
...
bool IndexPHPAstWalker::reportRelatedTypeRef(const TypeLoc &Ty, SymbolRoleSet Relations, Decl *Related) {
...
static bool isDynamicVarAccessorOrFunc(ValueDecl *D, SymbolInfo symInfo) {
   if (auto NTD = D->getDeclContext()->getSelfNominalTypeDecl()) {
      bool isClassOrInterface = isa<ClassDecl>(NTD) || isa<InterfaceDecl>(NTD);
...
bool IndexPHPAstWalker::reportPseudoAccessor(AbstractStorageDecl *D,
...
NominalTypeDecl *
IndexPHPAstWalker::getTypeLocAsNominalTypeDecl(const TypeLoc &Ty) {
...
bool IndexPHPAstWalker::reportExtension(ExtensionDecl *D) {
...
bool IndexPHPAstWalker::report(ValueDecl *D) {
...
bool IndexPHPAstWalker::reportRef(ValueDecl *D, SourceLoc Loc,
                                    IndexSymbol &Info,
                                    Optional<AccessKind> AccKind) {
   if (!shouldIndex(D, /*IsRef=*/true))
...
bool IndexPHPAstWalker::reportImplicitConformance(ValueDecl *witness, ValueDecl *requirement,
                                                    Decl *container) {
...
bool IndexPHPAstWalker::initIndexSymbol(ValueDecl *D, SourceLoc Loc,
...
bool IndexPHPAstWalker::initIndexSymbol(ExtensionDecl *ExtD, ValueDecl *ExtendedD,
...
bool IndexPHPAstWalker::initFuncDeclIndexSymbol(FuncDecl *D,
                                                  IndexSymbol &Info) {
...
bool IndexPHPAstWalker::initFuncRefIndexSymbol(ValueDecl *D, SourceLoc Loc,
                                                 IndexSymbol &Info) {
...
bool IndexPHPAstWalker::initVarRefIndexSymbols(Expr *CurrentE, ValueDecl *D,
                                                 SourceLoc Loc, IndexSymbol &Info,
                                                 Optional<AccessKind> AccKind) {
   ...
   switch (Kind) {
      case polar::AccessKind::Read:
         Info.roles |= (unsigned)SymbolRole::Read;
         break;
      case polar::AccessKind::ReadWrite:
         Info.roles |= (unsigned)SymbolRole::Read;
         LLVM_FALLTHROUGH;
      case polar::AccessKind::Write:
...
bool IndexPHPAstWalker::indexComment(const Decl *D) {
   ...
   warn([&Imports](llvm::raw_ostream &OS) {
      std::for_each(Imports.begin(), Imports.end(), [&OS](ModuleDecl *M) {
         if (M->getModuleFilename().empty()) {
            std::string Info = "polar::ModuleDecl with empty file name!! \nDetails: \n";
            Info += "  name: ";
            Info += M->getName().get();
            Info += "\n";
            ...
            OS << "polar::ModuleDecl with empty file name! " << Info << "\n";
         }
      });
   });
...
void IndexPHPAstWalker::collectRecursiveModuleImports(
...
void index::indexDeclContext(DeclContext *DC, IndexDataConsumer &consumer) {
   ...
   IndexPHPAstWalker walker(consumer, DC->getAstContext(), bufferId);
...
void index::indexSourceFile(SourceFile *SF, IndexDataConsumer &consumer) {
   ...
   IndexPHPAstWalker walker(consumer, SF->getAstContext(), bufferID);
...
void index::indexModule(ModuleDecl *module, IndexDataConsumer &consumer) {
   assert(module);
   IndexPHPAstWalker walker(consumer, module->getAstContext());

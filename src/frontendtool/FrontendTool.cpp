...
#include "polarphp/frontendtool/FrontendTool.h"
#include "polarphp/frontendtool/internal/ImportedModules.h"
#include "polarphp/frontendtool/internal/ReferenceDependencies.h"
#include "polarphp/frontendtool/internal/TBD.h"
...
#include "polarphp/frontend/DiagnosticVerifier.h"
#include "polarphp/frontend/Frontend.h"
#include "polarphp/frontend/PrintingDiagnosticConsumer.h"
#include "polarphp/frontend/SerializedDiagnosticConsumer.h"
...
StringRef
polar::frontend::utils::escapeForMake(StringRef raw,
...
static bool emitMakeDependenciesIfNeeded(DiagnosticEngine &diags,
   ...
   opts.forAllOutputPaths(input, [&](const StringRef targetName) {
      out << polar::frontend::utils::escapeForMake(targetName, buffer) << " :";
      // First include all other files in the module. Make-style dependencies
      // need to be conservative!
      for (auto const &path :
         reversePathSortedFilenames(opts.InputsAndOutputs.getInputFilenames()))
         out << ' ' << polar::frontend::utils::escapeForMake(path, buffer);
...
struct LoadedModuleTraceFormat {
   ...
   Identifier Name;
   std::string Arch;
   std::vector<PHPModuleTraceInfo> SwiftModules;
};
}

namespace polar {
namespace json {
template <> struct ObjectTraits<PHPModuleTraceInfo> {
   static void mapping(Output &out, PHPModuleTraceInfo &contents) {
...
// Version notes:
// 1. Keys: name, arch, phpmodules
// 2. New keys: version, phpmodulesDetailedInfo
...
static void computePHPModuleTraceInfo(
   ...
   auto computeAdjacentInterfacePath = [](SmallVectorImpl<char> &modPath) {
      auto swiftInterfaceExt =
         filetypes::get_extension(filetypes::TY_PHPModuleInterfaceFile);
   ...
   std::sort(
      ...
      [](const PHPModuleTraceInfo &m1, const PHPModuleTraceInfo &m2) -> bool {
...
static bool emitLoadedModuleTraceIfNeeded(ModuleDecl *mainModule,
   ...
   std::vector<PHPModuleTraceInfo> swiftModules;
   computePHPModuleTraceInfo(importedModules, pathToModuleDecl, *depTracker,
...
printModuleInterfaceIfNeeded(StringRef outputPath,
   ...
   if (!LangOpts.isPolarphpVersionAtLeast(5)) {
      assert(LangOpts.isPolarphpVersionAtLeast(4));
      diags.diagnose(SourceLoc(),
                     diag::warn_unsupported_module_interface_php_version,
                     LangOpts.isPolarphpVersionAtLeast(4, 2) ? "4.2" : "4");
   }
   ...
   return withOutputFile(diags, outputPath,
                         [M, Opts](raw_ostream &out) -> bool {
                            return polar::emitPHPInterface(out, Opts, M);
                         });
}
...
//      }
//
//      polar::writeEditsInJson(llvm::makeArrayRef(AllEdits), *OS);
//      return false;
//   }
//};
//
...
static void countStatsPostPILGen(UnifiedStatsReporter &Stats,
                                 const PILModule& Module) {
   auto &C = Stats.getFrontendCounters();
   // FIXME: calculate these in constant time, via the dense maps.
   C.NumPILGenFunctions += Module.getFunctionList().size();
   C.NumPILGenVtables += Module.getVTableList().size();
   C.NumPILGenWitnessTables += Module.getWitnessTableList().size();
   C.NumPILGenDefaultWitnessTables += Module.getDefaultWitnessTableList().size();
   C.NumPILGenGlobalVariables += Module.getPILGlobalList().size();
}
...
emitSwiftRangesForAllPrimaryInputsIfNeeded(CompilerInvocation &Invocation,
   ...
   if (Invocation.getFrontendOptions().InputsAndOutputs.hasPHPRangesPath() &&
       Instance.getPrimarySourceFiles().empty()) {
      Instance.getAstContext().Diags.diagnose(
         SourceLoc(), diag::emit_php_ranges_without_primary_file);
      return;
   }
   for (auto *SF : Instance.getPrimarySourceFiles()) {
      const std::string &phpRangesFilePath =
         Invocation.getPHPRangesFilePathForPrimary(SF->getFilename());
      if (!phpRangesFilePath.empty()) {
         (void)Instance.emitPHPRanges(Instance.getAstContext().Diags, SF,
                                      phpRangesFilePath);
      }
   }
}
...
static bool dumpAPI(ModuleDecl *Mod, StringRef OutDir) {
   ...
   auto dumpFile = [&](SourceFile *SF) -> bool {
      ...
      PO.SkipUnderscoredStdlibInterfaces = true;
      SF->print(TempOS, PO);
...
computeStatsReporter(const CompilerInvocation &Invocation, CompilerInstance *Instance) {
   ...
   auto &PILOpts = Invocation.getPILOptions();
   std::string InputName =
      FEOpts.InputsAndOutputs.getStatsFileMangledInputName();
   StringRef OptType = silOptModeArgStr(PILOpts.OptMode);
...
int polar::performFrontend(ArrayRef<const char *> Args,
   ...
   IRGenOptions &IRGenOpts = Invocation.getIRGenOptions();
   IRGenOpts.DWARFVersion = polar::DWARFVersion;
   ...
   if (Invocation.getFrontendOptions().PrintHelp ||
      ...
      unsigned ExcludedFlagsBitmask =
         ...
         llvm::opt::HelpHidden;
      std::unique_ptr<llvm::opt::OptTable> Options(create_polarphp_opt_table());
...
void FrontendObserver::performedPILGeneration(PILModule &module) {}
void FrontendObserver::performedPILProcessing(PILModule &module) {}

...
bool polar::writeTBD(ModuleDecl *M, StringRef OutputFilename,
...
bool polar::inputFileKindCanHaveTBDValidated(InputFileKind kind) {
...
static bool validateSymbolSet(DiagnosticEngine &diags,
   ...
   for (auto &name : irNotTBD) {
      diags.diagnose(SourceLoc(), diag::symbol_in_ir_not_in_tbd, name,
                     demangling::demangleSymbolAsString(name));
      error = true;
   }

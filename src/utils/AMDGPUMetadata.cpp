...
std::error_code toString(const Metadata &palMetadata, std::string &str)
{
   RawStringOutStream stream(str);
   for (auto iter = palMetadata.begin(), end = palMetadata.end(); iter != end; ++iter) {
      stream << Twine(iter == palMetadata.begin() ? " 0x" : ",0x");
      stream << Twine::utohexstr(*iter);
   }
   stream.flush();
   return std::error_code();
}

...
bool is_printable(int ucs)
{
   ...
   static const UnicodeCharRange s_nonPrintableRanges[] = {
   ...
   static const UnicodeCharSet nonPrintables(s_nonPrintableRanges);
...
static inline int char_width(int ucs)
{
   if (!is_printable(ucs)) {
   ...
   static const UnicodeCharRange s_combiningCharacterRanges[] = {
   ...
   static const UnicodeCharSet combiningCharacters(s_combiningCharacterRanges);

   if (combiningCharacters.contains(ucs)) {
      return 0;
   }
   static const UnicodeCharRange s_doubleWidthCharacterRanges[] = {
      // Hangul Jamo
      { 0x1100, 0x11FF },
   ...
   static const UnicodeCharSet doubleWidthCharacters(s_doubleWidthCharacterRanges);
...
int column_width_utf8(StringRef text)
{
   ...
   unsigned length;
   for (size_t i = 0, e = text.getSize(); i < e; i += length) {
      length = polar::utils::get_num_bytes_for_utf8(text[i]);
      if (length <= 0 || i + length > text.getSize()) {
         return ErrorInvalidUTF8;
      }
      Utf32 buf[1];
      const Utf8 *start = reinterpret_cast<const Utf8 *>(text.getData() + i);
      Utf32 *target = &buf[0];
      if (ConversionResult::ConversionOK != polar::utils::convert_utf8_to_utf32(&start, start + length, &target,
                                                                                target + 1, ConversionFlags::StrictConversion)) {
         return ErrorInvalidUTF8;
      }
      int width = char_width(buf[0]);
      if (width < 0) {
         return ErrorNonPrintableCharacter;
      }
      columnWidth += width;

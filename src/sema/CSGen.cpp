...
using namespace polar::constraints;
...
bool computeFavoredTypeForExpr(Expr *expr, ConstraintSystem &CS) {
   ...
   if (lti.haveFloatLiteral) {
      if (auto floatProto = CS.getAstContext().getInterface(
   ...
   if (lti.haveIntLiteral) {
      if (auto intProto = CS.getAstContext().getInterface(
   ...
   if (lti.haveStringLiteral) {
      if (auto stringProto = CS.getAstContext().getInterface(
...
bool isFavoredParamAndArg(ConstraintSystem &CS, Type paramTy, Type argTy,
   ...
   llvm::SmallSetVector<InterfaceDecl *, 2> literalProtos;
   if (auto argTypeVar = argTy->getAs<TypeVariableType>()) {
      ...
      for (auto constraint : constraints) {
         literalProtos.insert(constraint->getInterface());
      }
   }
...
void favorMatchingOverloadExprs(ApplyExpr *expr,
   ...
   if (auto favoredTy = CS.getFavoredType(expr->getArg())) {
      ...
      auto mustConsider = [&](ValueDecl *value) -> bool {
         return isa<InterfaceDecl>(value->getDeclContext());
      };
...
public:
   ...
   Type visitLiteralExpr(LiteralExpr *expr) {
      ...
      auto protocol = TypeChecker::getLiteralInterface(CS.getAstContext(), expr);
      if (!protocol)
         return nullptr;
   ...
   Type visitArrayExpr(ArrayExpr *expr) {
      ...
      InterfaceDecl *arrayProto = TypeChecker::getInterface(
         ...
         KnownInterfaceKind::ExpressibleByArrayLiteral);
      if (!arrayProto) {
         return Type();
      }
   ...
   Type visitDictionaryExpr(DictionaryExpr *expr) {
      ...
      InterfaceDecl *dictionaryProto = TypeChecker::getInterface(
         C, expr->getLoc(), KnownInterfaceKind::ExpressibleByDictionaryLiteral);
      if (!dictionaryProto) {
         return Type();
      }
   ...
   std::pair<bool, Expr *> walkToExprPre(Expr *expr) override {
      while (true) {
         ...
         if (auto OOE = dyn_cast<OpenExistentialExpr>(expr)) {
            ...
            (void)inserted;
            POLAR_DEFER { OpenExistentials.erase(archetypeVal); };
...
bool polar::areGenericRequirementsSatisfied(
...
void polar::eraseOpenedExistentials(ConstraintSystem &CS, Expr *&expr) {
...
polar::resolveValueMember(DeclContext &DC, Type BaseTy, DeclName Name) {
...
polar::getOriginalArgumentList(Expr *expr) {

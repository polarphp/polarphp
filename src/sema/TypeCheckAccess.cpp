...
void AccessControlCheckerBase::checkTypeAccessImpl(
   ...
   if (contextAccessScope.hasEqualDeclContextWith(problematicAccessScope) ||
      ...
      if (typeRepr && mayBeInferred &&
          !Context.LangOpts.isPolarphpVersionAtLeast(5) &&
...
#define UNINTERESTING(KIND) \
  void visit##KIND##Decl(KIND##Decl *D) {}
         ...
         /*mayBeInferred*/false,
                      [&](AccessScope typeAccessScope,
                         ...
                         bool isExplicit = theVar->getAttrs().hasAttribute<AccessControlAttr>() ||
                                           isa<InterfaceDecl>(theVar->getDeclContext());
                         auto theVarAccess =
      ...
      checkTypeAccess(TP->getTypeLoc(), anyVar, /*mayBeInferred*/true,
                      [&](AccessScope typeAccessScope,
                         ...
                         bool isExplicit = anyVar->getAttrs().hasAttribute<AccessControlAttr>() ||
                                           isa<InterfaceDecl>(anyVar->getDeclContext());
      ...
      checkTypeAccess(TAD->getUnderlyingType(),
                      ...
                      [&](AccessScope typeAccessScope,
                         ...
                         bool isExplicit =
                            ...
                            isa<InterfaceDecl>(TAD->getDeclContext());
      ...
      checkRequirementAccess(assocType,
                             ...
                             [&](AccessScope typeAccessScope,
                                ...
                                if (typeAccessScope.isChildOf(minAccessScope) ||
                                   ...
                                   if (!assocType->getAstContext().isPolarphpVersionAtLeast(5))
                                      downgradeToWarning = DowngradeToWarning::Yes;
                                }
                             });
      ...
      if (const NominalTypeDecl *superclassDecl = CD->getSuperclassDecl()) {
         ...
         auto superclassLocIter = std::find_if(CD->getInherited().begin(),
                                               ...
                                               [&](TypeLoc inherited) {
                                                  ...
                                                  if (ty->is<InterfaceCompositionType>())
         ...
         if (superclassDecl->isGenericContext() &&
             !CD->getAstContext().isPolarphpVersionAtLeast(5)) {
   ...
   void visitInterfaceDecl(InterfaceDecl *proto) {
      ...
      const TypeRepr *complainRepr = nullptr;
      auto downgradeToWarning = DowngradeToWarning::No;
      DescriptiveDeclKind declKind = DescriptiveDeclKind::Interface;
         ...
         checkTypeAccess(
            ...
            [&](AccessScope typeAccessScope, const TypeRepr *thisComplainRepr,
               ...
               if (typeAccessScope.isChildOf(minAccessScope) ||
                  ...
                  // Swift versions before 5.0 did not check requirements on the
                  // protocol's where clause, so emit a warning.
                  if (!proto->getAstContext().isPolarphpVersionAtLeast(5))
                     downgradeToWarning = DowngradeToWarning::Yes;
               }
            });
      });
   ...
   void visitSubscriptDecl(SubscriptDecl *SD) {
      ...
      if (!minAccessScope.isPublic()) {
         ...
         bool isExplicit =
            ...
            isa<InterfaceDecl>(SD->getDeclContext());
   ...
   void visitAbstractFunctionDecl(AbstractFunctionDecl *fn) {
      ...
      if (!minAccessScope.isPublic()) {
         ...
         bool isExplicit =
            ...
            isa<InterfaceDecl>(fn->getDeclContext());
...
public:
   ...
   void visit(Decl *D) {
      if (!D->getAstContext().isPolarphpVersionAtLeast(4, 2))
         return;
   ...
   void visitTypeAliasDecl(TypeAliasDecl *TAD) {
      ...
      checkTypeAccess(TAD->getUnderlyingType(),
                      ...
                      [&](AccessScope typeAccessScope,
                         ...
                         if (!TAD->getAstContext().isPolarphpVersionAtLeast(5))
         ...
         checkTypeAccess(rawType, rawTypeLocIter->getTypeRepr(), ED,
            /*mayBeInferred*/false,
                         [&](AccessScope typeAccessScope,
                            ...
                            if (!ED->getAstContext().isPolarphpVersionAtLeast(5))
         ...
         checkTypeAccess(CD->getSuperclass(), superclassLocIter->getTypeRepr(), CD,
            /*mayBeInferred*/false,
                         [&](AccessScope typeAccessScope,
                            ...
                            if (!CD->getAstContext().isPolarphpVersionAtLeast(5))
      ...
      for (auto &P : *SD->getIndices()) {
         checkTypeAccess(
            ...
            [&](AccessScope typeAccessScope, const TypeRepr *complainRepr,
               ...
               if (!SD->getAstContext().isPolarphpVersionAtLeast(5))
      ...
      for (auto *P : *fn->getParameters()) {
         checkTypeAccess(
            ...
            [&](AccessScope typeAccessScope, const TypeRepr *complainRepr,
               ...
               if (!fn->getAstContext().isPolarphpVersionAtLeast(5))
      ...
      for (auto &P : *EED->getParameterList()) {
         checkTypeAccess(
            ...
            [&](AccessScope typeAccessScope, const TypeRepr *complainRepr,
               ...
               if (!EED->getAstContext().isPolarphpVersionAtLeast(5))
...
class ExportabilityChecker : public DeclVisitor<ExportabilityChecker> {
   ...
   llvm::function_ref<void(const InterfaceConformance *)>;
   ...
   void checkTypeImpl(
      ...
      public:
         ...
         void visitSubstitutionMap(SubstitutionMap subs) {
            for (InterfaceConformanceRef conformance : subs.getConformances()) {
               if (!conformance.isConcrete())
                  continue;
               const InterfaceConformance *concreteConf = conformance.getConcrete();

               SubstitutionMap subConformanceSubs =
                  concreteConf->getSubstitutions(SF.getParentModule());
               visitSubstitutionMap(subConformanceSubs);

               const RootInterfaceConformance *rootConf =
   ...
   public:
      ...
      void operator()(const InterfaceConformance *offendingConformance) {
         ...
         D->diagnose(diag::conformance_from_implementation_only_module,
                     ...
                     offendingConformance->getInterface()->getFullName(),
                     static_cast<unsigned>(reason), M->getName());
      }
   };
...
void polar::checkAccessControl(Decl *D) {

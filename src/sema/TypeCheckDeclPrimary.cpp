...
#include "polarphp/sema/internal/TypeCheckAccess.h"
...
static void checkInheritanceClause(
   ...
   if ((ext = declUnion.dyn_cast<ExtensionDecl *>())) {
      ...
      // Interface extensions cannot have inheritance clauses.
   ...
   bool canHaveSuperclass = (isa<ClassDecl>(decl) ||
                             (isa<InterfaceDecl>(decl) /*&&
                              !cast<InterfaceDecl>(decl)->isObjC() */));
      ...
      if (inheritedTy->isAnyObject()) {
         if (inheritedAnyObject) {
            ...
            if (!ctx.LangOpts.isPolarphpVersionAtLeast(5) &&
                (isa<InterfaceDecl>(decl) || isa<AbstractTypeParamDecl>(decl)) &&
...
static void checkForEmptyOptionSet(const VarDecl *VD) {
   ...
   auto *optionSetProto = VD->getAstContext().getInterface(KnownInterfaceKind::OptionSet);
   bool conformsToOptionSet = (bool)TypeChecker::containsInterface(
...
static void checkRedeclaration(AstContext &ctx, ValueDecl *current) {
   ...
   for (auto other : otherDefinitions) {
      ...
      if (isRedeclaration || wouldBeSwift5Redeclaration) {
         ...
         // warning) will result in PILGen crashes due to both properties mangling
...
static void checkInterfaceSelfRequirements(InterfaceDecl *proto,
...
#define CHECK_LITERAL_PROTOCOL(Kind, String)                                   \
  if (auto proto = TypeChecker::getInterface(                                   \
          type->getAstContext(), SourceLoc(), KnownInterfaceKind::Kind)) {      \
    if (TypeChecker::conformsToInterface(type, proto, dc,                       \
...
static void diagnoseClassWithoutInitializers(ClassDecl *classDecl) {
   ...
   if (auto *superclassDecl = classDecl->getSuperclassDecl()) {
      auto *decodableProto = C.getInterface(KnownInterfaceKind::Decodable);
      ...
      if (ref) {
         ...
         auto result =
            TypeChecker::lookupMember(superclassDecl, superclassType, initFrom,
                                      NameLookupFlags::InterfaceMembers |
         ...
         auto *encodableProto = C.getInterface(KnownInterfaceKind::Encodable);
         auto ref = TypeChecker::conformsToInterface(
...
static void maybeDiagnoseClassWithoutInitializers(ClassDecl *classDecl) {
   if (auto *SF = classDecl->getParentSourceFile()) {
      // Allow classes without initializers in PIL and module interface files.
      switch (SF->Kind) {
...
public:
   ...
   void visit(Decl *decl) {
      ...
      TypeChecker::checkUnsupportedInterfaceType(decl);
...
//         (void) VD->isObjC();
         ...
         // "Type" or "Interface" since we reserve the X.Type and X.Interface
         ...
         if (VD->getDeclContext()->isTypeContext() &&
             (VD->getFullName().isSimpleName(Context.Id_Type) ||
              VD->getFullName().isSimpleName(Context.Id_Interface)) &&
      ...
      if (VD->hasStorage()) {
         ...
         if (VD->isStatic()) {
            ...
            if (DC->getExtendedInterfaceDecl()) {
               unimplementedStatic(InterfaceExtensions);
...
//      }
   ...
   void visitPatternBindingDecl(PatternBindingDecl *PBD) {
      ...
      bool isInPILMode = false;
      if (auto sourceFile = DC->getParentSourceFile())
         isInPILMode = sourceFile->Kind == SourceFileKind::PIL;
      ...
      for (auto i : range(PBD->getNumPatternEntries())) {
         ...
         PBD->getPattern(i)->forEachVariable([&](VarDecl *var) {
            ...
            // protocol.
            if (var->isStatic() && !isa<InterfaceDecl>(DC)) {
               // ...but don't enforce this for PIL or module interface files.
   ...
   void visitSubscriptDecl(SubscriptDecl *SD) {
      ...
      if (!SD->isInvalid()) {
         ...
         TypeChecker::checkInterfaceSelfRequirements(SD);
      }
   ...
   void visitAssociatedTypeDecl(AssociatedTypeDecl *AT) {
      ...
      auto *proto = AT->getInterface();

      checkInterfaceSelfRequirements(proto, AT);
      // @todo
//      if (proto->isObjC()) {
...
//      }
   ...
   void checkUnsupportedNestedType(NominalTypeDecl *NTD) {
      ...
      if (NTD->isGenericContext()) {
         auto DC = NTD->getDeclContext();
         if (auto proto = DC->getSelfInterfaceDecl()) {
            if (DC->getExtendedInterfaceDecl()) {
...
//                     (void) vd->isObjC();
   ...
   void visitInterfaceDecl(InterfaceDecl *PD) {
      ...
      if (PD->getAstContext().TypeCheckerOpts.DebugGenericSignatures) {
         auto requirementsSig =
            GenericSignature::get({PD->getInterfaceSelfType()},
                                  PD->getRequirementSignature());

         llvm::errs() << "Interface requirement signature:\n";
   ...
   static bool requiresDefinition(Decl *decl) {
      ...
      // Interface requirements do not require definitions.
      if (isa<InterfaceDecl>(decl->getDeclContext()))
         return false;

      // Functions can have _pilgen_name, semantics, and NSManaged attributes.
      if (auto func = dyn_cast<AbstractFunctionDecl>(decl)) {
         if (func->getAttrs().hasAttribute<PILGenNameAttr>() ||
             func->getAttrs().hasAttribute<SemanticsAttr>() ||
             func->getAttrs().hasAttribute<NSManagedAttr>())
            return false;
      }

      // Declarations in PIL and module interface files don't require
      // definitions.
   ...
   void visitFuncDecl(FuncDecl *FD) {
      ...
      if (!FD->isInvalid()) {
         ...
         TypeChecker::checkInterfaceSelfRequirements(FD);
      }
      ...
      if (FD->getDeclContext()->isTypeContext()) {
         if (FD->isOperator() && !isMemberOperator(FD, nullptr)) {
            ...
            auto isInterface = selfNominal && isa<InterfaceDecl>(selfNominal);
            // We did not find 'Self'. Complain.
            FD->diagnose(diag::operator_in_unrelated_type,
                         FD->getDeclContext()->getDeclaredInterfaceType(), isInterface,
                         FD->getFullName());
         }
      }
      // @todo
      ...
      // productize @_cdecl.
//      if (auto CDeclAttr = FD->getAttrs().getAttribute<polar::CDeclAttr>()) {
...
//      }
   ...
   void visitTopLevelCodeDecl(TopLevelCodeDecl *TLCD) {
      // See polarphp::performTypeChecking for TopLevelCodeDecl handling.
   ...
   void visitConstructorDecl(ConstructorDecl *CD) {
      ...
      if (!CD->isInvalid()) {
         ...
         TypeChecker::checkInterfaceSelfRequirements(CD);
      }

//===--- TypeCheckInterface.cpp - Interface Checking ------------------------===//
//
...
#include "polarphp/sema/internal/TypeAccessScopeChecker.h"
#include "polarphp/sema/internal/TypeCheckAvailability.h"
...
#include "polarphp/ast/TypeMatcher.h"
...
polar::Witness RequirementMatch::getWitness(AstContext &ctx) const {
   auto syntheticEnv = ReqEnv->getSyntheticEnvironment();
   return polar::Witness(this->Witness, WitnessSubstitutions,
                         syntheticEnv, ReqEnv->getRequirementToSyntheticMap());
}

AssociatedTypeDecl *
polar::getReferencedAssocTypeOfInterface(Type type, InterfaceDecl *proto) {
   if (auto dependentMember = type->getAs<DependentMemberType>()) {
      if (auto assocType = dependentMember->getAssocType()) {
         if (dependentMember->getBase()->isEqual(proto->getSelfInterfaceType())) {
            ...
            if (assocType->getInterface() == proto)
               return assocType;
...
static const RequirementEnvironment &getOrCreateRequirementEnvironment(
   ...
   DeclContext *dc, GenericSignature reqSig, InterfaceDecl *proto,
   ClassDecl *covariantSelf, InterfaceConformance *conformance) {
...
static Optional<RequirementMatch> findMissingGenericRequirementForSolutionFix(
   ...
   switch (fix->getKind()) {
      case FixKind::AddConformance: {
         ...
         missingType = missingConform->getInterfaceType();
         break;
      }
...
RequirementMatch
polar::matchWitness(WitnessChecker::RequirementEnvironmentCache &reqEnvCache,
                    InterfaceDecl *proto, InterfaceConformance *conformance,
                    DeclContext *dc, ValueDecl *req, ValueDecl *witness) {
   using namespace constraints;
   ...
   if (witness->getDeclContext()->getExtendedInterfaceDecl()) {
      if (auto *classDecl = dc->getSelfClassDecl()) {
         if (!classDecl->isFinal()) {
            ...
            auto selfKind = proto->findInterfaceSelfReferences(req,
   ...
   auto setup = [&]() -> std::tuple<Optional<RequirementMatch>, Type, Type> {
      ...
      reqLocator = cs->getConstraintLocator(
         static_cast<Expr *>(nullptr), LocatorPathElt::InterfaceRequirement(req));
...
witnessHasImplementsAttrForExactRequirement(ValueDecl *witness,
                                            ValueDecl *requirement) {
   assert(requirement->isInterfaceRequirement());
   auto *PD = cast<InterfaceDecl>(requirement->getDeclContext());
   if (auto A = witness->getAttrs().getAttribute<ImplementsAttr>()) {
      if (Type T = A->getInterfaceType().getType()) {
         if (auto ProtoTy = T->getAs<InterfaceType>()) {
...
WitnessChecker::WitnessChecker(AstContext &ctx, InterfaceDecl *proto,
...
WitnessChecker::lookupValueWitnesses(ValueDecl *req, bool *ignoringNames) {
   ...
   assert(req->isInterfaceRequirement());
   ...
   if (req->isOperator()) {
      ...
      for (auto candidate : lookup) {
         ...
         if (polar::isMemberOperator(cast<FuncDecl>(decl), Adoptee)) {
...
bool WitnessChecker::findBestWitness(
   ...
   for (Attempt attempt = Regular; numViable == 0 && attempt != Done;
      ...
      for (auto witness : witnesses) {
         ...
         // (unrelated) protocol.
         if (isa<InterfaceDecl>(witness->getDeclContext())) {
            continue;
         }
         ...
         if (auto *ext = dyn_cast<ExtensionDecl>(match.Witness->getDeclContext())){
            if (!ext->isConstrainedExtension() && ext->getExtendedInterfaceDecl())
...
class polar::MultiConformanceChecker {
   AstContext &Context;
   llvm::SmallVector<ValueDecl*, 16> UnsatisfiedReqs;
   llvm::SmallVector<ConformanceChecker, 4> AllUsedCheckers;
   llvm::SmallVector<NormalInterfaceConformance*, 4> AllConformances;
   llvm::SetVector<ValueDecl*> MissingWitnesses;
   llvm::SmallPtrSet<ValueDecl *, 8> CoveredMembers;

   /// Check one conformance.
   InterfaceConformance * checkIndividualConformance(
      NormalInterfaceConformance *conformance, bool issueFixit);

   /// Determine whether the given requirement was left unsatisfied.
   bool isUnsatisfiedReq(NormalInterfaceConformance *conformance, ValueDecl *req);
public:
   ...
   void addConformance(NormalInterfaceConformance *conformance) {
...
isUnsatisfiedReq(NormalInterfaceConformance *conformance, ValueDecl *req) {
   ...
   if (witness->getDeclContext()->getSelfInterfaceDecl())
      return true;
...
void MultiConformanceChecker::checkAllConformances() {
   ...
   for (unsigned I = 0, N = AllConformances.size(); I < N; I ++) {
      ...
      for (auto member : proto->getMembers()) {
         ...
         if (!req || !req->isInterfaceRequirement()) continue;
...
static void diagnoseConformanceImpliedByConditionalConformance(
   DiagnosticEngine &Diags, NormalInterfaceConformance *conformance,
   NormalInterfaceConformance *implyingConf, bool issueFixit) {
   Type T = conformance->getType();
   auto proto = conformance->getInterface();
   Type protoType = proto->getDeclaredType();
   auto implyingProto = implyingConf->getInterface()->getDeclaredType();
...
InterfaceConformance *MultiConformanceChecker::
checkIndividualConformance(NormalInterfaceConformance *conformance,
                           bool issueFixit) {
   ...
   switch (conformance->getState()) {
      case InterfaceConformanceState::Incomplete:
      ...
      case InterfaceConformanceState::CheckingTypeWitnesses:
      case InterfaceConformanceState::Checking:
      case InterfaceConformanceState::Complete:
         // Nothing to do.
         return conformance;
   }
   ...
   conformance->setState(InterfaceConformanceState::Checking);
   POLAR_DEFER { conformance->setState(InterfaceConformanceState::Complete); };
...
//            }
   ...
   for (auto InheritedProto : Proto->getInheritedInterfaces()) {
...
static Type getRequirementTypeForDisplay(ModuleDecl *module,
   ...
   auto substType = [&](Type type, bool isResult) -> Type {
      ...
      auto selfTy = conformance->getInterface()->getSelfInterfaceType();
...
static void
diagnoseMatch(ModuleDecl *module, NormalInterfaceConformance *conformance,
   ...
   for (auto assocType : conformance->getInterface()->getAssociatedTypeMembers()) {
...
ConformanceChecker::ConformanceChecker(
   AstContext &ctx, NormalInterfaceConformance *conformance,
   llvm::SetVector<ValueDecl *> &GlobalMissingWitnesses,
   bool suppressDiagnostics)
   : WitnessChecker(ctx, conformance->getInterface(), conformance->getType(),
...
ConformanceChecker::getReferencedAssociatedTypes(ValueDecl *req) {
   ...
   public:
      Walker(InterfaceDecl *Proto,
             llvm::SmallVectorImpl<AssociatedTypeDecl *> &assocTypes)
         : Proto(Proto), assocTypes(assocTypes) {}

      Action walkToTypePre(Type type) override {
         if (type->is<DependentMemberType>()) {
            if (auto assocType = getReferencedAssocTypeOfInterface(type, Proto)) {
...
static SourceLoc
getLocForDiagnosingWitness(const NormalInterfaceConformance *conformance,
...
public:
   ...
   void operator()(const NormalInterfaceConformance *conformance) {
      auto proto = conformance->getInterface();
...
//   checkObjCTypeErasedGenerics(assocType, type, typeDecl);
   ...
   if (typeDecl) {
      ...
      if (auto nominal = DC->getSelfNominalTypeDecl()) {
         ...
         if (!getAstContext().isPolarphpVersionAtLeast(5) &&
...
printRequirementStub(ValueDecl *Requirement, DeclContext *Adopter,
   ...
   AccessLevel Access =
      std::min(
         ...
         Requirement->getDeclContext()->getSelfInterfaceDecl()->
            getFormalAccess());
   ...
   if (auto MissingTypeWitness = dyn_cast<AssociatedTypeDecl>(Requirement)) {
      ...
      Options.PrintAccessorBodiesInInterfaces = true;
...
/// NoStubRequirements.
static void
printInterfaceStubFixitString(SourceLoc TypeLoc, InterfaceConformance *Conf,
...
diagnoseMissingWitnesses(MissingWitnessDiagnosisKind Kind) {
   ...
   auto InsertFixitCallback = [ComplainLoc, EditorMode, MissingWitnesses]
      (NormalInterfaceConformance *Conf) {
      ...
      printInterfaceStubFixitString(TypeLoc, Conf,
...
void ConformanceChecker::checkNonFinalClassWitness(ValueDecl *requirement,
   ...
   if (auto ctor = dyn_cast<ConstructorDecl>(witness)) {
      if (!ctor->isRequired() &&
          !ctor->getDeclContext()->getSelfInterfaceDecl() &&
          !ctor->hasClangNode()) {
         // FIXME: We're not recovering (in the Ast), so the Fix-It
         // should move.
         diagnoseOrDefer(requirement, false,
                         [ctor, requirement](NormalInterfaceConformance *conformance) {
   ...
   auto selfKind = Proto->findInterfaceSelfReferences(requirement,
      /*allowCovariantParameters=*/false,
      /*skipAssocTypes=*/true);
   ...
   if (selfKind.other) {
      ...
      // class.
      diagnoseOrDefer(requirement, false,
                      [witness, requirement](NormalInterfaceConformance *conformance) {
                         auto proto = conformance->getInterface();
   ...
   // associated types.
   if (auto func = dyn_cast<FuncDecl>(witness)) {
      if (func->getDeclContext()->getExtendedInterfaceDecl()) {
         auto selfKindWithAssocTypes = Proto->findInterfaceSelfReferences(
            requirement,
            /*allowCovariantParameters=*/false,
            /*skipAssocTypes=*/false);
...
ConformanceChecker::resolveWitnessViaLookup(ValueDecl *requirement) {
   ...
   if (auto derivable = DerivedConformance::getDerivableRequirement(
      ...
      if (derivable == requirement) {
         ...
         auto derivableProto = cast<InterfaceDecl>(derivable->getDeclContext());
         auto conformance =
            TypeChecker::conformsToInterface(Adoptee, derivableProto, DC, None);
...
//                                  fixDeclarationName(diag, witness, requirement->getFullName());
      ...
      switch (check.Kind) {
            ...
            diagnoseOrDefer(requirement, false,
                            [witness, requirement, check](
                               ...
                               diags.diagnose(
                                  ...
                                  conformance->getInterface()->getFullName(),
                                  witness->getFullName(),
         ...
         case CheckKind::OptionalityConflict: {
            ...
            diagnoseOrDefer(requirement, false,
                            [witness, adjustments, requirement](NormalInterfaceConformance *conformance) {
                               auto proto = conformance->getInterface();
   ...
   if (!numViable) {
      ...
      diagnoseOrDefer(requirement, true,
                      [requirement, matches, nominal](NormalInterfaceConformance *conformance) {
...
ResolveWitnessResult ConformanceChecker::resolveWitnessViaDerivation(
   ...
   auto derived =
      TypeChecker::deriveInterfaceRequirement(DC, derivingTypeDecl, requirement);
   if (!derived)
      return ResolveWitnessResult::ExplicitFailed;
...
CheckTypeWitnessResult polar::checkTypeWitness(DeclContext *dc,
                                               InterfaceDecl *proto,
                                               AssociatedTypeDecl *assocType,
                                               Type type) {
   ...
   for (auto reqProto : genericSig->getConformsTo(depTy)) {
      ...
      // FIXME: Why is conformsToInterface() not enough? The stdlib doesn't
...
ResolveWitnessResult ConformanceChecker::resolveTypeWitnessViaLookup(
   ...
   auto candidates = TypeChecker::lookupMemberType(
      DC, Adoptee, assocType->getName(), NameLookupFlags::InterfaceMembers);
   ...
   if (!viable.empty()) {
      diagnoseOrDefer(assocType, true,
                      [assocType, viable](NormalInterfaceConformance *conformance) {
   ...
   diagnoseOrDefer(assocType, true,
                   [nonViable](NormalInterfaceConformance *conformance) {
...
void ConformanceChecker::ensureRequirementsAreSatisfied() {
   ...
   auto substitutions = SubstitutionMap::getInterfaceSubstitutions(
      proto, substitutingType, InterfaceConformanceRef(Conformance));
   ...
   class GatherConformancesListener : public GenericRequirementsCheckListener {
      NormalInterfaceConformance *conformanceBeingChecked;
      SourceFile *SF;
      ...
      void checkExportability(Type depTy, Type replacementTy,
         ...
         Type selfTy = rootConformance->getInterface()->getInterfaceSelfType();
         if (depTy->isEqual(selfTy)) {
            ctx.Diags.diagnose(
               conformanceBeingChecked->getLoc(),
               diag::conformance_from_implementation_only_module,
               rootConformance->getType(),
               rootConformance->getInterface()->getName(), 0, M->getName());
         } else {
            ctx.Diags.diagnose(
               conformanceBeingChecked->getLoc(),
               diag::assoc_conformance_from_implementation_only_module,
               rootConformance->getType(),
               rootConformance->getInterface()->getName(), M->getName(),
   ...
   public:
      ...
      void satisfiedConformance(Type depTy, Type replacementTy,
                                InterfaceConformanceRef conformance) override {
         if (conformance.isConcrete())
...
#pragma mark Interface conformance checking
...
void ConformanceChecker::resolveValueWitnesses() {
   for (auto member : Proto->getMembers()) {
      ...
      if (!requirement->isInterfaceRequirement())
         continue;
...
//                     Conformance->getInterface()->getDeclaredInterfaceType());
//                  witness
...
void ConformanceChecker::checkConformance(MissingWitnessDiagnosisKind Kind) {
   ...
   POLAR_DEFER { diagnoseMissingWitnesses(Kind); };
   ...
   // @todo
//   if (Proto->isSpecificInterface(KnownInterfaceKind::ObjectiveCBridgeable)) {
//      auto nominal = Adoptee->getAnyNominal();
...
static void diagnoseConformanceFailure(Type T,
   ...
   if (T->isExistentialType() &&
       TypeChecker::containsInterface(T, Proto, DC, None)) {
   ...
   if (Proto->isSpecificInterface(KnownInterfaceKind::ExpressibleByNilLiteral)) {
   ...
   if (auto enumDecl = T->getEnumOrBoundGenericEnum()) {
      if (Proto->isSpecificInterface(KnownInterfaceKind::RawRepresentable) &&
          DerivedConformance::derivesInterfaceConformance(DC, enumDecl,
                                                          Proto) &&
          enumDecl->hasRawType() &&
         ...
         // Equatable, say so.
         auto equatableProto = ctx.getInterface(KnownInterfaceKind::Equatable);
         if (!equatableProto)
            return;

         if (TypeChecker::conformsToInterface(rawType, equatableProto, enumDecl,
                                              None)
            .isInvalid()) {
...
void ConformanceChecker::diagnoseOrDefer(
   ...
   std::function<void(NormalInterfaceConformance *)> fn) {
   if (isError)
      Conformance->setInvalid();
...
TypeChecker::containsInterface(Type T, InterfaceDecl *Proto, DeclContext *DC,
   ...
   if (T->isExistentialType()) {
      ...
      if (auto superclass = layout.getSuperclass()) {
         auto result =
            TypeChecker::conformsToInterface(superclass, Proto, DC, options);
         if (result) {
            return result;
         }
      }
      ...
      for (auto P : layout.getInterfaces()) {
         auto *PD = P->getDecl();
   ...
   return TypeChecker::conformsToInterface(T, Proto, DC, options);
}

InterfaceConformanceRef
TypeChecker::conformsToInterface(Type T, InterfaceDecl *Proto, DeclContext *DC,
                                 ConformanceCheckOptions options,
                                 SourceLoc ComplainLoc) {
   bool InExpression = options.contains(ConformanceCheckFlags::InExpression);

   auto recordDependency = [=](InterfaceConformance *conformance = nullptr) {
...
//
// Invoking TypeChecker::conformsToInterface from the ModuleDecl bypasses
...
InterfaceConformanceRef
ModuleDecl::conformsToInterface(Type sourceTy, InterfaceDecl *targetInterface) {

   auto flags = ConformanceCheckFlags::SuppressDependencyTracking;

   return TypeChecker::conformsToInterface(sourceTy, targetInterface, this, flags);
}

InterfaceConformanceRef TypeChecker::LookUpConformance::
operator()(CanType dependentType, Type conformingReplacementType,
           InterfaceDecl *conformedInterface) const {
   if (conformingReplacementType->isTypeParameter())
      return InterfaceConformanceRef(conformedInterface);

   return TypeChecker::conformsToInterface(
...
void TypeChecker::checkConformance(NormalInterfaceConformance *conformance) {
   MultiConformanceChecker checker(conformance->getInterface()->getAstContext());
...
scorePotentiallyMatching(ValueDecl *req, ValueDecl *witness, unsigned limit) {
   ...
   // both names.
//   if (cast<InterfaceDecl>(req->getDeclContext())->isObjC()) {
...
static void diagnosePotentialWitness(NormalInterfaceConformance *conformance,
                                     ValueDecl *req, ValueDecl *witness,
                                     AccessLevel access) {
   auto proto = cast<InterfaceDecl>(req->getDeclContext());
   ...
   auto dc = conformance->getDeclContext();
   auto match = matchWitness(oneUseCache, conformance->getInterface(),
...
//static bool isNSCoding(InterfaceDecl *protocol) {
...
static void diagnoseUnstableName(InterfaceConformance *conformance,
                                 ClassDecl *classDecl) {
...
//       isa<NormalInterfaceConformance>(conformance) /* &&
//       !hasExplicitObjCName(classDecl) */) {
//      C.Diags.diagnose(cast<NormalInterfaceConformance>(conformance)->getLoc(),
//                       diag::nscoding_unstable_mangled_name,
...
void TypeChecker::checkConformancesInContext(DeclContext *dc,
   ...
   for (auto conformance : conformances) {
      ...
      if (tracker)
         tracker->addUsedMember({conformance->getInterface(), Identifier()},
...
//             isNSCoding(conformance->getInterface()) &&
...
//      }
      ...
      if (conformance->getInterface()->
         isSpecificInterface(KnownInterfaceKind::StringInterpolationInterface)) {
   ...
   for (const auto &diag : diagnostics) {
      ...
      if (existingModule != dc->getParentModule() &&
          (existingModule->getName() ==
           ...
           existingModule == diag.Interface->getParentModule())) {
         ...
         for (auto decl : idc->getMembers()) {
            ...
            for (auto requirement
               : diag.Interface->lookupDirect(value->getFullName())) {
               if (requirement->getDeclContext() != diag.Interface)
                  continue;
      ...
      if (auto enumDecl = dyn_cast<EnumDecl>(existingDecl)) {
         if (diag.Interface->isSpecificInterface(
      ...
      existingDecl->diagnose(diag::declared_protocol_conformance_here,
                             ...
                             diag.Interface->getName(),
                             diag.ExistingExplicitInterface->getName());
   }
      ...
      if (SF && SF->Kind != SourceFileKind::Interface) {
         ...
         for (auto member : idc->getMembers()) {
            ...
            if (!bestOptionalReqs.empty()) {
               ...
               for (auto conformance : conformances) {
                  if (conformance->getInterface() == req->getDeclContext()) {
...
polar::findWitnessedObjCRequirements(const ValueDecl *witness,
   ...
   // @todo
//   for (auto proto : nominal->getAllInterfaces()) {
//      // We only care about Objective-C protocols.
//      if (!proto->isObjC()) continue;
//
//      Optional<InterfaceConformance *> conformance;
...
//            SmallVector<InterfaceConformance *, 2> conformances;
...
//                !isa<InheritedInterfaceConformance>(*conformance)) {
//               continue;
//            }
//
   ...
   if (result.size() > 2) {
      std::stable_sort(result.begin(), result.end(),
                       [&](ValueDecl *lhs, ValueDecl *rhs) {
                          InterfaceDecl *lhsProto
                             = cast<InterfaceDecl>(lhs->getDeclContext());
                          InterfaceDecl *rhsProto
                             = cast<InterfaceDecl>(rhs->getDeclContext());
...
ValueDecl *TypeChecker::deriveInterfaceRequirement(DeclContext *DC,
                                                   NominalTypeDecl *TypeDecl,
                                                   ValueDecl *Requirement) {
   // Note: whenever you update this function, also update
   // DerivedConformance::getDerivableRequirement.
   auto *protocol = cast<InterfaceDecl>(Requirement->getDeclContext());

   auto knownKind = protocol->getKnownInterfaceKind();
   ...
   switch (*knownKind) {
      ...
      case KnownInterfaceKind::Encodable:
         return derived.deriveEncodable(Requirement);

      case KnownInterfaceKind::Decodable:
...
Type TypeChecker::deriveTypeWitness(DeclContext *DC,
                                    NominalTypeDecl *TypeDecl,
                                    AssociatedTypeDecl *AssocType) {
   auto *protocol = cast<InterfaceDecl>(AssocType->getDeclContext());
...
public:
   DefaultWitnessChecker(InterfaceDecl *proto)
...
void TypeChecker::inferDefaultWitnesses(InterfaceDecl *proto) {
   ...
   for (auto *requirement : proto->getMembers()) {
      ...
      if (!valueDecl->isInterfaceRequirement())
         continue;
   ...
   for (const auto &req : proto->getRequirementSignature()) {
      ...
      if (assocType->getInterface() != proto) {
         ...
         proto->getModuleContext()->lookupQualified(
            ...
            NL_QualifiedDefault|NL_InterfaceMembers|NL_OnlyTypes,
            found);
      ...
      auto conformance = conformsToInterface(defaultAssocTypeInContext,

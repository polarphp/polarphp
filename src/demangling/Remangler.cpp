...
#include "polarphp/demangling/internal/RemanglerBase.h"
...
class Remangler : public RemanglerBase {
   ...
   void manglePureInterface(Node *Proto) {
   ...
   void mangleInterfaceList(Node *protocols, Node *superclass,
   ...
   void mangleAnyInterfaceConformance(Node *node);
...
bool Remangler::mangleStandardSubstitution(Node *node) {
   if (node->getKind() != Node::Kind::Structure
       ...
       && node->getKind() != Node::Kind::Interface)
      return false;
...
void Remangler::mangleGenericArgs(Node *node, char &Separator,
   ...
   switch (node->getKind()) {
      ...
      case Node::Kind::PropertyWrapperBackingInitializer:
         ...
         mangleGenericArgs(node->getChild(0), Separator,
                           fullSubstitutionMap);
         if (demangling::nodeConsumesGenericArgs(node)) {
            Buffer << Separator;
            Separator = '_';
         }
         break;
      ...
      case Node::Kind::BoundGenericInterface:
...
void Remangler::mangleAssociatedConformanceDescriptor(Node *node) {
   ...
   manglePureInterface(node->getChild(2));
   Buffer << "Tn";
}
...
void Remangler::mangleBaseConformanceDescriptor(Node *node) {
   mangle(node->getChild(0));
   manglePureInterface(node->getChild(1));
   Buffer << "Tb";
}
...
void Remangler::mangleBoundGenericInterface(Node *node) {
   mangleAnyNominalType(node);
}
...
void Remangler::mangleBuiltinTypeName(Node *node) {
   ...
   if (text == BUILTIN_TYPE_NAME_BRIDGEOBJECT) {
      ...
      Buffer << 'p';
   } else if (text == BUILTIN_TYPE_NAME_PILTOKEN) {
      Buffer << 't';
...
void Remangler::mangleDependentGenericConformanceRequirement(Node *node) {
   ...
   if (ProtoOrClass->getFirstChild()->getKind() == Node::Kind::Interface) {
      manglePureInterface(ProtoOrClass);
...
void Remangler::mangleGenericInterfaceWitnessTable(Node *node) {
   mangleSingleChildNode(node);
   Buffer << "WG";
}

void Remangler::mangleGenericInterfaceWitnessTableInstantiationFunction(Node *node) {
   mangleSingleChildNode(node);
   Buffer << "WI";
}

void Remangler::mangleResilientInterfaceWitnessTable(Node *node) {
   mangleSingleChildNode(node);
   Buffer << "Wr";
}
...
void Remangler::mangleLazyInterfaceWitnessTableAccessor(Node *node) {
   mangleChildNodes(node);
   Buffer << "Wl";
}

void Remangler::mangleLazyInterfaceWitnessTableCacheVariable(Node *node) {
   mangleChildNodes(node);
   Buffer << "WL";
}
...
void Remangler::mangleInterface(Node *node) {
   mangleAnyGenericType(node, "P");
}

void Remangler::mangleRetroactiveConformance(Node *node) {
   mangleAnyInterfaceConformance(node->getChild(1));
   Buffer << 'g';
   mangleIndex(node->getChild(0)->getIndex());
}

void Remangler::mangleInterfaceConformance(Node *node) {
...
void Remangler::mangleInterfaceConformanceRefInTypeModule(Node *node) {
   manglePureInterface(node->getChild(0));
   Buffer << "HP";
}

void Remangler::mangleInterfaceConformanceRefInInterfaceModule(Node *node) {
   manglePureInterface(node->getChild(0));
   Buffer << "Hp";
}

void Remangler::mangleInterfaceConformanceRefInOtherModule(Node *node) {
   manglePureInterface(node->getChild(0));
   mangleChildNode(node, 1);
}

void Remangler::mangleConcreteInterfaceConformance(Node *node) {
   mangleType(node->getChild(0));
   mangle(node->getChild(1));
   if (node->getNumChildren() > 2)
      mangleAnyInterfaceConformanceList(node->getChild(2));
   else
      Buffer << "y";
   Buffer << "HC";
}

void Remangler::mangleDependentInterfaceConformanceRoot(Node *node) {
   mangleType(node->getChild(0));
   manglePureInterface(node->getChild(1));
   Buffer << "HD";
...
void Remangler::mangleDependentInterfaceConformanceInherited(Node *node) {
   mangleAnyInterfaceConformance(node->getChild(0));
   manglePureInterface(node->getChild(1));
   Buffer << "HI";
   mangleDependentConformanceIndex(node->getChild(2));
}
...
void Remangler::mangleDependentInterfaceConformanceAssociated(Node *node) {
   mangleAnyInterfaceConformance(node->getChild(0));
...
void Remangler::mangleAnyInterfaceConformance(Node *node) {
   switch (node->getKind()) {
      case Node::Kind::ConcreteInterfaceConformance:
         return mangleConcreteInterfaceConformance(node);
      case Node::Kind::DependentInterfaceConformanceRoot:
         return mangleDependentInterfaceConformanceRoot(node);
      case Node::Kind::DependentInterfaceConformanceInherited:
         return mangleDependentInterfaceConformanceInherited(node);
      case Node::Kind::DependentInterfaceConformanceAssociated:
         return mangleDependentInterfaceConformanceAssociated(node);
      default:
         break;
   }
}

void Remangler::mangleAnyInterfaceConformanceList(Node *node) {
   bool firstElem = true;
   for (NodePointer child : *node) {
      mangleAnyInterfaceConformance(child);
      mangleListSeparator(firstElem);
   }
   mangleEndOfList(firstElem);
}

void Remangler::mangleInterfaceDescriptor(Node *node) {
   manglePureInterface(getSingleChild(node));
   Buffer << "Mp";
}

void Remangler::mangleInterfaceRequirementsBaseDescriptor(Node *node) {
   manglePureInterface(getSingleChild(node));
   Buffer << "TL";
}

void Remangler::mangleInterfaceSelfConformanceDescriptor(Node *node) {
   manglePureInterface(node->getChild(0));
   Buffer << "MS";
}

void Remangler::mangleInterfaceConformanceDescriptor(Node *node) {
   mangleInterfaceConformance(node->getChild(0));
   Buffer << "Mc";
}

void Remangler::mangleInterfaceList(Node *node, Node *superclass,
                                   bool hasExplicitAnyObject) {
   auto *protocols = getSingleChild(node, Node::Kind::TypeList);
   bool FirstElem = true;
   for (NodePointer Child : *protocols) {
      manglePureInterface(Child);
      mangleListSeparator(FirstElem);
   }
   mangleEndOfList(FirstElem);
   if (superclass) {
...
void Remangler::mangleInterfaceList(Node *node) {
   mangleInterfaceList(node, nullptr, false);
}

void Remangler::mangleInterfaceListWithClass(Node *node) {
   mangleInterfaceList(node->getChild(0),
                      node->getChild(1),
                      false);
}

void Remangler::mangleInterfaceListWithAnyObject(Node *node) {
   mangleInterfaceList(node->getChild(0), nullptr, true);
}

void Remangler::mangleInterfaceSelfConformanceWitness(Node *node) {
   mangleSingleChildNode(node);
   Buffer << "TS";
}

void Remangler::mangleInterfaceWitness(Node *node) {
   mangleChildNodes(node);
   Buffer << "TW";
}

void Remangler::mangleInterfaceSelfConformanceWitnessTable(Node *node) {
   manglePureInterface(node->getChild(0));
   Buffer << "WS";
}

void Remangler::mangleInterfaceWitnessTable(Node *node) {
   mangleSingleChildNode(node);
   Buffer << "WP";
}

void Remangler::mangleInterfaceWitnessTablePattern(Node *node) {
   mangleSingleChildNode(node);
   Buffer << "Wp";
}

void Remangler::mangleInterfaceWitnessTableAccessor(Node *node) {
   mangleSingleChildNode(node);
   Buffer << "Wa";
}
...
void Remangler::manglePILBoxType(Node *node) {
   mangleSingleChildNode(node);
   Buffer << "Xb";
}
...
void Remangler::mangleUncurriedFunctionType(Node *node) {
   mangleFunctionSignature(node);
   // mangling as regular function type (there is no "uncurried function type"
...
void Remangler::manglePILBoxTypeWithLayout(Node *node) {
   assert(node->getNumChildren() == 1 || node->getNumChildren() == 3);
   assert(node->getChild(0)->getKind() == Node::Kind::PILBoxLayout);
   ...
   for (unsigned i = 0, e = layout->getNumChildren(); i < e; ++i) {
      assert(layout->getChild(i)->getKind() == Node::Kind::PILBoxImmutableField
             || layout->getChild(i)->getKind() == Node::Kind::PILBoxMutableField);
      ...
      if (field->getKind() == Node::Kind::PILBoxMutableField) {
...
void Remangler::manglePILBoxLayout(Node *node) {
   unreachable("should be part of PILBoxTypeWithLayout");
}

void Remangler::manglePILBoxMutableField(Node *node) {
   unreachable("should be part of PILBoxTypeWithLayout");
}

void Remangler::manglePILBoxImmutableField(Node *node) {
   unreachable("should be part of PILBoxTypeWithLayout");
}
...
void Remangler::mangleInterfaceSymbolicReference(Node *node) {
   return mangle(Resolver(SymbolicReferenceKind::Context,
                          (const void *) node->getIndex()));
}
...
std::string mangleNode(NodePointer node) {
...
std::string mangleNode(NodePointer node, SymbolicResolver resolver) {
   if (!node) return "";
...
llvm::StringRef mangleNode(NodePointer node, SymbolicResolver resolver,
                     NodeFactory &Factory) {
   if (!node)
      return StringRef();

...
bool ArgsToFrontendOptionsConverter::convert(
   ...
   Opts.EmitSortedPIL |= Args.hasArg(OPT_emit_sorted_pil);
...
ArgsToFrontendOptionsConverter::determineRequestedAction(const ArgList &args) {
   ...
   if (Opt.matches(OPT_emit_pil))
      return FrontendOptions::ActionType::EmitPIL;
   if (Opt.matches(OPT_emit_pilgen))
      return FrontendOptions::ActionType::EmitPILGen;
   if (Opt.matches(OPT_emit_sib))
      return FrontendOptions::ActionType::EmitPIB;
   if (Opt.matches(OPT_emit_sibgen))
      return FrontendOptions::ActionType::EmitPIBGen;
...
bool ArgsToFrontendOptionsConverter::setUpInputKindAndImmediateArgs() {
   using namespace options;
   bool treatAsPIL =
      Args.hasArg(OPT_parse_pil) || Opts.InputsAndOutputs.shouldTreatAsPIL();
   ...
   if (treatAsPIL)
      Opts.InputKind = InputFileKind::PIL;
      ...
      Opts.InputKind = InputFileKind::PHPModuleInterface;
   else if (Args.hasArg(OPT_parse_as_library))
      Opts.InputKind = InputFileKind::PHPLibrary;
   else if (Opts.RequestedAction == FrontendOptions::ActionType::REPL)
      Opts.InputKind = InputFileKind::PHPREPL;
   else
      Opts.InputKind = InputFileKind::PHP;
...
bool ArgsToFrontendOptionsConverter::computeModuleName() {
   ...
   if (!FrontendOptions::needsProperModuleName(Opts.RequestedAction) ||
       Opts.isCompilingExactlyOnePolarphpFile()) {
...
const {
   ...
   if (!FrontendOptions::canActionEmitPHPRanges(Opts.RequestedAction) &&
       Opts.InputsAndOutputs.hasPHPRangesPath()) {
      Diags.diagnose(SourceLoc(), diag::error_mode_cannot_emit_php_ranges);
      return true;
   }

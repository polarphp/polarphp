...
std::string
polar::getModuleCachePathFromClang(const clang::CompilerInstance &Clang) {
...
class ModuleInterfaceLoaderImpl {
   ...
   friend class polar::ModuleInterfaceLoader;
   AstContext &ctx;
   ...
   std::string getCacheHash(const CompilerInvocation &SubInvocation) {
      auto normalizedTargetTriple =
         get_target_specific_module_triple(SubInvocation.getLangOptions().Target);
      ...
      llvm::hash_code H = hash_combine(
         ...
         polar::version::retrieve_polarphp_full_version(),
   ...
   void computeCachedOutputPath(const CompilerInvocation &SubInvocation,
      ...
      OutPath.append(".");
      auto OutExt = filetypes::get_extension(filetypes::TY_PHPModuleFile);
...
//      }
   ...
   computeFallbackPrebuiltModulePath(llvm::SmallString<256> &scratch) {
      ...
      StringRef inParentDirName =
         path::filename(path::parent_path(interfacePath));
      if (path::extension(inParentDirName) != ".phpmodule")
         return None;

      // If the interface is already using the target-specific name, there's
      // nothing else to try.
      auto normalizedTarget = get_target_specific_module_triple(ctx.LangOpts.Target);
...
//            !serialization::isSerializedAST(buf->getBuffer());
   ...
   findOrBuildLoadableModule() {
      ...
      if (builder.buildPHPModule(cachedOutputPath, /*shouldSerializeDeps*/true,
                                   &moduleBuffer))
...
std::error_code ModuleInterfaceLoader::findModuleFilesInDirectory(
   ...
   auto Ext = filetypes::get_extension(filetypes::TY_PHPModuleInterfaceFile);
   InPath = ModPath;
   path::replace_extension(InPath, Ext);
   if (!fs.exists(InPath)) {
      if (fs.exists(ModPath)) {
         LLVM_DEBUG(llvm::dbgs()
                       << "No .phpinterface file found adjacent to module file "
                       << ModPath.str() << "\n");
         return std::make_error_code(std::errc::not_supported);
      }
...
bool ModuleInterfaceLoader::buildSwiftModuleFromSwiftInterface(
   ...
   return builder.buildPHPModule(OutPath, /*shouldSerializeDeps*/true,

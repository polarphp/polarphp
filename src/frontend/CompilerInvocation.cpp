...
/// The path for Polarphp libraries in the OS on Darwin.
#define DARWIN_OS_LIBRARY_PATH "/usr/lib/phpc"

polar::CompilerInvocation::CompilerInvocation() {
...
void CompilerInvocation::setMainExecutablePath(StringRef Path) {
   ...
   llvm::sys::path::remove_filename(DiagnosticDocsPath); // Remove /polarphp
   llvm::sys::path::remove_filename(DiagnosticDocsPath); // Remove /bin
   llvm::sys::path::append(DiagnosticDocsPath, "share", "doc", "polarphp",
                           "diagnostics");
...
static void setDefaultPrebuiltCacheIfNecessary(
   ...
   StringRef platform = get_platform_name_for_triple(triple);
...
static void updateRuntimeLibraryPaths(SearchPathOptions &SearchPathOpts,
   ...
   llvm::sys::path::append(LibPath, get_platform_name_for_triple(Triple));
   ...
   if (!Triple.isOSDarwin())
      llvm::sys::path::append(LibPath, polar::get_major_architecture_name(Triple));
   SearchPathOpts.RuntimeLibraryImportPaths.push_back(LibPath.str());

   if (!SearchPathOpts.SDKPath.empty()) {
      LibPath = SearchPathOpts.SDKPath;
      llvm::sys::path::append(LibPath, "usr", "lib", "phpc");
      if (!Triple.isOSDarwin()) {
         llvm::sys::path::append(LibPath, get_platform_name_for_triple(Triple));
         llvm::sys::path::append(LibPath, polar::get_major_architecture_name(Triple));
      }
...
SourceFileKind CompilerInvocation::getSourceFileKind() const {
   switch (getInputKind()) {
      case InputFileKind::PHP:
         return SourceFileKind::Main;
      case InputFileKind::PHPLibrary:
         return SourceFileKind::Library;
      case InputFileKind::PHPREPL:
         return SourceFileKind::REPL;
      case InputFileKind::PHPModuleInterface:
         return SourceFileKind::Interface;
      case InputFileKind::PIL:
         return SourceFileKind::PIL;
...
static void diagnosePHPVersion(llvm::Optional<version::Version> &vers, Arg *verArg,
   ...
   diags.diagnose(SourceLoc(), diag::note_valid_php_versions, versStr);
}
...
static bool ParseLangArgs(LangOptions &Opts, ArgList &Args,
   ...
   if (auto A = Args.getLastArg(OPT_polarphp_version)) {
      ...
      if (!isValid)
         diagnosePHPVersion(vers, A, Args, Diags);
   }
   ...
   Opts.EnableSubstPILFunctionTypesForFunctionValues |=
      Args.hasArg(OPT_enable_subst_pil_function_types_for_function_values);
   ...
   if (FrontendOpts.InputKind == InputFileKind::PIL)
...
//      Args.hasFlag(OPT_enable_php3_objc_inference,
//                   OPT_disable_php3_objc_inference, false);
   // @todo
//   if (Opts.EnableSwift3ObjCInference) {
//      if (const Arg *A = Args.getLastArg(
//         OPT_warn_php3_objc_inference_minimal,
//         OPT_warn_php3_objc_inference_complete)) {
//         if (A->getOption().getID() == OPT_warn_php3_objc_inference_minimal)
//            Opts.WarnPHP3ObjCInference = Swift3ObjCInferenceWarnings::Minimal;
//         else
...
//   }
   ...
   Opts.EnableNonFrozenEnumExhaustivityDiagnostics =
      Args.hasFlag(OPT_enable_nonfrozen_enum_exhaustivity_diagnostics,
                   ...
                   Opts.isPolarphpVersionAtLeast(5));
   ...
   Opts.EnableObjCInterop =
      Args.hasFlag(OPT_enable_objc_interop, OPT_disable_objc_interop,
                   Target.isOSDarwin());
   Opts.EnablePILOpaqueValues |= Args.hasArg(OPT_enable_pil_opaque_values);
...
static bool ParseTypeCheckerArgs(TypeCheckerOptions &Opts, ArgList &Args,
   ...
   auto setUnsignedIntegerArgument =
      [&Args, &Diags, &HadError](options::FileTypeId optionID, unsigned &valueToSet) {
...
static bool ParsePILArgs(PILOptions &Opts, ArgList &Args,
   ...
   if (const Arg *A = Args.getLastArg(OPT_pil_inline_threshold)) {
   ...
   if (const Arg *A = Args.getLastArg(OPT_pil_inline_caller_benefit_reduction_factor)) {
   ...
   if (const Arg *A = Args.getLastArg(OPT_pil_unroll_threshold)) {
   ...
   if (const Arg *A = Args.getLastArg(OPT_O_Group)) {
      if (A->getOption().matches(OPT_Onone)) {
         ...
         Opts.AssertConfig = PILOptions::Unchecked;
   ...
   if (const Arg *A = Args.getLastArg(OPT_AssertConfig)) {
      ...
      if (Configuration == "DisableReplacement") {
         Opts.AssertConfig = PILOptions::DisableReplacement;
      } else if (Configuration == "Debug") {
         Opts.AssertConfig = PILOptions::Debug;
      } else if (Configuration == "Release") {
         Opts.AssertConfig = PILOptions::Release;
   ...
   Opts.DisablePILPerfOptimizations |= Args.hasArg(OPT_disable_pil_perf_optzns);
   Opts.CrossModuleOptimization |= Args.hasArg(OPT_CrossModuleOptimization);
   Opts.VerifyAll |= Args.hasArg(OPT_pil_verify_all);
   Opts.DebugSerialization |= Args.hasArg(OPT_pil_debug_serialization);
   Opts.EmitVerbosePIL |= Args.hasArg(OPT_emit_verbose_pil);
   ...
   Opts.DisablePILPartialApply |=
      Args.hasArg(OPT_disable_pil_partial_apply);
   Opts.VerifyPILOwnership &= !Args.hasArg(OPT_disable_pil_ownership_verifier);
   ...
   if (Args.hasArg(OPT_debug_on_pil)) {
      // Derive the name of the PIL file for debugging from
      ...
      Opts.PILOutputFileNameForDebugging = BaseName;
   }

   if (const Arg *A = Args.getLastArg(options::OPT_sanitize_EQ)) {
      Opts.Sanitizers = parse_sanitizer_arg_values(
         Args, A, Triple, Diags,
   ...
   if (const Arg *A = Args.getLastArg(options::OPT_sanitize_recover_EQ)) {
      IRGenOpts.SanitizersWithRecoveryInstrumentation =
         parse_sanitizer_recover_arg_values(A, Opts.Sanitizers, Diags,
            /*emitWarnings=*/true);
   }
...
static bool ParseTBDGenArgs(TBDGenOptions &Opts, ArgList &Args,
   ...
   Opts.HasMultipleIGMs = Invocation.getPILOptions().hasMultipleIGMs();
...
static bool ParseIRGenArgs(IRGenOptions &Opts, ArgList &Args,
                           DiagnosticEngine &Diags,
                           const FrontendOptions &FrontendOpts,
                           const PILOptions &PILOpts,
                           StringRef SDKPath,
                           StringRef ResourceDir,
                           const llvm::Triple &Triple) {
   using namespace options;

   if (!PILOpts.PILOutputFileNameForDebugging.empty()) {
   ...
   Opts.DisablePHPSpecificLLVMOptzns |=
      Args.hasArg(OPT_disable_php_specific_llvm_optzns);
   ...
   Opts.UsePHPCall = Args.hasArg(OPT_enable_phpcall);
   ...
   if (const Arg *A = Args.getLastArg(options::OPT_sanitize_coverage_EQ)) {
      Opts.SanitizeCoverage =
         parse_sanitizer_coverage_arg_value(A, Triple, Diags, Opts.Sanitizers);
   ...
   // witness.
   Opts.LazyInitializeInterfaceConformances = Triple.isOSBinFormatCOFF();
...
bool CompilerInvocation::parseArgs(
   ...
   *ConfigurationFileBuffers,
   ...
   std::unique_ptr<llvm::opt::OptTable> Table = create_polarphp_opt_table();
   ...
   if (ParsePILArgs(PILOpts, ParsedArgs, IRGenOpts, FrontendOpts, Diags,
                    LangOpts.Target, ClangImporterOpts)) {
      return true;
   }

   if (ParseIRGenArgs(IRGenOpts, ParsedArgs, Diags, FrontendOpts, PILOpts,
...
CompilerInvocation::setUpInputForPILTool(
   ...
   // If it looks like we have an AST, set the source file kind to PIL and the

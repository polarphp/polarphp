...
CompilerInstance::openModuleSourceInfo(const InputFile &input) {
   ...
   llvm::sys::path::replace_extension(pathWithoutProjectDir,
                                      filetypes::get_extension(filetypes::TY_PHPSourceInfoFile));
...
CompilerInstance::openModuleDoc(const InputFile &input) {
   ...
   llvm::sys::path::replace_extension(
      moduleDocFilePath,
      filetypes::get_extension(filetypes::TY_PHPModuleDocFile));
   using FileOrError = llvm::ErrorOr<std::unique_ptr<llvm::MemoryBuffer>>;
   FileOrError moduleDocFileOrErr =
      polar::vfs::get_file_or_stdin(getFileSystem(), moduleDocFilePath);
...
shouldImplicityImportSwiftOnoneSupportModule(CompilerInvocation &Invocation) {
   if (Invocation.getImplicitModuleImportKind() !=
      ...
      return false;
   if (Invocation.getPILOptions().shouldOptimize())
      return false;
...
void CompilerInstance::parseAndCheckTypesUpTo(
   const ImplicitImports &implicitImports, SourceFile::AstStageType limitStage) {
...
void CompilerInstance::performParseOnly(bool EvaluateConditionals,
   ...
   assert((Kind == InputFileKind::PHP ||
           Kind == InputFileKind::PHPLibrary ||
           Kind == InputFileKind::PHPModuleInterface) &&

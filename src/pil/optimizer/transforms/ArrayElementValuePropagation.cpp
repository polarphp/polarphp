...
class ArrayAllocation {
  ...
  llvm::DenseMap<uint64_t, PILValue> ElementValueMap;

  bool mapInitializationStores(PILValue ElementBuffer);
...
bool ArrayAllocation::mapInitializationStores(PILValue ElementBuffer) {
...
bool ArrayAllocation::analyze(ApplyInst *Alloc) {
  ...
  PILValue ElementBuffer = Uninitialized.getArrayElementStoragePointer();
  if (!ElementBuffer)
    return false;
...
bool ArrayAllocation::replaceAppendContentOf() {
  ...
  llvm::SmallVector<PILValue, 4> ElementValueVector;
  for (unsigned i = 0; i < ElementValueMap.size(); ++i) {
    PILValue V = ElementValueMap[i];
    ElementValueVector.push_back(V);
  }

  PILFunction *Fn = AppendContentsOfCalls[0]->getFunction();
  ...
  auto Mangled = PILDeclRef(AppendFnDecl, PILDeclRef::Kind::Func).mangle();
  PILFunction *AppendFn = M.findFunction(Mangled, PILLinkage::PublicExternal);
  if (!AppendFn)
    return false;

  Mangled = PILDeclRef(ReserveFnDecl, PILDeclRef::Kind::Func).mangle();
  PILFunction *ReserveFn = M.findFunction(Mangled, PILLinkage::PublicExternal);
  if (!ReserveFn)
    return false;
...
class ArrayElementPropagation : public PILFunctionTransform {
public:
  ...
  void run() override {
      ...
      for (auto &Inst : BB) {
        if (auto *Apply = dyn_cast<ApplyInst>(&Inst)) {
          ...
          if (ALit.replaceGetElements()) {
            Changed = true;
            // Re-do the analysis if the PIL changed.
            if (!ALit.analyze(Apply))
              continue;
          }
    ...
    if (Changed) {
      PM->invalidateAnalysis(
          &Fn, PILAnalysis::InvalidationKind::CallsAndInstructions);
    }
  }
};
} // end anonymous namespace

PILTransform *polar::createArrayElementPropagation() {

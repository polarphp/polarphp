...
#define DEBUG_TYPE "pil-speculative-devirtualizer"
...
static PILBasicBlock *cloneEdge(TermInst *TI, unsigned SuccIndex) {
#ifndef NDEBUG
...
static FullApplySite CloneApply(FullApplySite AI, PILBuilder &Builder) {
   // Clone the Apply.
   Builder.setCurrentDebugScope(AI.getDebugScope());
   Builder.addOpenedArchetypeOperands(AI.getInstruction());
   auto Args = AI.getArguments();
   SmallVector<PILValue, 8> Ret(Args.size());
...
static FullApplySite speculateMonomorphicTarget(FullApplySite AI,
   ...
   PILBasicBlock *Entry = AI.getParent();

   // Iden is the basic block containing the direct call.
   PILBasicBlock *Iden = F->createBasicBlock();
   // Virt is the block containing the slow virtual call.
   PILBasicBlock *Virt = F->createBasicBlock();
   Iden->createPhiArgument(
      PILType::getPrimitiveObjectType(SubType),
      ValueOwnershipKind::Owned);

   PILBasicBlock *Continue = Entry->split(It);

   PILBuilderWithScope Builder(Entry, AI.getInstruction());
   // Create the checked_cast_branch instruction that checks at runtime if the
   // class instance is identical to the PILType.
   ...
   PILBuilderWithScope VirtBuilder(Virt, AI.getInstruction());
   PILBuilderWithScope IdenBuilder(Iden, AI.getInstruction());
   // This is the class reference downcasted into subclass SubType.
   PILValue DownCastedClassInstance = Iden->getArgument(0);
...
static bool tryToSpeculateTarget(FullApplySite AI, ClassHierarchyAnalysis *CHA,
   ...
   if (LastCCBI && SubTypeValue == LastCCBI->getOperand()) {
      ...
      PILBuilderWithScope B(LastCCBI);
...
class SpeculativeDevirtualization : public PILFunctionTransform {
public:
   ...
   void run() override {
      ...
      if (Changed) {
         CurFn.getModule().linkFunction(&CurFn, PILModule::LinkingMode::LinkAll);
...
PILTransform *polar::createSpeculativeDevirtualization() {

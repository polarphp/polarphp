//===--- ARCCodeMotion.cpp - PIL ARC Code Motion --------------------------===//
//
...
///
/// This pass is more sophisticated than PILCodeMotion, as arc optimizations
...
#define DEBUG_TYPE "pil-rr-code-motion"
...
protected:
   ...
   llvm::SmallPtrSet<PILInstruction *, 8> RCInstructions;

   /// All the places to place the new retains or releases after code motion.
   using InsertPointList = llvm::SmallVector<PILInstruction *, 2>;
   llvm::SmallDenseMap<PILValue, InsertPointList> InsertPoints;
   ...
   llvm::SmallPtrSet<PILBasicBlock *, 8> InterestBlocks;

#ifndef NDEBUG
   // PILPrintContext is used to print block IDs in RPO order.
   // It is optional so only the final insertion point interference is printed.
   Optional<PILPrintContext> printCtx;
#endif

   /// Return the rc-identity root of the PILValue.
   PILValue getRCRoot(PILValue R) {
...
public:
   ...
   virtual void mergeBBDataFlowStates(PILBasicBlock *BB) = 0;

   /// Compute the BBSetIn and BBSetOut for the current basic
   /// block with the generated gen and kill set.
   virtual bool processBBWithGenKillSet(PILBasicBlock *BB) = 0;

   /// Return true if the instruction blocks the Ptr to be moved further.
   virtual bool mayBlockCodeMotion(PILInstruction *II, PILValue Ptr) = 0;
};
...
class RetainCodeMotionContext : public CodeMotionContext {
   ...
   llvm::SmallDenseMap<PILBasicBlock *, RetainBlockState *> BlockStates;

   ProgramTerminationFunctionInfo PTFI;

   /// Return true if the instruction blocks the Ptr to be moved further.
   bool mayBlockCodeMotion(PILInstruction *II, PILValue Ptr) override {
   ...
   PILInstruction *getPrevReusableInst(PILInstruction *I, PILValue Root) {
      if (&*I->getParent()->begin() == I)
         return nullptr;
      auto Prev = &*std::prev(PILBasicBlock::iterator(I));
...
public:
   ...
   RetainCodeMotionContext(llvm::SpecificBumpPtrAllocator<BlockState> &BPA,
                           PILFunction *F, PostOrderFunctionInfo *PO,
   ...
   bool processBBWithGenKillSet(PILBasicBlock *BB) override;

   /// Merge the data flow states.
   void mergeBBDataFlowStates(PILBasicBlock *BB) override;
};
...
bool RetainCodeMotionContext::requireIteration() {
   ...
   // genset and killset.
   llvm::SmallPtrSet<PILBasicBlock *, 4> PBBs;
...
void RetainCodeMotionContext::mergeBBDataFlowStates(PILBasicBlock *BB) {
   BlockState *State = BlockStates[BB];
   State->BBSetIn.reset();
...
bool RetainCodeMotionContext::processBBWithGenKillSet(PILBasicBlock *BB) {
...
void RetainCodeMotionContext::convergeCodeMotionDataFlow() {
   ...
   llvm::SmallPtrSet<PILBasicBlock *, 4> HandledBBs;
...
class ReleaseCodeMotionContext : public CodeMotionContext {
   ...
   llvm::SmallDenseMap<PILBasicBlock *, ReleaseBlockState *> BlockStates;
...
void ReleaseCodeMotionContext::initializeCodeMotionDataFlow() {
   ...
   llvm::SmallPtrSet<PILBasicBlock *, 32> BlocksInitOptimistically;

   llvm::SmallVector<PILBasicBlock *, 32> Worklist;
...
void ReleaseCodeMotionContext::initializeCodeMotionBBMaxSet() {
   for (PILBasicBlock *BB : PO->getPostOrder()) {
...
void ReleaseCodeMotionContext::computeCodeMotionGenKillSet() {
   for (PILBasicBlock *BB : PO->getPostOrder()) {
      ...
      // Handle PILArgument, PILArgument can invalidate.
      for (unsigned i = 0; i < RCRootVault.size(); ++i) {
         auto *A = dyn_cast<PILArgument>(RCRootVault[i]);
         if (!A || A->getParent() != BB)
            continue;
         InterestBlock = true;
         State->BBKillSet.set(i);
...
void ReleaseCodeMotionContext::mergeBBDataFlowStates(PILBasicBlock *BB) {
   BlockState *State = BlockStates[BB];
...
bool ReleaseCodeMotionContext::performCodeMotion() {
   bool Changed = false;
   SmallVector<PILInstruction *, 8> NewReleases;
   ...
   for (auto RC : RCRootVault) {
      ...
      for (auto IP : Iter->second) {
         ...
         if (PILInstruction *I = createDecrementBefore(Iter->first, IP).getPtrOrNull())
            NewReleases.push_back(I);
         Changed = true;
      }
   }
   ...
   //    strong_retain %2
   //    strong_release %2
   for (PILInstruction *ReleaseInst : NewReleases) {
      auto InstIter = ReleaseInst->getIterator();
      if (InstIter == ReleaseInst->getParent()->begin())
         continue;

      PILInstruction *PrevInst = &*std::prev(InstIter);
...
bool ReleaseCodeMotionContext::processBBWithGenKillSet(PILBasicBlock *BB) {
...
void ReleaseCodeMotionContext::convergeCodeMotionDataFlow() {
   ...
   // predecessors.
   llvm::SmallVector<PILBasicBlock *, 16> WorkList;
   llvm::SmallPtrSet<PILBasicBlock *, 8> HandledBBs;
   // Push into reverse post order so that we can pop from the back and get
   // post order.
   for (PILBasicBlock *B : PO->getPostOrder()) {
      WorkList.push_back(B);
...
void ReleaseCodeMotionContext::computeCodeMotionInsertPoints() {
   ...
   for (PILBasicBlock *BB : PO->getPostOrder()) {
      ...
      for (unsigned i = 0; i < RCRootVault.size(); ++i) {
         PILInstruction *Term = BB->getTerminator();
      ...
      for (auto I = std::next(BB->rbegin()), E = BB->rend(); I != E; ++I) {
         for (unsigned i = 0; i < RCRootVault.size(); ++i) {
            if (!S->BBSetOut[i] || !mayBlockCodeMotion(&*I, RCRootVault[i]))
               continue;
            auto *InsertPt = &*std::next(PILBasicBlock::iterator(&*I));
      ...
      // Compute insertion point generated by PILArgument. PILArgument blocks if
...
static void eliminateRetainsPrecedingProgramTerminationPoints(PILFunction *f) {
   for (auto &block : *f) {
...
class ARCCodeMotion : public PILFunctionTransform {
...
public:
   ...
   void run() override {
      ...
      if (Kind == Release) {
         ...
         PILArgumentConvention Conv[] = {PILArgumentConvention::Direct_Owned};
...
/// Sink Retains.
PILTransform *polar::createRetainSinking() {
...
PILTransform *polar::createReleaseHoisting() {
   return new ARCCodeMotion(CodeMotionKind::Release, true);
}

/// Hoist all releases.
PILTransform *polar::createLateReleaseHoisting() {

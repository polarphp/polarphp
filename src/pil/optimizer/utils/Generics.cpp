...
static std::pair<unsigned, unsigned> getTypeDepthAndWidth(Type t) {
   ...
   if (auto *FnTy = t->getAs<PILFunctionType>()) {
      Depth++;
      unsigned MaxTypeDepth = 0;
...
static bool shouldNotSpecialize(PILFunction *Callee, PILFunction *Caller,
                                SubstitutionMap Subs = {}) {
   if (Callee->hasSemanticsAttr(semantics::OPTIMIZE_PIL_SPECIALIZE_GENERIC_NEVER))
      return true;

   if (Caller &&
       Caller->getEffectiveOptimizationMode() == OptimizationMode::ForSize &&
       Callee->hasSemanticsAttr(semantics::OPTIMIZE_PIL_SPECIALIZE_GENERIC_SIZE_NEVER)) {
      return true;
   }


   if (Subs.hasAnySubstitutableParams() &&
       Callee->hasSemanticsAttr(semantics::OPTIMIZE_PIL_SPECIALIZE_GENERIC_PARTIAL_NEVER))
      return true;
...
bool ReabstractionInfo::prepareAndCheck(ApplySite Apply, PILFunction *Callee,
...
bool ReabstractionInfo::canBeSpecialized(ApplySite Apply, PILFunction *Callee,
                                         SubstitutionMap ParamSubs) {
...
ReabstractionInfo::ReabstractionInfo(
   ...
   PILFunction *Callee, SubstitutionMap ParamSubs, IsSerialized_t Serialized,
   ...
   if (getSubstitutedType() == Callee->getLoweredFunctionType()) {
      ...
      SpecializedType = CanPILFunctionType();
      SubstitutedType = CanPILFunctionType();
      SpecializedGenericSig = nullptr;
      SpecializedGenericEnv = nullptr;
      return;
   }
...
void ReabstractionInfo::createSubstitutedAndSpecializedTypes() {
   ...
   PILFunctionConventions substConv(SubstitutedType, M);
   ...
   if (SubstitutedType->getNumDirectFormalResults() == 0) {
      ...
      for (PILResultInfo RI : SubstitutedType->getIndirectFormalResults()) {
         assert(RI.isFormalIndirect());

         auto ResultTy = substConv.getPILType(RI);
   ...
   for (PILParameterInfo PI : SubstitutedType->getParameters()) {
      auto IdxToInsert = IdxForParam;
      ++IdxForParam;

      auto ParamTy = substConv.getPILType(PI);
...
CanPILFunctionType
ReabstractionInfo::createSubstitutedType(PILFunction *OrigF,
   ...
   CanPILFunctionType FnTy;
   {
      ...
      if (CanSpecializedGenericSig)
         FnTy = cast<PILFunctionType>(
            CanSpecializedGenericSig->getCanonicalTypeInContext(FnTy));
      else {
         FnTy = cast<PILFunctionType>(FnTy->getCanonicalType());
   ...
   auto NewFnTy = PILFunctionType::get(
...
CanPILFunctionType ReabstractionInfo::
createSpecializedType(CanPILFunctionType SubstFTy, PILModule &M) const {
   llvm::SmallVector<PILResultInfo, 8> SpecializedResults;
   llvm::SmallVector<PILYieldInfo, 8> SpecializedYields;
   llvm::SmallVector<PILParameterInfo, 8> SpecializedParams;

   unsigned IndirectResultIdx = 0;
   for (PILResultInfo RI : SubstFTy->getResults()) {
      if (RI.isFormalIndirect()) {
         if (isFormalResultConverted(IndirectResultIdx++)) {
            // Convert the indirect result to a direct result.
            PILType PILResTy =
               PILType::getPrimitiveObjectType(RI.getReturnValueType(M, SubstFTy));

            // Indirect results are passed as owned, so we also need to pass the
            // direct result as owned (except it's a trivial type).
            auto C = (PILResTy.isTrivial(*Callee)
                      ? ResultConvention::Unowned
                      : ResultConvention::Owned);
            SpecializedResults.push_back(PILResultInfo(RI.getReturnValueType(M, SubstFTy), C));
            continue;
         }
      }
   ...
   unsigned ParamIdx = 0;
   for (PILParameterInfo PI : SubstFTy->getParameters()) {
      ...
      PILType PILParamTy =
         PILType::getPrimitiveObjectType(PI.getArgumentType(M, SubstFTy));
      ...
      if (!PILParamTy.isTrivial(*Callee)) {
         if (PI.isGuaranteed()) {
      ...
      SpecializedParams.push_back(PILParameterInfo(PI.getArgumentType(M, SubstFTy), C));
   }
   for (PILYieldInfo YI : SubstFTy->getYields()) {
...
getGenericEnvironmentAndSignatureWithRequirements(
   ...
   ArrayRef<Requirement> Requirements, PILModule &M) {
...
public:
   FunctionSignaturePartialSpecializer(PILModule &M,
                                       GenericSignature CallerGenericSig,
                                       GenericEnvironment *CallerGenericEnv,
                                       GenericSignature CalleeGenericSig,
                                       GenericEnvironment *CalleeGenericEnv,
...
void ReabstractionInfo::performPartialSpecializationPreparation(
   PILFunction *Caller, PILFunction *Callee,
   SubstitutionMap ParamSubs) {
   PILModule &M = Callee->getModule();

   // Caller is the PILFunction containing the apply instruction.
...
ReabstractionInfo::ReabstractionInfo(ModuleDecl *targetModule,
                                     bool isWholeModule, PILFunction *Callee,
                                     GenericSignature SpecializedSig)
   : TargetModule(targetModule), isWholeModule(isWholeModule) {
...
GenericFuncSpecializer::GenericFuncSpecializer(
   PILOptFunctionBuilder &FuncBuilder, PILFunction *GenericFunc,
   SubstitutionMap ParamSubs,
...
PILFunction *GenericFuncSpecializer::lookupSpecialization() {
   if (PILFunction *SpecializedF = M.lookUpFunction(ClonedName)) {
...
PILFunction *GenericFuncSpecializer::tryCreateSpecialization() {
   ...
   PILFunction *SpecializedF = GenericCloner::cloneFunction(
...
static void fixUsedVoidType(PILValue VoidVal, PILLocation Loc,
                            PILBuilder &Builder) {
...
static void prepareCallArguments(ApplySite AI, PILBuilder &Builder,
                                 const ReabstractionInfo &ReInfo,
                                 SmallVectorImpl<PILValue> &Arguments,
                                 PILValue &StoreResultTo) {
   /// PIL function conventions for the original apply site with substitutions.
   ...
   for (auto &Op : AI.getArgumentOperands()) {
      auto handleConversion = [&]() {
         // Rewriting PIL arguments is only for lowered addresses.
         ...
         if (ArgIdx < substConv.getPILArgIndexOfFirstParam()) {
            ...
            if (ReInfo.isParamConverted(paramIdx)) {
               ...
               PILValue Val = Builder.createLoad(
...
getCalleeSubstFunctionType(PILValue Callee, SubstitutionMap Subs,
                           TypeExpansionContext context) {
   // Create a substituted callee type.
   auto CanFnTy = Callee->getType().castTo<PILFunctionType>();
...
static ApplySite replaceWithSpecializedCallee(ApplySite AI,
   ...
   PILFunctionConventions substConv(CalleeSubstFnTy, Builder.getModule());

   if (auto *TAI = dyn_cast<TryApplyInst>(AI)) {
      PILBasicBlock *ResultBB = TAI->getNormalBB();
      ...
      if (StoreResultTo) {
         ...
         PILArgument *Arg = ResultBB->replacePhiArgument(
   ...
   if (auto *A = dyn_cast<ApplyInst>(AI)) {
      ...
      if (StoreResultTo) {
         assert(substConv.useLoweredAddresses());
         if (!CalleePILSubstFnTy.isNoReturnFunction(Builder.getModule())) {
            // Store the direct result to the original result address.
   ...
   if (auto *PAI = dyn_cast<PartialApplyInst>(AI)) {
      auto *NewPAI = Builder.createPartialApply(
         ...
         PAI->getType().getAs<PILFunctionType>()->getCalleeConvention(),
         PAI->isOnStack());
...
/// function being applied.
ApplySite polar::
replaceWithSpecializedFunction(ApplySite AI, PILFunction *NewF,
                               const ReabstractionInfo &ReInfo) {
   PILBuilderWithScope Builder(AI.getInstruction());
...
public:
   ReabstractionThunkGenerator(PILOptFunctionBuilder &FunctionBuilder,
                               const ReabstractionInfo &ReInfo,
                               PartialApplyInst *OrigPAI,
                               PILFunction *SpecializedFunc)
   ...
   PILFunction *createThunk();

protected:
   PILValue createReabstractionThunkApply(PILBuilder &Builder);
   PILArgument *convertReabstractionThunkArguments(PILBuilder &Builder);
};
} // anonymous namespace

PILFunction *ReabstractionThunkGenerator::createThunk() {
   PILFunction *Thunk = FunctionBuilder.getOrCreateSharedFunction(
   ...
   PILBasicBlock *EntryBB = Thunk->createBasicBlock();
   ...
   if (!PILModuleConventions(M).useLoweredAddresses()) {
      for (auto SpecArg : SpecializedFunc->getArguments()) {
         PILArgument *NewArg = EntryBB->createFunctionArgument(SpecArg->getType(),
                                                               SpecArg->getDecl());
         Arguments.push_back(NewArg);
      }
      PILValue ReturnValue = createReabstractionThunkApply(Builder);
      Builder.createReturn(Loc, ReturnValue);
      return Thunk;
   }
   // Handle lowered addresses.
   PILArgument *ReturnValueAddr = convertReabstractionThunkArguments(Builder);
...
PILValue ReabstractionThunkGenerator::createReabstractionThunkApply(
   PILBuilder &Builder) {
   PILFunction *Thunk = &Builder.getFunction();
   ...
   PILBasicBlock *NormalBB = Thunk->createBasicBlock();
   PILBasicBlock *ErrorBB = Thunk->createBasicBlock();
   Builder.createTryApply(Loc, FRI, Subs, Arguments, NormalBB, ErrorBB);
   auto *ErrorVal = ErrorBB->createPhiArgument(
      SpecializedFunc->mapTypeIntoContext(specConv.getPILErrorType()),
      ValueOwnershipKind::Owned);
   Builder.setInsertionPoint(ErrorBB);
   Builder.createThrow(Loc, ErrorVal);
   PILValue ReturnValue = NormalBB->createPhiArgument(
      SpecializedFunc->mapTypeIntoContext(specConv.getPILResultType()),
      ValueOwnershipKind::Owned);
   Builder.setInsertionPoint(NormalBB);
   return ReturnValue;
}

/// Create PIL arguments for a reabstraction thunk with lowered addresses. This
/// may involve replacing indirect arguments with loads and stores. Return the
/// PILArgument for the address of an indirect result, or nullptr.
///
/// FIXME: Remove this if we don't need to create reabstraction thunks after
/// address lowering.
PILArgument *ReabstractionThunkGenerator::convertReabstractionThunkArguments(
   PILBuilder &Builder) {
   PILFunction *Thunk = &Builder.getFunction();
   CanPILFunctionType SpecType = SpecializedFunc->getLoweredFunctionType();
   CanPILFunctionType SubstType = ReInfo.getSubstitutedType();
   auto specConv = SpecializedFunc->getConventions();
   (void)specConv;
   PILFunctionConventions substConv(SubstType, M);
   ...
   PILBasicBlock *EntryBB = Thunk->getEntryBlock();
   PILArgument *ReturnValueAddr = nullptr;
   auto SpecArgIter = SpecializedFunc->getArguments().begin();
   auto cloneSpecializedArgument = [&]() {
      // No change to the argument.
      PILArgument *SpecArg = *SpecArgIter++;
      PILArgument *NewArg =
   ...
   for (auto substRI : SubstType->getIndirectFormalResults()) {
      if (ReInfo.isFormalResultConverted(resultIdx++)) {
         ...
         PILType ResultTy =
            SpecializedFunc->mapTypeIntoContext(substConv.getPILType(substRI));
   ...
   for (unsigned paramIdx = 0; paramIdx < numParams; ++paramIdx) {
      if (ReInfo.isParamConverted(paramIdx)) {
         ...
         assert(!specConv.isPILIndirect(SpecType->getParameters()[paramIdx]));
         // Instead of passing the address, pass the loaded value.
         PILType ParamTy = SpecializedFunc->mapTypeIntoContext(
            substConv.getPILType(SubstType->getParameters()[paramIdx]));
...
static bool createPrespecialized(StringRef UnspecializedName,
   ...
   PILModule &M = FuncBuilder.getModule();
   PILFunction *UnspecFunc = M.lookUpFunction(UnspecializedName);
   if (UnspecFunc) {
...
//         /*declarationOnly*/ false);
   ...
   GenericFuncSpecializer FuncSpecializer(FuncBuilder,
                                          UnspecFunc, Apply.getSubstitutionMap(),
                                          ReInfo);
   PILFunction *SpecializedF = FuncSpecializer.lookupSpecialization();
   if (!SpecializedF)
   ...
   M.linkFunction(SpecializedF, PILModule::LinkingMode::LinkAll);

   SpecializedF->setLinkage(PILLinkage::Public);
...
static bool createPrespecializations(ApplySite Apply, PILFunction *ProxyFunc,
                                     PILOptFunctionBuilder &FuncBuilder) {
...
void polar::trySpecializeApplyOfGeneric(
   PILOptFunctionBuilder &FuncBuilder,
   ...
   // as we do not PIL serialize their bodies.
...
#include "polarphp/pil/optimizer/internal/utils/OnonePrespecializationsDef.h"
   nullptr
};
...
bool polar::isKnownPrespecialization(StringRef SpecName) {
   if (PrespecSet.empty()) {
...
void polar::checkCompletenessOfPrespecializations(PILModule &M) {
   const char **Pos = &PrespecSymbols[0];
   while (const char *Sym = *Pos++) {
      StringRef FunctionName(Sym);
      PILFunction *F = M.lookUpFunction(FunctionName);
      if (!F || F->getLinkage() != PILLinkage::Public) {
...
static PILFunction *lookupExistingSpecialization(PILModule &M,
   ...
   if (isKnownPrespecialization(FunctionName))
      return M.findFunction(FunctionName, PILLinkage::PublicExternal);

   return nullptr;
}

PILFunction *polar::lookupPrespecializedSymbol(PILModule &M,
                                               StringRef FunctionName) {
   // First check if the module contains a required specialization already.
   auto *Specialization = M.lookUpFunction(FunctionName);
   if (Specialization) {
      if (Specialization->getLinkage() == PILLinkage::PublicExternal)
   ...
   Specialization->setLinkage(PILLinkage::PublicExternal);
   ...
   LLVM_DEBUG(llvm::dbgs() << "Found existing specialization for: "
                           << FunctionName << '\n';
                 llvm::dbgs() << polar::demangling::demangleSymbolAsString(

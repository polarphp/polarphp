...
void ConstantTracker::trackInst(PILInstruction *inst) {
   if (auto *LI = dyn_cast<LoadInst>(inst)) {
      PILValue baseAddr = scanProjections(LI->getOperand());
      if (PILInstruction *loadLink = getMemoryContent(baseAddr))
         links[LI] = loadLink;
   } else if (auto *SI = dyn_cast<StoreInst>(inst)) {
      PILValue baseAddr = scanProjections(SI->getOperand(1));
      memoryContent[baseAddr] = SI;
   } else if (auto *CAI = dyn_cast<CopyAddrInst>(inst)) {
      if (!CAI->isTakeOfSrc()) {
         // Treat a copy_addr as a load + store
         PILValue loadAddr = scanProjections(CAI->getOperand(0));
         if (PILInstruction *loadLink = getMemoryContent(loadAddr)) {
            links[CAI] = loadLink;
            PILValue storeAddr = scanProjections(CAI->getOperand(1));
...
PILValue ConstantTracker::scanProjections(PILValue addr,
   ...
   for (;;) {
      ...
      if (PILValue param = getParam(addr)) {
         // Go to the caller.
         addr = param;
         continue;
      }
...
PILValue ConstantTracker::getStoredValue(PILInstruction *loadInst,
                                         ProjectionPath &projStack) {
   PILInstruction *store = links[loadInst];
   if (!store && callerTracker)
      store = callerTracker->links[loadInst];
   if (!store) return PILValue();
...
static PILValue getMember(PILInstruction *inst, ProjectionPath &projStack) {
   if (!projStack.empty()) {
...
PILInstruction *ConstantTracker::getDef(PILValue val,
                                        ProjectionPath &projStack) {
   ...
   for (;;) {
      if (auto *inst = dyn_cast<SingleValueInstruction>(val)) {
         if (auto pi = Projection::isObjectProjection(val)) {
            ...
            val = pi->getOperand(0);
            continue;
         } else if (PILValue member = getMember(inst, projStack)) {
            // The opposite of a projection instruction: composing a struct/tuple.
            projStack.pop_back();
            val = member;
            continue;
         } else if (PILValue loadedVal = getStoredValue(inst, projStack)) {
         ...
         return inst;
      } else if (PILValue param = getParam(val)) {
...
ConstantTracker::IntConst ConstantTracker::getIntConst(PILValue val, int depth) {

   // Don't spend too much time with constant evaluation.
   if (depth >= 10)
      return IntConst();

   PILInstruction *I = getDef(val);
   if (!I)
      return IntConst();
...
// out to be constant.
PILBasicBlock *ConstantTracker::getTakenBlock(TermInst *term) {
   ...
   if (auto *SEI = dyn_cast<SwitchEnumInst>(term)) {
      if (PILInstruction *def = getDefInCaller(SEI->getOperand())) {
   ...
   if (auto *CCB = dyn_cast<CheckedCastBranchInst>(term)) {
      if (PILInstruction *def = getDefInCaller(CCB->getOperand())) {
         if (auto *UCI = dyn_cast<UpcastInst>(def)) {
            PILType castType = UCI->getOperand()->getType();
...
int ShortestPathAnalysis::getEntryDistFromPreds(const PILBasicBlock *BB,
                                                int LoopDepth) {
...
int ShortestPathAnalysis::getExitDistFromSuccs(const PILBasicBlock *BB,
                                               int LoopDepth) {
   int MinDist = InitialDist;
   for (const PILSuccessor &Succ : BB->getSuccessors()) {
...
static PILBasicBlock *detectLoopBypassPreheader(PILLoop *Loop) {
   PILBasicBlock *Pred = Loop->getLoopPreheader();
   if (!Pred)
      return nullptr;
   ...
   PILBasicBlock *Succ = (CBR->getTrueBB() == Pred ? CBR->getFalseBB() :
                          CBR->getTrueBB());

   for (PILBasicBlock *PredOfSucc : Succ->getPredecessorBlocks()) {
      PILBasicBlock *Exiting = PredOfSucc->getSinglePredecessorBlock();
      if (!Exiting)
         Exiting = PredOfSucc;
...
void ShortestPathAnalysis::analyzeLoopsRecursively(PILLoop *Loop, int LoopDepth) {
   if (LoopDepth >= MaxNumLoopLevels)
      return;

   // First dive into the inner loops.
   for (PILLoop *SubLoop : Loop->getSubLoops()) {
   ...
   if (PILBasicBlock *Bypass = detectLoopBypassPreheader(Loop))
...
getWeight(PILBasicBlock *BB, Weight CallerWeight) {
...
void ShortestPathAnalysis::printLoop(llvm::raw_ostream &OS, PILLoop *Loop,
                                     int LoopDepth) {
   if (LoopDepth >= MaxNumLoopLevels)
      return;
   ...
   for (PILBasicBlock *BB : Loop->getBlocks()) {
...
void ShortestPathAnalysis::printBlockInfo(llvm::raw_ostream &OS,
                                          PILBasicBlock *BB, int LoopDepth) {
...
static bool calleeIsSelfRecursive(PILFunction *Callee) {
   for (auto &BB : *Callee)
      for (auto &I : BB)
...
PILFunction *polar::getEligibleFunction(FullApplySite AI,
   ...
   PILFunction *Caller = AI.getFunction();
   ...
   if (mayBindDynamicSelf(Callee)) {
      ...
      if (AI.hasSelfArgument() && Caller->hasSelfMetadataParam()) {
         ...
         if (CalleeSelf != PILValue(CallerSelf))
            return nullptr;
      } else
         return nullptr;
   }
   ...
   // in PIL something like casting a value of generic type T into a value of
...
static bool hasInterestingSideEffect(PILInstruction *I) {
   switch (I->getKind()) {
      ...
      case polar::PILInstructionKind::StrongRetainInst:
      case polar::PILInstructionKind::StrongReleaseInst:
      case polar::PILInstructionKind::RetainValueInst:
      case polar::PILInstructionKind::ReleaseValueInst:
      case polar::PILInstructionKind::StoreInst:
      case polar::PILInstructionKind::DeallocRefInst:
         return false;
      default:
         return I->getMemoryBehavior() != PILInstruction::MemoryBehavior::None;
   }
}
...
static bool isConstantArg(Operand *Arg) {
   ...
   auto addToWorklist = [&](PILInstruction *I) {
   ...
   while (!Worklist.empty()) {
      ...
      for (Operand &Op : I->getAllOperands()) {
         if (PILInstruction *OpInst = Op.get()->getDefiningInstruction()) {
...
bool polar::isPureCall(FullApplySite AI, SideEffectAnalysis *SEA) {
   ...
   auto Args = AI.getArgumentOperands().slice(AI.getNumIndirectPILResults());

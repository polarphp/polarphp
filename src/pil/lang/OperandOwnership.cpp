...
using namespace polar::ownership;
...
class OperandOwnershipKindClassifier
    : public PILInstructionVisitor<OperandOwnershipKindClassifier,
                                   OperandOwnershipKindMap> {
public:
  using Map = OperandOwnershipKindMap;

private:
  LLVM_ATTRIBUTE_UNUSED PILModule &mod;
...
public:
  ...
  /// like struct_extract.
  OperandOwnershipKindClassifier(PILModule &mod, const Operand &op,
                                 ErrorBehaviorKind errorBehavior,
                                 bool checkingSubObject)
  ...
  PILValue getValue() const { return op.get(); }
  ...
  PILType getType() const { return op.get()->getType(); }
  ...
  OperandOwnershipKindMap visitForwardingInst(PILInstruction *i,
  ...
  OperandOwnershipKindMap visitCallee(CanPILFunctionType substCalleeType);
  OperandOwnershipKindMap
  checkTerminatorArgumentMatchesDestBB(PILBasicBlock *destBB, unsigned opIndex);
...
OperandOwnershipKindMap
OperandOwnershipKindClassifier::visitForwardingInst(PILInstruction *i,
                                                    ArrayRef<Operand> ops) {
  assert(i->getNumOperands() && "Expected to have non-zero operands");
...
OperandOwnershipKindClassifier::checkTerminatorArgumentMatchesDestBB(
    PILBasicBlock *destBB, unsigned opIndex) {
...
OperandOwnershipKindClassifier::visitSwitchEnumInst(SwitchEnumInst *sei) {
  ...
  auto mergedKind = ValueOwnershipKind::merge(makeTransformRange(
      ...
      [&](PILPhiArgumentArrayRef array) -> ValueOwnershipKind {
        ...
        PILPhiArgument *arg = array.front();
...
OperandOwnershipKindClassifier::visitReturnInst(ReturnInst *ri) {
  ...
  PILFunctionConventions fnConv = f->getConventions();

  auto results = fnConv.getDirectPILResults();
  if (results.empty())
    return Map();

  auto ownershipKindRange = makeTransformRange(results,
                                               [&](const PILResultInfo &info) {
...
OperandOwnershipKindClassifier::visitEndBorrowInst(EndBorrowInst *i) {
  ...
  if (isCheckingSubObject()) {
    auto *phiArg = cast<PILPhiArgument>(op.get());
    (void)phiArg;
...
// FIXME: Why not use PILArgumentConvention here?
OperandOwnershipKindMap OperandOwnershipKindClassifier::visitCallee(
  ...
  case ParameterConvention::Indirect_In_Constant:
    assert(!PILModuleConventions(mod).isPILIndirect(
                                      PILParameterInfo(substCalleeType, conv)));
    return Map::compatibilityMap(ValueOwnershipKind::Owned,
                                 UseLifetimeConstraint::MustBeInvalidated);
...
// PILFunctionArgument as a phi in a global program graph.
...
OperandOwnershipKindClassifier::visitFullApply(FullApplySite apply) {
  ...
  PILParameterInfo paramInfo = conv.getParamInfoForPILArg(argIndex);
...
struct OperandOwnershipKindBuiltinClassifier
    : PILBuiltinVisitor<OperandOwnershipKindBuiltinClassifier,
                        OperandOwnershipKindMap> {
  using Map = OperandOwnershipKindMap;
...
// Builtins that should be lowered to PIL instructions so we should never see
// them.
#define BUILTIN_PIL_OPERATION(ID, NAME, CATEGORY)                              \
  OperandOwnershipKindMap OperandOwnershipKindBuiltinClassifier::visit##ID(    \
      BuiltinInst *, StringRef) {                                              \
    llvm_unreachable("Builtin should have been lowered to PIL instruction?!"); \
  }
...
Operand::getOwnershipKindMap(bool isForwardingSubValue) const {
  ...
  return classifier.visit(const_cast<PILInstruction *>(getUser()));
}

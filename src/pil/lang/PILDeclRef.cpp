//===--- PILDeclRef.cpp - Implements PILDeclRef ---------------------------===//
//
...
MethodDispatch
polar::getMethodDispatch(AbstractFunctionDecl *method) {
...
bool polar::requiresForeignToNativeThunk(ValueDecl *vd) {
   ...
   // @todo
//   if (auto proto = dyn_cast<InterfaceDecl>(dc))
//      if (proto->isObjC())
//         return true;
...
bool polar::requiresForeignEntryPoint(ValueDecl *vd) {
...
//   }
//
//   if (vd->isObjC() && isa<InterfaceDecl>(vd->getDeclContext()))
//      return true;
...
PILDeclRef::PILDeclRef(ValueDecl *vd, PILDeclRef::Kind kind,
...
PILDeclRef::PILDeclRef(PILDeclRef::Loc baseLoc,
                       bool isCurried, bool asForeign)
   : isCurried(isCurried), isDirectReference(0), defaultArgIndex(0)
{
   if (auto *vd = baseLoc.dyn_cast<ValueDecl*>()) {
      if (auto *fd = dyn_cast<FuncDecl>(vd)) {
         // Map FuncDecls directly to Func PILDeclRefs.
         loc = fd;
         kind = Kind::Func;
      }
         // Map ConstructorDecls to the Allocator PILDeclRef of the constructor.
      else if (auto *cd = dyn_cast<ConstructorDecl>(vd)) {
         loc = cd;
         kind = Kind::Allocator;
      }
         // Map EnumElementDecls to the EnumElement PILDeclRef of the element.
         ...
         llvm_unreachable("must create PILDeclRef for VarDecl with explicit kind");
      }
         ...
         llvm_unreachable("invalid loc decl for PILDeclRef!");
      }
   } else if (auto *ACE = baseLoc.dyn_cast<AbstractClosureExpr *>()) {
      loc = ACE;
      kind = Kind::Func;
   } else {
      llvm_unreachable("impossible PILDeclRef loc");
   }

   isForeign = asForeign;
}

Optional<AnyFunctionRef> PILDeclRef::getAnyFunctionRef() const {
...
bool PILDeclRef::isThunk() const {
   return isCurried || isForeignToNativeThunk() || isNativeToForeignThunk();
}

bool PILDeclRef::isClangImported() const {
   if (!hasDecl())
      return false;
...
bool PILDeclRef::isClangGenerated() const {
   if (!hasDecl())
      return false;
...
bool PILDeclRef::isClangGenerated(ClangNode node) {
...
bool PILDeclRef::isImplicit() const {
   if (hasDecl())
      return getDecl()->isImplicit();
   return getAbstractClosureExpr()->isImplicit();
}

PILLinkage PILDeclRef::getLinkage(ForDefinition_t forDefinition) const {
   if (getAbstractClosureExpr()) {
      return isSerialized() ? PILLinkage::Shared : PILLinkage::Private;
   }

   // Add External to the linkage (e.g. Public -> PublicExternal) if this is a
   // declaration not a definition.
   auto maybeAddExternal = [&](PILLinkage linkage) {
   ...
   if (kind == PILDeclRef::Kind::DefaultArgGenerator) {
      if (isSerialized())
         return maybeAddExternal(PILLinkage::PublicNonABI);
   }
   ...
   // properties.
   if (kind == PILDeclRef::Kind::PropertyWrapperBackingInitializer) {
   ...
   switch (effectiveAccess) {
      ...
      case AccessLevel::Open:
         ...
         if (limit == Limit::AlwaysEmitIntoClient)
            return maybeAddExternal(PILLinkage::PublicNonABI);
         return maybeAddExternal(PILLinkage::Public);
   }
   llvm_unreachable("unhandled access");
}

PILDeclRef PILDeclRef::getDefaultArgGenerator(Loc loc,
...
IsSerialized_t PILDeclRef::isSerialized() const {
   DeclContext *dc;
   ...
   if (kind == PILDeclRef::Kind::Allocator) {
...
bool PILDeclRef::isNoinline() const {
   if (!hasDecl())
      return false;
...
bool PILDeclRef::isAlwaysInline() const {
   if (!hasDecl())
      return false;
...
bool PILDeclRef::hasEffectsAttribute() const {
   if (!hasDecl())
      return false;
   return getDecl()->getAttrs().hasAttribute<EffectsAttr>();
}

EffectsKind PILDeclRef::getEffectsAttribute() const {
   assert(hasEffectsAttribute());
   EffectsAttr *MA = getDecl()->getAttrs().getAttribute<EffectsAttr>();
   return MA->getKind();
}

bool PILDeclRef::isForeignToNativeThunk() const {
...
bool PILDeclRef::isNativeToForeignThunk() const {
...
std::string PILDeclRef::mangle(ManglingKind MKind) const {
   ...
   switch (MKind) {
      case PILDeclRef::ManglingKind::Default:
         if (isForeign) {
         ...
         break;
      case PILDeclRef::ManglingKind::DynamicThunk:
   ...
   switch (kind) {
      case PILDeclRef::Kind::Func:
         ...
         // Use the PILGen name only for the original non-thunked, non-curried entry
         // point.
         if (auto NameA = getDecl()->getAttrs().getAttribute<PILGenNameAttr>())
            if (!NameA->Name.empty() &&
...
bool PILDeclRef::requiresNewVTableEntry() const {
   if (cast<AbstractFunctionDecl>(getDecl())->needsNewVTableEntry())
      return true;
   return false;
}

bool PILDeclRef::requiresNewWitnessTableEntry() const {
   return requiresNewWitnessTableEntry(cast<AbstractFunctionDecl>(getDecl()));
}

bool PILDeclRef::requiresNewWitnessTableEntry(AbstractFunctionDecl *func) {
   return func->getOverriddenDecls().empty();
}

PILDeclRef PILDeclRef::getOverridden() const {
   if (!hasDecl())
      return PILDeclRef();
   ...
   return PILDeclRef(overridden, kind, isCurried);
}

PILDeclRef PILDeclRef::getNextOverriddenVTableEntry() const {
   if (auto overridden = getOverridden()) {
      ...
      // purposes.
      if (overridden.kind == PILDeclRef::Kind::Allocator) {
      ...
      if (overridden.kind == PILDeclRef::Kind::Initializer) {
         return PILDeclRef();
      }
...
PILDeclRef PILDeclRef::getOverriddenWitnessTableEntry() const {
   auto bestOverridden =
      getOverriddenWitnessTableEntry(cast<AbstractFunctionDecl>(getDecl()));
   return PILDeclRef(bestOverridden, kind, isCurried);
}

AbstractFunctionDecl *PILDeclRef::getOverriddenWitnessTableEntry(
   AbstractFunctionDecl *func) {
   if (!isa<InterfaceDecl>(func->getDeclContext()))
      return func;
   ...
   while (!stack.empty()) {
      ...
      if (overriddenDecls.empty()) {
         ...
         if (!bestOverridden ||
             InterfaceDecl::compare(
                cast<InterfaceDecl>(current->getDeclContext()),
                cast<InterfaceDecl>(bestOverridden->getDeclContext()))
             < 0) {
...
PILDeclRef PILDeclRef::getOverriddenVTableEntry() const {
   PILDeclRef cur = *this, next = *this;
   do {
      cur = next;
...
PILLocation PILDeclRef::getAsRegularLocation() const {
   if (hasDecl())
      return RegularLocation(getDecl());
   return RegularLocation(getAbstractClosureExpr());
}

SubclassScope PILDeclRef::getSubclassScope() const {
   if (!hasDecl())
   ...
   if (auto *CD = dyn_cast<ConstructorDecl>(decl)) {
      ...
      if (kind == PILDeclRef::Kind::Initializer)
         return SubclassScope::NotApplicable;
...
unsigned PILDeclRef::getParameterListCount() const {
   ...
   if (isa<AbstractFunctionDecl>(vd) || isa<EnumElementDecl>(vd)) {
      ...
      llvm_unreachable("Unhandled ValueDecl for PILDeclRef");
   }
}
...
bool PILDeclRef::canBeDynamicReplacement() const {
   if (kind == PILDeclRef::Kind::Destroyer ||
       kind == PILDeclRef::Kind::DefaultArgGenerator)
      return false;
   if (kind == PILDeclRef::Kind::Initializer)
...
bool PILDeclRef::isDynamicallyReplaceable() const {
   if (kind == PILDeclRef::Kind::DefaultArgGenerator)
      return false;
   if (isStoredPropertyInitializer())
      return false;

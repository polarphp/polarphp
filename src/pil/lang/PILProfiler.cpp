//===--- PILProfiler.cpp - Instrumentation based profiling ----------------===//
//
...
DeclContext *getProfilerContextForDecl(AstNode N, PILDeclRef forDecl) {
   if (auto *D = N.dyn_cast<Decl *>())
      if (auto *TLCD = dyn_cast<TopLevelCodeDecl>(D))
         return TLCD;
   assert(!forDecl.isNull() && "Expected a nonnull PILDeclRef");
...
static bool canCreateProfilerForAST(AstNode N, PILDeclRef forDecl) {
...
PILProfiler *PILProfiler::create(PILModule &M, ForDefinition_t forDefinition,
   ...
   auto *Buf = M.allocate<PILProfiler>(1);
   auto *SP =
      ::new (Buf) PILProfiler(M, N, forDecl, Opts.EmitProfileCoverageMapping);
...
struct MapRegionCounters : public AstWalker {
   ...
   std::pair<bool, Expr *> walkToExprPre(Expr *E) override {
      ...
      // missing coverage. PILGen treats the init expr as part of the getter, but
      // its PILProfiler has no information about the init because the LIE isn't
      // visited here.
...
public:
   ...
   /// source regions.
   PILCoverageMap *emitSourceRegions(
      PILModule &M, StringRef Name, StringRef PGOFuncName, uint64_t Hash,
      ...
      return PILCoverageMap::create(M, Filename, Name, PGOFuncName, Hash, Regions,
...
static StringRef getCurrentFileName(AstNode N, PILDeclRef forDecl) {
...
void PILProfiler::assignRegionCounters() {
...
ProfileCounter PILProfiler::getExecutionCount(AstNode Node) {
...
Optional<AstNode> PILProfiler::getPGOParent(AstNode Node) {
   if (!Node || !M.getPGOReader() || !hasRegionCounters()) {
      return None;
   }

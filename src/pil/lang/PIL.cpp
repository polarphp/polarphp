//===--- PIL.cpp - Implements random PIL functionality --------------------===//
//
...
PILLinkage polar::getPILLinkage(FormalLinkage linkage,
                                ForDefinition_t forDefinition) {
   switch (linkage) {
      case FormalLinkage::PublicUnique:
         return (forDefinition ? PILLinkage::Public : PILLinkage::PublicExternal);

      case FormalLinkage::PublicNonUnique:
         // FIXME: any place we have to do this that actually requires
         // uniqueness is buggy.
         return (forDefinition ? PILLinkage::Shared : PILLinkage::PublicExternal);

      case FormalLinkage::HiddenUnique:
         return (forDefinition ? PILLinkage::Hidden : PILLinkage::HiddenExternal);
...
PILLinkage
polar::getLinkageForInterfaceConformance(const RootInterfaceConformance *C,
   ...
   AccessLevel access = std::min(C->getInterface()->getEffectiveAccess(),
   ...
   switch (access) {
      ...
      default:
         return (definition ? PILLinkage::Public : PILLinkage::PublicExternal);
   }
}

bool PILModule::isTypeMetadataAccessible(CanType type) {
   // PILModules built for the debugger have special powers to access metadata
...
static bool isTypeMetadataForLayoutAccessible(PILModule &M, PILType type) {
...
bool PILModule::isTypeABIAccessible(PILType type,
   ...
   assert(!type.is<ReferenceStorageType>() &&
          !type.is<PILFunctionType>() &&
          !type.is<AnyMetatypeType>() &&
          "unexpected PIL lowered-only type with non-fixed layout");
...
bool AbstractStorageDecl::exportsPropertyDescriptor() const {
   ...
   // Interface requirements do not need property descriptors.
   if (isa<InterfaceDecl>(getDeclContext()))
      return false;
   ...
   auto getterLinkage = PILDeclRef(getter).getLinkage(ForDefinition);

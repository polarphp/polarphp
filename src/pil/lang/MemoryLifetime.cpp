...
#define DEBUG_TYPE "pil-memory-lifetime"
...
void polar::printBitsAsArray(llvm::raw_ostream &OS, const SmallBitVector &bits) {
...
void polar::dumpBits(const SmallBitVector &bits) {
   llvm::dbgs() << bits << '\n';
}
...
static bool shouldTrackLocation(PILType ty, PILFunction *function) {
...
MemoryLocations::Location::Location(PILValue val, unsigned index, int parentIdx) :
...
void MemoryLocations::Location::updateFieldCounters(PILType ty, int increment) {
   PILFunction *function = representativeValue->getFunction();
...
static PILValue getBaseValue(PILValue addr) {
   while (true) {
      switch (addr->getKind()) {
...
int MemoryLocations::getLocationIdx(PILValue addr) const {
...
void MemoryLocations::analyzeLocations(PILFunction *function) {
   ...
   for (PILArgument *arg : function->getArguments()) {
      PILFunctionArgument *funcArg = cast<PILFunctionArgument>(arg);
...
void MemoryLocations::analyzeLocation(PILValue loc) {
   PILFunction *function = loc->getFunction();
   assert(function && "cannot analyze a PILValue which is not in a function");
   ...
   SmallVector<PILValue, 8> collectedVals;
...
void MemoryLocations::handleSingleBlockLocations(
   std::function<void (PILBasicBlock *block)> handlerFunc) {
...
bool MemoryLocations::analyzeLocationUsesRecursively(PILValue V, unsigned locIdx,
                                                     SmallVectorImpl<PILValue> &collectedVals,
                                                     SubLocationMap &subLocationMap) {
...
bool MemoryLocations::analyzeAddrProjection(
   ...
   SmallVectorImpl<PILValue> &collectedVals, SubLocationMap &subLocationMap) {
...
void MemoryLocations::initFieldsCounter(Location &loc) {
   ...
   if (StructDecl *decl = ty.getStructOrBoundGenericStruct()) {
      ...
      PILModule &module = function->getModule();
...
MemoryDataflow::MemoryDataflow(PILFunction *function, unsigned numLocations) {
...
void MemoryDataflow::entryReachabilityAnalysis() {
   ...
   while (!workList.empty()) {
      ...
      for (PILBasicBlock *succ : state->block->getSuccessorBlocks()) {
...
void MemoryDataflow::exitReachableAnalysis() {
   ...
   while (!workList.empty()) {
      BlockState *state = workList.pop_back_val();
      for (PILBasicBlock *pred : state->block->getPredecessorBlocks()) {
...
void MemoryDataflow::solveForward(JoinOperation join) {
   ...
   do {
      ...
      for (BlockState &st : blockStates) {
         ...
         assert(!bits.empty());
         for (PILBasicBlock *pred : st.block->getPredecessorBlocks()) {
...
void MemoryDataflow::solveBackward(JoinOperation join) {
   ...
   do {
      ...
      for (BlockState &st : llvm::reverse(blockStates)) {
         ...
         assert(!bits.empty());
         for (PILBasicBlock *succ : st.block->getSuccessorBlocks()) {
...
class MemoryLifetimeVerifier {
   ...
   void setBitsOfPredecessor(Bits &bits, PILBasicBlock *block);
   ...
   void checkBlock(PILBasicBlock *block, Bits &bits);
   ...
   void checkFuncArgument(Bits &bits, Operand &argumentOp,
                          PILArgumentConvention argumentConvention,
                          PILInstruction *applyInst);

public:
   MemoryLifetimeVerifier(PILFunction *function) : function(function) {}
...
void MemoryLifetimeVerifier::require(bool condition, const Twine &complaint,
   ...
   llvm::errs() << "PIL memory lifetime failure in @" << function->getName()
                << ": " << complaint << '\n';
   if (locationIdx >= 0) {
...
void MemoryLifetimeVerifier::require(const Bits &wrongBits,
                                     const Twine &complaint, PILInstruction *where) {
   require(wrongBits.none(), complaint, wrongBits.find_first(), where);
}

void MemoryLifetimeVerifier::requireBitsClear(const Bits &bits, PILValue addr,
                                              PILInstruction *where) {
...
void MemoryLifetimeVerifier::requireBitsSet(const Bits &bits, PILValue addr,
                                            PILInstruction *where) {
   if (auto *loc = locations.getLocation(addr)) {
      require(~bits & loc->subLocations,
      ...
      if (st.block == function->getEntryBlock()) {
         ...
         for (PILArgument *arg : function->getArguments()) {
            ...
            if (funcArg->getArgumentConvention() !=
                PILArgumentConvention::Indirect_Out) {
...
void MemoryLifetimeVerifier::initDataflowInBlock(BlockState &state) {
   ...
   for (PILInstruction &I : *state.block) {
      switch (I.getKind()) {
         case PILInstructionKind::LoadInst: {
...
void MemoryLifetimeVerifier::setBitsOfPredecessor(Bits &bits,
                                                  PILBasicBlock *block) {
   PILBasicBlock *pred = block->getSinglePredecessorBlock();
   if (!pred)
      return;
   ...
   for (Operand &op : TAI->getAllOperands()) {
      if (FAS.isArgumentOperand(op) &&
          FAS.getArgumentConvention(op) == PILArgumentConvention::Indirect_Out) {
...
void MemoryLifetimeVerifier::checkBlock(PILBasicBlock *block, Bits &bits) {
   ...
   for (PILInstruction &I : *block) {
      switch (I.getKind()) {
         case PILInstructionKind::LoadInst: {
            ...
            switch (LI->getOwnershipQualifier()) {
               ...
               case LoadOwnershipQualifier::Unqualified:
                  llvm_unreachable("unqualified load shouldn't be in ownership PIL");
            }
            break;
         }
         case PILInstructionKind::StoreInst: {
            ...
            switch (SI->getOwnershipQualifier()) {
               ...
               case StoreOwnershipQualifier::Unqualified:
                  llvm_unreachable("unqualified store shouldn't be in ownership PIL");
            }
            break;
         }
         case PILInstructionKind::CopyAddrInst: {
         ...
         case PILInstructionKind::DebugValueAddrInst:
            ...
            break;
         case PILInstructionKind::DeallocStackInst: {
            PILValue opVal = cast<DeallocStackInst>(&I)->getOperand();
...
void MemoryLifetimeVerifier::verify() {
   ...
   locations.handleSingleBlockLocations([this](PILBasicBlock *block) {
      Bits bits(locations.getNumLocations());
      checkBlock(block, bits);
   });
}

void polar::verifyMemoryLifetime(PILFunction *function) {

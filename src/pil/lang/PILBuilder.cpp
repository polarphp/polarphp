//===--- PILBuilder.cpp - Class for creating PIL Constructs ---------------===//
//
...
//===----------------------------------------------------------------------===//
// PILBuilder Implementation
//===----------------------------------------------------------------------===//

PILBuilder::PILBuilder(PILGlobalVariable *GlobVar,
                       SmallVectorImpl<PILInstruction *> *InsertedInstrs)
   : TempContext(GlobVar->getModule(), InsertedInstrs), C(TempContext),
     F(nullptr) {
   setInsertionPoint(&GlobVar->StaticInitializerBlock);
}

IntegerLiteralInst *PILBuilder::createIntegerLiteral(IntegerLiteralExpr *E) {
   return insert(IntegerLiteralInst::create(E, getPILDebugLocation(E),
                                            getModule()));
}

FloatLiteralInst *PILBuilder::createFloatLiteral(FloatLiteralExpr *E) {
   return insert(FloatLiteralInst::create(E, getPILDebugLocation(E),
                                          getModule()));
}

TupleInst *PILBuilder::createTuple(PILLocation loc, ArrayRef<PILValue> elts) {
   // Derive the tuple type from the elements.
   SmallVector<TupleTypeElt, 4> eltTypes;
   for (auto elt : elts)
      eltTypes.push_back(elt->getType().getAstType());
   auto tupleType = PILType::getPrimitiveObjectType(
      CanType(TupleType::get(eltTypes, getAstContext())));

   return createTuple(loc, tupleType, elts);
}

PILType PILBuilder::getPartialApplyResultType(
   TypeExpansionContext context, PILType origTy, unsigned argCount,
   PILModule &M, SubstitutionMap subs, ParameterConvention calleeConvention,
   PartialApplyInst::OnStackKind onStack) {
   CanPILFunctionType FTI = origTy.castTo<PILFunctionType>();
   if (!subs.empty())
   ...
   for (auto &result : results) {
      if (result.getConvention() == ResultConvention::UnownedInnerPointer)
         result = PILResultInfo(result.getReturnValueType(M, FTI),
   ...
   auto appliedFnType = PILFunctionType::get(nullptr, extInfo,
   ...
   return PILType::getPrimitiveObjectType(appliedFnType);
}

ProjectBoxInst *PILBuilder::createProjectBox(PILLocation Loc,
                                             PILValue boxOperand,
                                             unsigned index) {
   auto boxTy = boxOperand->getType().castTo<PILBoxType>();
   auto fieldTy = getPILBoxFieldType(getTypeExpansionContext(), boxTy,
                                     getModule().Types, index);

   return insert(new (getModule()) ProjectBoxInst(
      getPILDebugLocation(Loc), boxOperand, index, fieldTy));
}

// If legal, create an unchecked_ref_cast from the given operand and result
// type, otherwise return null.
SingleValueInstruction *
PILBuilder::tryCreateUncheckedRefCast(PILLocation Loc, PILValue Op,
                                      PILType ResultTy) {
   if (!PILType::canRefCast(Op->getType(), ResultTy, getModule()))
      return nullptr;

   return insert(UncheckedRefCastInst::create(getPILDebugLocation(Loc), Op,
                                              ResultTy, getFunction(),
                                              C.OpenedArchetypes));
}

ClassifyBridgeObjectInst *
PILBuilder::createClassifyBridgeObject(PILLocation Loc, PILValue value) {
   auto &ctx = getAstContext();
   Type int1Ty = BuiltinIntegerType::get(1, ctx);
   Type resultTy = TupleType::get({ int1Ty, int1Ty }, ctx);
   auto ty = PILType::getPrimitiveObjectType(resultTy->getCanonicalType());
   return insert(new (getModule())
                    ClassifyBridgeObjectInst(getPILDebugLocation(Loc), value, ty));
}


// Create the appropriate cast instruction based on result type.
SingleValueInstruction *
PILBuilder::createUncheckedBitCast(PILLocation Loc, PILValue Op, PILType Ty) {
...
BranchInst *PILBuilder::createBranch(PILLocation Loc,
                                     PILBasicBlock *TargetBlock,
                                     OperandValueArrayRef Args) {
   SmallVector<PILValue, 6> ArgsCopy;
...
void PILBuilder::emitBlock(PILBasicBlock *BB, PILLocation BranchLoc) {
...
/// continuation block.
PILBasicBlock *PILBuilder::splitBlockForFallthrough() {
...
static bool setAccessToDeinit(BeginAccessInst *beginAccess) {
   ...
   // a common analysis.
   if (beginAccess->getEnforcement() == PILAccessEnforcement::Dynamic)
      return false;

   beginAccess->setAccessKind(PILAccessKind::Deinit);
   return true;
}

PointerUnion<CopyAddrInst *, DestroyAddrInst *>
PILBuilder::emitDestroyAddr(PILLocation Loc, PILValue Operand) {
...
PILBuilder::emitStrongRelease(PILLocation Loc, PILValue Operand) {
...
PILBuilder::emitReleaseValue(PILLocation Loc, PILValue Operand) {
   // Check to see if the instruction immediately before the insertion point is a
...
PILBuilder::emitDestroyValue(PILLocation Loc, PILValue Operand) {
   // Check to see if the instruction immediately before the insertion point is a
   // retain_value of the specified operand.  If so, we can zap the pair.
   auto I = getInsertionPoint(), BBStart = getInsertionBB()->begin();
   while (I != BBStart) {
...
void PILBuilder::addOpenedArchetypeOperands(PILInstruction *I) {
   ...
   SmallPtrSet<PILInstruction *, 8> Visited;
...
ValueMetatypeInst *PILBuilder::createValueMetatype(PILLocation Loc,
                                                   PILType MetatypeTy,
                                                   PILValue Base) {
   assert(Base->getType().isLoweringOf(
      ...
      MetatypeTy.castTo<MetatypeType>().getInstanceType()) &&
          ...
          "type");
   return insert(new (getModule()) ValueMetatypeInst(getPILDebugLocation(Loc),
                                                     MetatypeTy, Base));
}

// TODO: This should really be an operation on type lowering.
void PILBuilder::emitDestructureValueOperation(
   PILLocation loc, PILValue v, SmallVectorImpl<PILValue> &results) {
   ...
   // In non qualified ownership PIL, drop back to using projection code.
...
void PILBuilder::emitDestructureAddressOperation(
   PILLocation loc, PILValue v, SmallVectorImpl<PILValue> &results) {
...
void PILBuilder::emitDestructureValueOperation(
   ...
   SmallVector<PILValue, 8> results;
...
DebugValueInst *PILBuilder::createDebugValue(PILLocation Loc, PILValue src,
   ...
   return insert(
      DebugValueInst::create(getPILDebugLocation(Loc), src, getModule(), Var));
}

DebugValueAddrInst *PILBuilder::createDebugValueAddr(PILLocation Loc,
                                                     PILValue src,
                                                     PILDebugVariable Var) {
   // Debug location overrides cannot apply to debug addr instructions.
   DebugLocOverrideRAII LocOverride{*this, None};
   return insert(DebugValueAddrInst::create(getPILDebugLocation(Loc), src,
                                            getModule(), Var));
}

void PILBuilder::emitScopedBorrowOperation(PILLocation loc, PILValue original,
...
CheckedCastBranchInst *PILBuilder::createCheckedCastBranch(
   PILLocation Loc, bool isExact, PILValue op,
   ...
   return insertTerminator(CheckedCastBranchInst::create(
      getPILDebugLocation(Loc), isExact, op,

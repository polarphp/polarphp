...
/// \file
///
/// This file defines a linear lifetime checker for PIL. A value with a linear
...
#define DEBUG_TYPE "pil-linear-lifetime-checker"
...
using BrPropUserAndBlockPair = std::pair<BranchPropagatedUser, PILBasicBlock *>;
...
struct State {
  ...
  SmallVectorImpl<PILBasicBlock *> *leakingBlocks;

  /// The set of blocks with consuming uses.
  SmallPtrSet<PILBasicBlock *, 8> blocksWithConsumingUses;

  /// The set of blocks with non-consuming uses and the associated
  /// non-consuming use PILInstruction.
  llvm::SmallDenseMap<PILBasicBlock *, BranchPropagatedUser, 8>
  ...
  /// terminates.
  SmallSetVector<PILBasicBlock *, 8> successorBlocksThatMustBeVisited;

  State(PILValue value, SmallPtrSetImpl<PILBasicBlock *> &visitedBlocks,
  ...
  State(PILBasicBlock *beginBlock,
        SmallPtrSetImpl<PILBasicBlock *> &visitedBlocks,
        ErrorBehaviorKind errorBehavior,
        SmallVectorImpl<PILBasicBlock *> *leakingBlocks)
  ...
  void checkPredsForDoubleConsume(BranchPropagatedUser consumingUser,
                                  PILBasicBlock *userBlock);
  void checkPredsForDoubleConsume(PILBasicBlock *userBlock);
...
void State::initializeAllNonConsumingUses(
  ...
  for (BranchPropagatedUser user : nonConsumingUsers) {
    ...
    if (std::find_if(result.first->second.getIterator(), userBlock->end(),
                     [&user](const PILInstruction &i) -> bool {
                       return user == &i;
                     }) == userBlock->end()) {
      continue;
    }
...
void State::initializeAllConsumingUses(
  ...
  for (BranchPropagatedUser user : consumingUses) {
    PILBasicBlock *userBlock = user.getParent();
...
void State::checkForSameBlockUseAfterFree(BranchPropagatedUser consumingUser,
  ...
  if (std::find_if(consumingUser.getIterator(), userBlock->end(),
                   [&nonConsumingUser](const PILInstruction &i) -> bool {
...
void State::checkPredsForDoubleConsume(PILBasicBlock *userBlock) {
  if (!blocksWithConsumingUses.count(userBlock))
    return;
...
void State::performDataflow(DeadEndBlocks &deBlocks) {
  ...
  while (!worklist.empty()) {
    ...
    PILBasicBlock *block = worklist.pop_back_val();
    ...
    // PILGen before NoReturn folding has run.
...
LinearLifetimeError LinearLifetimeChecker::checkValue(
    PILValue value, ArrayRef<BranchPropagatedUser> consumingUses,
  ...
  for (auto pair : predsToAddToWorklist) {
    ...
    PILBasicBlock *predBlock = pair.second;

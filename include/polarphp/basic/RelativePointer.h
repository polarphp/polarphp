...
#ifndef POLARPHP_BASIC_RELATIVE_POINTER_H
#define POLARPHP_BASIC_RELATIVE_POINTER_H
...
static inline uintptr_t applyRelativeOffset(BasePtrTy *basePtr, Offset offset)
{
   ...
   auto extendOffset = static_cast<uintptr_t>(static_cast<intptr_t>(offset));
...
static inline Offset measureRelativeOffset(A *referent, B *base)
{
   ...
   auto distance = static_cast<uintptr_t>(referent) - static_cast<uintptr_t>(base);
   // Truncate as unsigned, then wrap around to signed.
   auto truncatedDistance =
         static_cast<Offset>(static_cast<typename std::make_unsigned<Offset>::type>(distance));
   // Assert that the truncation didn't discard any non-sign-extended bits.
   assert(static_cast<intptr_t>(truncatedDistance) == static_cast<intptr_t>(distance)
...
public:
   ...
   {
      // Check for null.
      if (Nullable && m_relativeOffset == 0) {
         return nullptr;
      }
      uintptr_t address = internal::applyRelativeOffset(this, m_relativeOffset);
...
public:
   ...
   RelativeIndirectablePointer(ValueTy *absolute)
      : RelativeOffsetPlusIndirect(
           ...
           ? 0
           : internal::measureRelativeOffset<Offset>(absolute, this))
   {
      if (!Nullable) {
   ...
   const ValueTy *get() const &
   {
      ...
      uintptr_t address = internal::applyRelativeOffset(this,
...
private:
   ...
   /// If the low bit is clear, this is a direct reference; otherwise, it is
   /// an indirect reference.
   Offset m_relativeOffsetPlusIndirectAndInt;
   ...
   {
      return (alignof(Offset) - 1) & ~ static_cast<Offset>(0x01);
   }
...
public:
   const ValueTy *getPointer() const &
   {
      ...
      Offset offset = (m_relativeOffsetPlusIndirectAndInt & ~getIntMask());
   ...
   IntTy getInt() const &
   {
      return IntTy((m_relativeOffsetPlusIndirectAndInt & getIntMask()) >> 1);
   }
};
...
public:
   ...
   RelativeDirectPointerImpl(PointerTy absolute)
      : m_relativeOffset(Nullable && absolute == nullptr
                         ? 0
                         : internal::measureRelativeOffset<Offset>(absolute, this))
   {
      if (!Nullable) {
   ...
   PointerTy get() const &
   {
      ...
      uintptr_t absolute = internal::applyRelativeOffset(this, m_relativeOffset);
...
{
   Offset m_relativeOffsetPlusInt;
...
public:
   ...
   {
      Offset offset = (m_relativeOffsetPlusInt & ~getMask());
      ...
      // The value is addressed relative to `this`.
      uintptr_t absolute = internal::applyRelativeOffset(this, offset);
      return reinterpret_cast<PointerTy>(absolute);
   }

   IntTy getInt() const &
   {
      return IntTy(m_relativeOffsetPlusInt & getMask());
   }

   Offset getOpaqueValue() const &
   {
      return m_relativeOffsetPlusInt;
   }
};

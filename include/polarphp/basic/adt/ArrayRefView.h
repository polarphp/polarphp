...
#ifndef POLARPHP_BASIC_ADT_ARRAY_REF_VIEW_H
#define POLARPHP_BASIC_ADT_ARRAY_REF_VIEW_H
...
class ArrayRefView
{
   ArrayRef<Orig> m_array;
public:
   ArrayRefView()
   {}

   ArrayRefView(ArrayRef<Orig> array)
      : m_array(array)
   {}
   ...
   ArrayRef<Orig> getOriginalArray() const
   {
   ...
   friend bool operator==(ArrayRef<Projected> lhs, ArrayRefView rhs)
   {
      if (lhs.size() != rhs.size()) {
         return false;
      }
   ...
   friend bool operator==(ArrayRefView lhs, ArrayRef<Projected> rhs)
   {
      if (lhs.size() != rhs.size()) {
         return false;
      }
...
inline Projected *array_ref_view_cast_helper(const Orig &value) {
   using polar::utils::cast_or_null;
...
#endif // POLARPHP_BASIC_ADT_ARRAY_REF_VIEW_H

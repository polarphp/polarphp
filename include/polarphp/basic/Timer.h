...
#ifndef POLARPHP_BASIC_TIMER_H
#define POLARPHP_BASIC_TIMER_H
...
/// subsystem in include/polarphp/basic/Statistic.h. In addition to not
...
public:
   ...
   {
      if (sm_compilationTimersEnabled == State::Enabled) {
         m_timer.emplace(name, name, "polarphp", "polarphp compilation");
      } else {
         sm_compilationTimersEnabled = State::Skipped;
      }
   }
   ...
   {
      assert(sm_compilationTimersEnabled != State::Skipped &&
            "a timer has already been created");
      sm_compilationTimersEnabled = State::Enabled;
   }

private:
   static State sm_compilationTimersEnabled;

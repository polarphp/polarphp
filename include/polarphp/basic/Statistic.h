...
#ifndef POLARPHP_BASIC_STATISTIC_H
#define POLARPHP_BASIC_STATISTIC_H
...
//    - Calls polar::enableCompilationTimers()
...
private:
   ...
   llvm::TimeRecord m_startedTime;
   std::thread::id m_mainThreadID;

   // This is unique_ptr because NamedRegionTimer is non-copy-constructable.
   std::unique_ptr<llvm::NamedRegionTimer> m_timer;

   SourceManager *m_sourceMgr;
   clang::SourceManager *m_clangSourceMgr;
   Optional<AlwaysOnDriverCounters> m_driverCounters;
   Optional<AlwaysOnFrontendCounters> m_frontendCounters;
   Optional<AlwaysOnFrontendCounters> m_lastTracedFrontendCounters;
   Optional<std::vector<FrontendStatsEvent>> m_frontendStatsEvents;
...
StringRef, const T *>::value,
FrontendStatsTracer>::type
make_tracer_direct(UnifiedStatsReporter *reporter, StringRef name, T *)
{

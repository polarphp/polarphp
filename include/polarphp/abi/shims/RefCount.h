...
//
//===----------------------------------------------------------------------===//
#ifndef POLARPHP_ABI_SHIMS_REFCOUNT_H
#define POLARPHP_ABI_SHIMS_REFCOUNT_H
...
typedef struct {
   __polarphp_uintptr_t refCounts SWIFT_ATTRIBUTE_UNAVAILABLE;
} InlineRefCountsPlaceholder;

#if defined(__polarphp__)
...
#include "polarphp/runtime/Atomic.h"
#include "polarphp/runtime/Config.h"
#include "polarphp/runtime/Debug.h"
...
static_assert(sizeof(InlineRefCounts) == sizeof(__polarphp_uintptr_t),
              "InlineRefCounts must be pointer-sized");
static_assert(__alignof(InlineRefCounts) == __alignof(__polarphp_uintptr_t),
              "InlineRefCounts must be pointer-aligned");
#else
_Static_assert(sizeof(InlineRefCounts) == sizeof(InlineRefCountsPlaceholder),
               "InlineRefCounts and InlineRefCountsPlaceholder must match");
_Static_assert(sizeof(InlineRefCounts) == sizeof(__polarphp_uintptr_t),
               "InlineRefCounts must be pointer-sized");
_Static_assert(_Alignof(InlineRefCounts) == _Alignof(__polarphp_uintptr_t),
               "InlineRefCounts must be pointer-aligned");
#endif

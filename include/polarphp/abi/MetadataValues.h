...
#ifndef POLARPHP_ABI_METADATAVALUES_H
#define POLARPHP_ABI_METADATAVALUES_H
...
enum class InterfaceClassConstraint : bool {
...
class InterfaceDescriptorFlags {
   ...
   enum : int_type {
      ...
      SpecialInterfaceMask  = 0x000003C0U,
      SpecialInterfaceShift = 6,
   ...
   constexpr InterfaceDescriptorFlags(int_type Data) : Data(Data) {}
public:
   ...
   constexpr InterfaceDescriptorFlags withClassConstraint(
      ...
      return InterfaceDescriptorFlags((Data & ~ClassConstraint)
                                     | (bool(c) ? ClassConstraint : 0));
   }
   constexpr InterfaceDescriptorFlags withDispatchStrategy(
      InterfaceDispatchStrategy s) const {
      return InterfaceDescriptorFlags((Data & ~DispatchStrategyMask)
                                     | (int_type(s) << DispatchStrategyShift));
   }
   constexpr InterfaceDescriptorFlags
   withSpecialInterface(SpecialInterface sp) const {
      return InterfaceDescriptorFlags((Data & ~SpecialInterfaceMask)
                                     | (int_type(sp) << SpecialInterfaceShift));
   }
   constexpr InterfaceDescriptorFlags withResilient(bool s) const {
      return InterfaceDescriptorFlags((Data & ~IsResilient) | (s ? IsResilient : 0));
   }
   ...
   InterfaceClassConstraint getClassConstraint() const {
      return InterfaceClassConstraint(bool(Data & ClassConstraint));
   }

   /// What dispatch strategy does this protocol use?
   InterfaceDispatchStrategy getDispatchStrategy() const {
      return InterfaceDispatchStrategy((Data & DispatchStrategyMask)
   ...
   static bool needsWitnessTable(InterfaceDispatchStrategy strategy) {
      switch (strategy) {
         case InterfaceDispatchStrategy::ObjC:
            return false;
         case InterfaceDispatchStrategy::Swift:
            return true;
      }
   ...
   SpecialInterface getSpecialInterface() const {
      return SpecialInterface(uint8_t((Data & SpecialInterfaceMask)
                                        >> SpecialInterfaceShift));
   }
...
/// Flags that go in a InterfaceRequirement structure.
class InterfaceRequirementFlags {
public:
   typedef uint32_t int_type;
   enum class Kind {
      BaseInterface,
...
public:
   InterfaceRequirementFlags(Kind kind) : Value(unsigned(kind)) {}

   InterfaceRequirementFlags withIsInstance(bool isInstance) const {
      auto copy = *this;
      if (isInstance) {
         copy.Value |= IsInstanceMask;
      } else {
   ...
   enum : uint8_t {
      ...
      /// type's context.
         AssociatedTypeInInterfaceContextByte = 0xFF
   };
};
...
private:
   enum : int_type {
      ...
      SpecialInterfaceMask   = 0x3F000000U,
      SpecialInterfaceShift  = 24U,
   };
   int_type Data;
...
public:
   ...
   withClassConstraint(InterfaceClassConstraint c) const {
   ...
   constexpr ExistentialTypeFlags
   withSpecialInterface(SpecialInterface sp) const {
      return ExistentialTypeFlags((Data & ~SpecialInterfaceMask)
                                  | (int_type(sp) << SpecialInterfaceShift));
   }
   ...
   InterfaceClassConstraint getClassConstraint() const {
      return InterfaceClassConstraint(bool(Data & ClassConstraintMask));
   }
...
class InterfaceContextDescriptorFlags : public FlagSet<uint16_t> {
   enum {
      ...
      /// Special protocol value.
         SpecialInterfaceKind = 2,
      SpecialInterfaceKind_width = 6,
   };

public:
   explicit InterfaceContextDescriptorFlags(uint16_t bits) : FlagSet(bits) {}
   constexpr InterfaceContextDescriptorFlags() {}
   ...
   FLAGSET_DEFINE_FIELD_ACCESSORS(HasClassConstraint,
                                  ...
                                  InterfaceClassConstraint,
                                  getClassConstraint,
                                  setClassConstraint)

   FLAGSET_DEFINE_FIELD_ACCESSORS(SpecialInterfaceKind,
                                  SpecialInterfaceKind_width,
                                  SpecialInterface,
                                  getSpecialInterface,
                                  setSpecialInterface)
};

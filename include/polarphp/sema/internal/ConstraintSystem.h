...
//
//===----------------------------------------------------------------------===//
#ifndef POLARPHP_SEMA_CONSTRAINT_SYSTEM_H
#define POLARPHP_SEMA_CONSTRAINT_SYSTEM_H

#include "polarphp/sema/internal/CSFix.h"
#include "polarphp/sema/internal/Constraint.h"
#include "polarphp/sema/internal/ConstraintGraph.h"
#include "polarphp/sema/internal/ConstraintGraphScope.h"
#include "polarphp/sema/internal/ConstraintLocator.h"
#include "polarphp/sema/internal/OverloadChoice.h"
...
void *operator new(size_t bytes, polar::constraints::ConstraintSystem& cs,
                   size_t alignment = 8);
...
public:
  ...
  std::vector<std::pair<ConstraintLocator *, InterfaceConformanceRef>>
      Conformances;
...
private:
  ...
  struct SolverState {
    ...
    #include "polarphp/sema/internal/ConstraintSolverStatsDef.h"
...
private:
  ...
  friend class polar::TypeChecker;
...
public:
  ...
  ArrayRef<Type> getAlternativeLiteralTypes(KnownInterfaceKind kind);
...
public:
  ...
  void openGenericRequirements(DeclContext *outerDC,
                               GenericSignature signature,
                               bool skipInterfaceSelfConstraint,
                               ConstraintLocatorBuilder locator,
...
public:
  ...
  /// existential type.
  ///
  /// \param kind Either ConstraintKind::SelfObjectOfInterface or
  /// ConstraintKind::ConformsTo. Usually this uses SelfObjectOfInterface,
...
private:
  ...
  SolutionKind simplifyConformsToConstraint(Type type, InterfaceDecl *protocol,
                                            ConstraintKind kind,
                                            ConstraintLocatorBuilder locator,
                                            TypeMatchOptions flags);
  ...
  /// \param locator Locator describing where this constraint occurred.
  ///
  /// \param kind If this is SelfTypeOfInterface, we allow an existential type
...
private:
  ...
  struct PotentialBinding {
    ...
    InterfaceDecl *DefaultedInterface;
    ...
    PotentialBinding(Type type, AllowedBindingKind kind,
                     ConstraintKind bindingSource,
                     InterfaceDecl *defaultedInterface = nullptr,
                     ConstraintLocator *defaultableBinding = nullptr)
        : BindingType(type->getWithoutParens()), Kind(kind),
          BindingSource(bindingSource), DefaultedInterface(defaultedInterface),
    ...
    PotentialBinding withType(Type type) const {
      return {type, Kind, BindingSource, DefaultedInterface, DefaultableBinding};
    }
  };
  ...
  struct PotentialBindings {
    ...
    void foundLiteralBinding(InterfaceDecl *proto) {
      switch (*proto->getKnownInterfaceKind()) {
    ...
    void dump(llvm::raw_ostream &out,
      ...
      interleave(Bindings,
                 [&](const PotentialBinding &binding) {
                   ...
                   if (binding.DefaultedInterface)
                     out << "(default from "
                         << binding.DefaultedInterface->getName() << ") ";
...
public:
  ...
  class ArgumentInfoCollector {
    ...
    llvm::SetVector<InterfaceDecl *> LiteralInterfaces;
    ...
    void addLiteralInterface(InterfaceDecl *proto) {
      LiteralInterfaces.insert(proto);
    }

    void walk(Type argType);
    void minimizeLiteralInterfaces();
  ...
  public:
    ...
    const llvm::SetVector<InterfaceDecl *> &getLiteralInterfaces() const {
...
bool conformsToKnownInterface(ConstraintSystem &cs, Type type,
                             KnownInterfaceKind protocol);
...
Type isRawRepresentable(ConstraintSystem &cs, Type type,
                        KnownInterfaceKind rawRepresentableInterface);
...
public:
  ...
  bool hasDefaultedInterface() const { return Binding.DefaultedInterface; }
...
#endif // LLVM_POLARPHP_SEMA_CONSTRAINT_SYSTEM_H

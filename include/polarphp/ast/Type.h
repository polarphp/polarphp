...
class Type
{
   TypeBase *m_ptr;
public:
   /*implicit*/ Type(TypeBase *ptr = nullptr)
      : m_ptr(ptr) {}
...
public:
   explicit CanType(TypeBase *ptr = nullptr) : Type(ptr)
   {
...
{
   auto typePointer = type.getPointer();
   POLAR_ASSUME(typePointer != nullptr);
   return CanTypeWrapper<X>(dyn_cast<X>(typePointer));
}
...
inline bool isa(CanTypeWrapper<ptr> type)
{
   return isa<X>(type.getPointer());
}

template <class X, class ptr>
inline CanTypeWrapper<X> cast(CanTypeWrapper<ptr> type)
{
   return CanTypeWrapper<X>(cast<X>(type.getPointer()));
}

template <class X, class ptr>
inline CanTypeWrapper<X> dyn_cast(CanTypeWrapper<ptr> type)
{
   auto typePointer = type.getPointer();
   POLAR_ASSUME(typePointer != nullptr);
   return CanTypeWrapper<X>(dyn_cast<X>(typePointer));
...
template <class X, class ptr>
inline CanTypeWrapper<X> dyn_cast_or_null(CanTypeWrapper<ptr> type)
{
   return CanTypeWrapper<X>(dyn_cast_or_null<X>(type.getPointer()));
}
...
public:
   CanGenericSignature() : m_signature(nullptr) {}
   CanGenericSignature(std::nullptr_t) : m_signature(nullptr)
   {}

   // in Decl.h
   explicit CanGenericSignature(GenericSignature *m_signature);
   ...
   bool operator==(const polar::ast::CanGenericSignature& other)
   {
...
template <typename T>
inline T *static_cast_helper(const Type &type)
{
...
template <typename T>
using TypeArrayView = ArrayRefView<Type, T*, static_cast_helper,
...
operator<<(raw_ostream &OS, polar::ast::Type type)
{
   type.print(OS);
   return OS;
}
...
template<>
struct simplify_type<const ::polar::ast::Type>
{
   typedef ::polar::ast::TypeBase *SimpleType;
   static SimpleType getSimplifiedValue(const ::polar::ast::Type &value)
   {
...
template<> struct DenseMapInfo<polar::ast::Type>
{
   ...
   static unsigned getHashValue(polar::ast::Type value)
   {
      return DenseMapInfo<polar::ast::TypeBase*>::getHashValue(value.getPointer());
   }
...
{
public:
   static inline void *getAsVoidPointer(polar::ast::Type type)
   {
      return reinterpret_cast<void*>(type.getPointer());
   }

   static inline polar::ast::Type getFromVoidPointer(void *pointer)
   {
      return reinterpret_cast<polar::ast::TypeBase*>(pointer);
   }
...
{
public:
   static inline polar::ast::CanType getFromVoidPointer(void *pointer)
   {
      return polar::ast::CanType(reinterpret_cast<polar::ast::TypeBase*>(pointer));
   }
};

template<>
struct PointerLikeTypeTraits<polar::ast::CanGenericSignature>
{
public:
   static inline polar::ast::CanGenericSignature getFromVoidPointer(void *pointer)
   {
      return polar::ast::CanGenericSignature(reinterpret_cast<polar::ast::GenericSignature*>(pointer));
   }

   static inline void *getAsVoidPointer(polar::ast::CanGenericSignature signature)
   {
      return reinterpret_cast<void *>(signature.getPointer());
   }
   enum { NumLowBitsAvailable = polar::ast::TypeAlignInBits };
};

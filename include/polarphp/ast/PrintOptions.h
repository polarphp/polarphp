...
public:
   BracketOptions(Decl *target = nullptr, bool openExtension = true,
      ...
      m_target(target), m_openExtension(openExtension),
...
public:
   ...
   AnyAttrKind(DeclAttrKind kind) : m_kind(static_cast<unsigned>(kind)), m_isType(0)
   {
   ...
   bool operator==(AnyAttrKind kind) const
   {
      return m_kind == kind.m_kind && m_isType == kind.m_isType;
   }
...
struct PrintOptions
{
   ...
   llvm::DenseMap<CanType, TokenSyntax> *alternativeTypeNames = nullptr;
...
//   void initForSynthesizedExtension(TypeOrExtensionDecl decl);
   ...
   bool shouldPrint(const Decl* decl) const
   {
      return currentPrintabilityChecker->shouldPrint(decl, *this);
   }
   ...
   static PrintOptions printQuickHelpDeclaration()
   {
      ...
      printOptions.skipUnderscoredKeywords = true;
      printOptions.enumRawValues = EnumRawValueMode::Print;
      printOptions.printImplicitAttrs = false;
      printOptions.printFunctionRepresentationAttrs = false;
      printOptions.printDocumentationComments = false;
      printOptions.excludeAttrList.push_back(DAK_Available);
      printOptions.skipPrivateStdlibDecls = true;
      printOptions.explodeEnumCaseDecls = true;
      printOptions.shouldQualifyNestedDeclarations = QualifyNestedDeclarations::TypesOnly;
      printOptions.printParameterSpecifiers = true;
      printOptions.skipImplicit = true;
      return printOptions;
   }
};

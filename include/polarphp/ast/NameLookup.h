...
#ifndef POLARPHP_AST_NAME_LOOKUP_H
#define POLARPHP_AST_NAME_LOOKUP_H
...
public:
   ...
   UnqualifiedLookup(DeclName Name, DeclContext *m_declContext, LazyResolver *TypeResolver,
...
public:
   ...
   virtual void foundDecl(ValueDecl *foundDecl, DeclVisibilityKind reason) = 0;
...
{
public:
   LambdaDeclConsumer(FuncType &&callback)
      : m_callback(std::move(callback))
   {}

   void foundDecl(ValueDecl *valueDecl, DeclVisibilityKind reason)
   {
      m_callback(valueDecl, reason);
   }
...
template <class FuncType>
LambdaDeclConsumer<FuncType> make_decl_consumer(FuncType &&callback)
{
   return LambdaDeclConsumer<FuncType>(std::move(callback));
}
...
{
public:
   AccessFilteringDeclConsumer(const DeclContext *declContext,
                               VisibleDeclConsumer &consumer)
      : m_declContext(declContext),
        m_chainedConsumer(consumer)
   {}

   void foundDecl(ValueDecl *valueDecl, DeclVisibilityKind reason) override;
private:
   const DeclContext *m_declContext;
   VisibleDeclConsumer &m_chainedConsumer;
};
...
bool remove_overridden_decls(SmallVectorImpl<ValueDecl*> &decls);
...
bool remove_shadowed_decls(SmallVectorImpl<ValueDecl*> &decls,
...
/// are visible.
void lookup_visible_decls(VisibleDeclConsumer &consumer,
...
/// VisibleDeclConsumer.
///
/// \param currDeclContext the DeclContext from which the lookup is done.
void lookup_visible_member_decls(VisibleDeclConsumer &consumer,
                                 Type baseType,
...
void get_directly_inherited_nominal_type_decls(
      PointerUnion<TypeDecl *, ExtensionDecl *> decl,
      unsigned i,
      SmallVectorImpl<std::pair<SourceLoc, NominalTypeDecl *>> &result,
      bool &anyObject);
...
get_directly_inherited_nominal_type_decls(
      PointerUnion<TypeDecl *, ExtensionDecl *> decl,
      bool &anyObject);
...
SelfBounds get_self_bounds_from_where_clause(
      PointerUnion<TypeDecl *, ExtensionDecl *> decl);
...
{
public:
   FindLocalVal(const SourceManager &sourceMgr, SourceLoc loc,
                VisibleDeclConsumer &consumer)
      : m_sm(sourceMgr),
        m_loc(loc),
        m_consumer(consumer)
   ...
   void checkValueDecl(ValueDecl *valueDecl, DeclVisibilityKind reason)
   {
      m_consumer.foundDecl(valueDecl, reason);
   }

   void checkParameterList(const ParameterList *params);
   void checkGenericParams(GenericParamList *params);
   void checkSourceFile(const SourceFile &sourceFile);
...
private:
   ...
   void visitGuardStmt(GuardStmt *stmt);
   void visitWhileStmt(WhileStmt *stmt);
   void visitRepeatWhileStmt(RepeatWhileStmt *stmt);
   void visitDoStmt(DoStmt *stmt);
   void visitForEachStmt(ForEachStmt *stmt);
   void visitBraceStmt(BraceStmt *stmt, bool isTopLevelCode = false);
   void visitSwitchStmt(SwitchStmt *stmt);
   void visitCaseStmt(CaseStmt *stmt);
   void visitDoCatchStmt(DoCatchStmt *stmt);
...
private:
   ...
   SourceLoc m_loc;
   VisibleDeclConsumer &m_consumer;
};

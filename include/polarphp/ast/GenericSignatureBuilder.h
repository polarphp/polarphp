...
#ifndef POLARPHP_AST_GENERICSIGNATURE_BUILDER_H
#define POLARPHP_AST_GENERICSIGNATURE_BUILDER_H
...
public:
   ...
   struct EquivalenceClass : llvm::ilist_node<EquivalenceClass> {
      ...
      llvm::MapVector<InterfaceDecl *, std::vector<Constraint<InterfaceDecl *>>>
      conformsTo;
      ...
      bool isConformanceSatisfiedBySuperclass(InterfaceDecl *proto) const;
      ...
      POLAR_DEBUG_DUMPER(dump(GenericSignatureBuilder *builder = nullptr));
      ...
      /// Cached access paths.
      llvm::SmallDenseMap<const InterfaceDecl *, ConformanceAccessPath, 8>
...
private:
   ...
   bool addConditionalRequirements(InterfaceConformanceRef conformance,
...
public:
   ...
   InterfaceConformanceRef lookupConformance(CanType dependentType,
                                            Type conformingReplacementType,
   ...
   static GenericSignature computeRequirementSignature(InterfaceDecl *proto);
...
class GenericSignatureBuilder::RequirementSource final
      ...
      private llvm::TrailingObjects<RequirementSource, InterfaceDecl *,
      WrittenRequirementLoc> {
...
public:
   enum Kind : uint8_t {
      ...
      /// dependent type (relative to that protocol) to which the conformance
      /// appertains.
      InferredInterfaceRequirement,

      /// A requirement that was resolved via a superclass requirement.
      ///
      /// This stores the \c InterfaceConformanceRef used to resolve the
      /// requirement.
      Superclass,
      ...
      /// concrete constraint.
      ///
      /// This stores the \c InterfaceConformance* used to resolve the
      /// requirement.
      Concrete,
...
public:
   ...
   RequirementSource(Kind kind, const RequirementSource *parent,
      ...
      : kind(kind), storageKind(StorageKind::InterfaceConformance),
...
private:
   ...
   const RequirementSource *viaInterfaceRequirement(
         GenericSignatureBuilder &builder,
         Type dependentType,
         InterfaceDecl *protocol,
         bool inferred,
...
public:
   ...
   bool isInterfaceRequirement() const {
      return kind == InterfaceRequirement || kind == InferredInterfaceRequirement;
   }
   ...
   InterfaceConformanceRef getInterfaceConformance() const {
      assert(storageKind == StorageKind::InterfaceConformance);
      return InterfaceConformanceRef::getFromOpaqueValue(storage.conformance);
   }
...
class GenericSignatureBuilder::FloatingRequirementSource {
   ...
   union {
      struct {
         InterfaceDecl *protocol = nullptr;
...
public:
   ...
   static FloatingRequirementSource viaInterfaceRequirement(
         const RequirementSource *base,
         InterfaceDecl *inInterface,
         bool inferred) {
      FloatingRequirementSource result{ AbstractInterface, base };
      result.protocolReq.protocol = inInterface;

...
#ifndef POLARPHP_AST_IRGENOPTIONS_H
#define POLARPHP_AST_IRGENOPTIONS_H
...
public:
  ...
  unsigned DisablePHPSpecificLLVMOptzns : 1;
  ...
  IRGenOptions()
      : DWARFVersion(2), OutputKind(IRGenOutputKind::LLVMAssembly),
        ...
        DisablePHPSpecificLLVMOptzns(false), DisableLLVMSLPVectorizer(false),
        ...
        UseIncrementalLLVMCodeGen(true), UsePHPCall(false),
  ...
  void writeLLVMCodeGenOptionsTo(llvm::raw_ostream &os) {
    ...
    os << 'O' << (unsigned)OptMode
       << 'd' << DisableLLVMOptzns
       << 'D' << DisablePHPSpecificLLVMOptzns
       << 'p' << GenerateProfile
       << 's' << Sanitizers.toRaw();
  }

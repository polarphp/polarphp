...
#ifndef POLARPHP_DRIVER_FINE_GRAINED_DEPENDENCY_DRIVER_GRAPH_H
#define POLARPHP_DRIVER_FINE_GRAINED_DEPENDENCY_DRIVER_GRAPH_H
...
public:
   ...
   bool operator==(const ModuleDepGraphNode &other) const {
      return static_cast<DepGraphNode>(*this) ==
             ...
             getPHPDeps() == other.getPHPDeps();
   }

   const Optional<std::string> &getPHPDeps() const { return phpDeps; }

   bool assertImplementationMustBeInAFile() const {
      assert((getPHPDeps().hasValue() || !getKey().isImplementation()) &&
   ...
   std::string humanReadableName() const {
      StringRef where =
         !getPHPDeps().hasValue()
         ? ""
         : llvm::sys::path::filename(getPHPDeps().getValue());
   ...
   /// compilation.
   void setSwiftDeps(Optional<std::string> s) { phpDeps = s; }

   bool getIsProvides() const { return getPHPDeps().hasValue(); }
};
...
public:
   ...
   using LoadResult = typename polar::DependencyGraphImpl::LoadResult;
};
...
class ModuleDepGraph {
   ...
   /// Find nodes, first by the phpDeps file, then by key.
   ...
   /// nodesBySwiftDepsFile and the phpDeps node instance variable here.
   void addToMap(ModuleDepGraphNode *n) {
      nodeMap.insert(n->getPHPDeps().getValueOr(std::string()), n->getKey(), n);
   }
   ...
   ModuleDepGraphNode *eraseNodeFromMap(ModuleDepGraphNode *nodeToErase) {
      ModuleDepGraphNode *nodeActuallyErased = nodeMap.findAndErase(
         nodeToErase->getPHPDeps().getValueOr(std::string()),
         nodeToErase->getKey());
   ...
   static StringRef getPHPDeps(const driver::Job *cmd) {
...
private:
   ...
   /// Use the known phpDeps to find a directory for
   ...
   /// Integrate the \p integrand, a node that represents a Decl in the phpDeps
   ...
   /// not known to reside in any phpDeps file. Return a bool indicating if
   ...
   bool rememberThatJobCascades(StringRef phpDeps) {
      return cascadingJobs.insert(phpDeps).second;
   }
   ...
   bool ensureJobIsTracked(const std::string &phpDeps) const {
      assert(phpDeps.empty() || getJob(phpDeps));
      return true;
   }
...
#endif // POLARPHP_DRIVER_FINE_GRAINED_DEPENDENCY_DRIVER_GRAPH_H

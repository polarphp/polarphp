...
//===----------------------------------------------------------------------===//
///
/// Utilities used by the PILOptimizer for SSA analysis and update.
///
//===----------------------------------------------------------------------===//

#ifndef POLARPHP_PIL_OPTIMIZER_UTILS_VALUE_LIFE_TIME_H
#define POLARPHP_PIL_OPTIMIZER_UTILS_VALUE_LIFE_TIME_H
...
/// of the analysis and can be a super set of the uses of the PILValue
/// e.g. it can be the set of transitive uses of the PILValue.
...
public:
   ...
   bool isWithinLifetime(PILInstruction *inst);

   /// Returns true if the value is alive at the begin of block \p bb.
   bool isAliveAtBeginOfBlock(PILBasicBlock *bb) {

...
#ifndef POLARPHP_PIL_OPTIMIZER_UTILS_PERFORMANCE_INLINER_UTILS_H
#define POLARPHP_PIL_OPTIMIZER_UTILS_PERFORMANCE_INLINER_UTILS_H
...
PILFunction *getEligibleFunction(FullApplySite AI,
...
class ConstantTracker {
  ...
  llvm::DenseMap<PILInstruction *, PILInstruction *> links;
  ...
  // aliasing.
  llvm::DenseMap<PILValue, PILInstruction *> memoryContent;
  ...
  PILInstruction *getDef(PILValue val, ProjectionPath &projStack);
...
public:
  ...
  ConstantTracker(PILFunction *function) :
    F(function), callerTracker(nullptr), AI()
  { }

  // Constructor for the callee function.
  ConstantTracker(PILFunction *function, ConstantTracker *caller,
  ...
  void trackInst(PILInstruction *inst);

  // Gets the estimated definition of a value.
  PILInstruction *getDef(PILValue val) {
  ...
  PILInstruction *getDefInCaller(PILValue val) {
    PILInstruction *def = getDef(val);
    if (def && def->getFunction() != F)
      return def;
    return nullptr;
  }

  bool isStackAddrInCaller(PILValue val) {
    if (PILValue Param = getParam(stripAddressProjections(val))) {
  ...
  IntConst getIntConst(PILValue val, int depth = 0);

  PILBasicBlock *getTakenBlock(TermInst *term);
};
...
private:
  ...
  PILFunction *F;
  PILLoopInfo *LI;
  llvm::DenseMap<const PILBasicBlock *, BlockInfo *> BlockInfos;
  std::vector<BlockInfo> BlockInfoStorage;
  bool valid = false;

  BlockInfo *getBlockInfo(const PILBasicBlock *BB) {
  ...
  const PILBasicBlock *getBlock(const PILBasicBlock *BB) { return BB; }
  const PILBasicBlock *getBlock(const PILBasicBlock &BB) { return &BB; }

  /// Returns the minimum distance from all predecessor blocks of \p BB.
  int getEntryDistFromPreds(const PILBasicBlock *BB, int LoopDepth);

  /// Returns the minimum distance from all successor blocks of \p BB.
  int getExitDistFromSuccs(const PILBasicBlock *BB, int LoopDepth);
  ...
  void solveDataFlow(const BlockList &Blocks, int LoopDepth) {
    ...
    do {
      ...
      for (auto &Block : Blocks) {
        const PILBasicBlock *BB = getBlock(Block);
  ...
  void analyzeLoopsRecursively(PILLoop *Loop, int LoopDepth);

  void printFunction(llvm::raw_ostream &OS);

  void printLoop(llvm::raw_ostream &OS, PILLoop *Loop, int LoopDepth);

  void printBlockInfo(llvm::raw_ostream &OS, PILBasicBlock *BB, int LoopDepth);

public:
  ShortestPathAnalysis(PILFunction *F, PILLoopInfo *LI) : F(F), LI(LI) { }
  ...
  int getScopeLength(PILBasicBlock *BB, int LoopDepth) {
  ...
  Weight getWeight(PILBasicBlock *BB, Weight CallerWeight);

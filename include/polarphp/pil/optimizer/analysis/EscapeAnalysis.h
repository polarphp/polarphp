//===--- EscapeAnalysis.h - PIL Escape Analysis -----------------*- C++ -*-===//
//
...
///
/// This design is customized for PIL and the Swift memory model as follows:
///
/// Each PILValue holding a memory address or object reference is mapped to a
/// node in the connection graph. The node's type depends on the value's
/// origin. PILArguments have "argument" type. Locally allocated storage and
...
/// pointed to by one of the other node types. So, except for loads, PILValues
/// do not directly map to content nodes. For debugging purposes only, content
/// nodes do refer back to the PILValue that originally pointed to them. When
/// content nodes are merged, only one of those PILValue back-references is
/// arbitrarily preserved. The content of the returned value is the only content
/// node that has no back-reference to a PILValue.
///
/// This code:
///   let a = SomeClass()
///   return a
///
/// Generates the following connection graph, where 'a' is in the PILValue %0:
...
/// 'a' is in the PILValue %0 and class allocation returns PILValue %3.
...
#ifndef POLARPHP_PIL_OPTIMIZER_ANALYSIS_ESCAPEANALYSIS_H
#define POLARPHP_PIL_OPTIMIZER_ANALYSIS_ESCAPEANALYSIS_H
...
public:
   ...
   class CGNode {
      ...
      /// the node via 'mappedValue'. Setting 'mappedValue' to a valid PILValue
      ...
      /// unmapped when their PILValue is deleted. Content nodes are conditionally
      ...
      /// Get the representative node that maps to a PILValue and depth in the
      /// pointsTo graph.
   ...
   public:
      struct RepValue {
         // May only be an invalid PILValue for Return nodes or deleted values.
         llvm::PointerIntPair<PILValue, 1, bool> valueAndIsReturn;
         unsigned depth;

         PILValue getValue() const { return valueAndIsReturn.getPointer(); }
         bool isReturn() const { return valueAndIsReturn.getInt(); }
         void
         print(llvm::raw_ostream &stream,
               const llvm::DenseMap<const PILNode *, unsigned> &instToIDMap) const;
      };
      // Get the representative PILValue for this node its depth relative to the
      ...
      bool valueEscapesInsideFunction(PILValue nodeValue) const {
      ...
      bool escapesInsideFunction() const {
         PILValue nodeValue = isContent() ? PILValue() : PILValue(mappedValue);
...
public:
   ...
   class ConnectionGraph {
      ...
      /// Mapping from pointer PIL values to nodes in the graph. Such a value can
      ...
      llvm::SmallVector<PILNode *, 16> UsePointTable;

      /// Mapping of use points to bit indices in CGNode::UsePoints.
      llvm::DenseMap<PILNode *, int> UsePoints;
      ...
      ConnectionGraph(PILFunction *F, EscapeAnalysis *EA, bool isSummaryGraph)
      ...
      /// Re-uses a node for another PIL value.
      ...
      int addUsePoint(CGNode *Node, PILNode *User) {
         if (Node->getEscapeState() >= EscapeState::Global)
            return -1;

         User = User->getRepresentativePILNodeInObject();
   ...
   public:
      ...
      bool isUsePoint(PILNode *UsePoint, CGNode *Node);

      /// Returns all use points of \p Node in \p UsePoints.
      void getUsePoints(CGNode *Node,
                        llvm::SmallVectorImpl<PILNode *> &UsePoints);
...
private:
   ...
   struct FunctionInfo : public FunctionInfoBase<FunctionInfo> {
      FunctionInfo(PILFunction *F, EscapeAnalysis *EA)
   ...
   /// functions).
   llvm::DenseMap<PILFunction *, FunctionInfo *> Function2Info;

   /// The allocator for the connection graphs in Function2ConGraph.
   llvm::SpecificBumpPtrAllocator<FunctionInfo> Allocator;

   /// Cache for isPointer().
   llvm::DenseMap<PILType, bool> isPointerCache;
   ...
   PILValue getPointerBase(PILValue value) const;
   ...
   /// value.
   PILValue getPointerRoot(PILValue value) const;
   ...
   CGNode *getValueContent(ConnectionGraph *conGraph, PILValue addrVal);

   /// Build a connection graph for reach callee from the callee list.
   bool buildConnectionGraphForCallees(PILInstruction *Caller,
                                       CalleeList Callees,
                                       FunctionInfo *FInfo,
   ...
   /// PILValue.
   bool buildConnectionGraphForDestructor(PILValue V,
                                          PILInstruction *Caller,
                                          FunctionInfo *FInfo,
                                          FunctionOrder &BottomUpOrder,
                                          int RecursionDepth);
   ...
   /// Visited callees are added to \p BottomUpOrder until \p RecursionDepth
   /// reaches MaxRecursionDepth.
   void analyzeInstruction(PILInstruction *I, FunctionInfo *FInfo,
                           FunctionOrder &BottomUpOrder,
                           int RecursionDepth);
   ...
   bool deinitIsKnownToNotCapture(PILValue V);

   /// Sets all operands and results of \p I as global escaping.
   void setAllEscaping(PILInstruction *I, ConnectionGraph *ConGraph);
   ...
   bool mergeCalleeGraph(PILInstruction *FAS,
   ...
   bool canEscapeToUsePoint(PILValue V, PILNode *UsePoint,
                            ConnectionGraph *ConGraph);

   friend struct ::CGForDotView;

public:
   EscapeAnalysis(PILModule *M);

   static bool classof(const PILAnalysis *S) {
      return S->getKind() == PILAnalysisKind::Escape;
   }

   virtual void initialize(PILPassManager *PM) override;

   /// Gets the connection graph for \a F.
   ConnectionGraph *getConnectionGraph(PILFunction *F) {
   ...
   bool canEscapeTo(PILValue V, FullApplySite FAS);
   ...
   bool canEscapeTo(PILValue V, RefCountingInst *RI);
   ...
   /// to \p V.
   bool canEscapeToValue(PILValue V, PILValue To);
   ...
   bool canPointToSameMemory(PILValue V1, PILValue V2);
   ...
   virtual void handleDeleteNotification(PILNode *N) override;
   ...
   virtual void verify(PILFunction *F) const override {
#ifndef NDEBUG

...
#ifndef POLARPHP_PIL_OPTIMIZER_ANALYSIS_ANALYSIS_H
#define POLARPHP_PIL_OPTIMIZER_ANALYSIS_ANALYSIS_H
...
struct PILAnalysisKind {
   enum InnerTy {
#define ANALYSIS(NAME) NAME,
#include "polarphp/pil/optimizer/analysis/AnalysisDef.h"
   } value;

   PILAnalysisKind(InnerTy newValue) : value(newValue) {}
   operator InnerTy() const { return value; }
};

/// The base class for all PIL-level analysis.
class PILAnalysis : public DeleteNotificationHandler {
public:
...
public:
   ...
   PILAnalysisKind getKind() const { return kind; }

   /// Constructor.
   PILAnalysis(PILAnalysisKind k) : kind(k), invalidationLock(false) {}
   ...
   /// analysis depends on.
   virtual void initialize(PILPassManager *pm) { }
   ...
   virtual void invalidate(PILFunction *f, InvalidationKind k) = 0;

   /// Notify the analysis about a newly added or modified function.
   virtual void notifyAddedOrModifiedFunction(PILFunction *f) = 0;

   /// Notify the analysis about a function that will be deleted from the
   /// module.
   virtual void notifyWillDeleteFunction(PILFunction *f) = 0;
   ...
   virtual void verify(PILFunction *F) const { verify(); }
...
class FunctionAnalysisBase : public PILAnalysis {
   using StorageTy = llvm::DenseMap<PILFunction *,
...
protected:
   ...
   newFunctionAnalysis(PILFunction *f) = 0;

   /// Return True if the analysis should be invalidated given trait \K is
   /// preserved.
   virtual bool shouldInvalidate(PILAnalysis::InvalidationKind k) = 0;
...
public:
   ...
   bool hasFunctionInfo(PILFunction *f) const { return storage.count(f); }

   /// Attempt to lookup up the information that the analysis has for the given
   /// function. Returns nullptr upon failure.
   NullablePtr<FunctionInfoTy> maybeGet(PILFunction *f) {
   ...
   FunctionInfoTy *get(PILFunction *f) {
   ...
   void invalidateFunction(PILFunction *f) {
      storage.erase(f);
   }

   /// Invalidate all of the information for a specific function.
   virtual void invalidate(PILFunction *f,
   ...
   FunctionAnalysisBase(PILAnalysisKind k) : PILAnalysis(k), storage() {}
   ...
   virtual void verify(PILFunction *f) const override final {
      auto iter = storage.find(f);
      if (iter == storage.end())
         return;
      if (!iter->second)
...
public:
   LazyFunctionInfo(PILFunction *func, AnalysisTy *analysis) : func(func), analysis(analysis), funcInfo() {}

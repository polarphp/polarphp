//===--- PILVTableVisitor.h - Class vtable visitor --------------*- C++ -*-===//
//
...
//===----------------------------------------------------------------------===//
//
// This file defines the PILVTableVisitor class, which is used to generate and
// perform lookups in class method vtables.
//
//===----------------------------------------------------------------------===//

#ifndef POLARPHP_PIL_PILVTABLEVISITOR_H
#define POLARPHP_PIL_PILVTABLEVISITOR_H
...
///
/// - addMethod(PILDeclRef):
///   introduce a new vtable entry
///
/// - addMethodOverride(PILDeclRef baseRef, PILDeclRef derivedRef):
...
///
template <class T> class PILVTableVisitor {
  T &asDerived() { return *static_cast<T*>(this); }

  void maybeAddMethod(FuncDecl *fd) {
    assert(!fd->hasClangNode());

    maybeAddEntry(PILDeclRef(fd, PILDeclRef::Kind::Func));
  }
  ...
  void maybeAddConstructor(ConstructorDecl *cd) {
    ...
    maybeAddEntry(PILDeclRef(cd, PILDeclRef::Kind::Allocator));
  }
  ...
  void maybeAddEntry(PILDeclRef declRef) {

...
enum {
   NumPILNodeKindBits = count_bits_used(static_cast<unsigned>(PILNodeKind::Last_PILNode))
};
...
protected:
   union {
      ...
      POLAR_INLINE_BITFIELD_BASE(
            PILNode, bitmax(NumPILNodeKindBits,8)+1+1,
      ...
      POLAR_INLINE_BITFIELD(
            PILArgument, ValueBase, NumVOKindBits,
      ...
      POLAR_INLINE_BITFIELD_FULL(
            MultipleValueInstructionResult, ValueBase,
            NumVOKindBits+32,
            VOKind : NumVOKindBits,
            : NumPadBits,
            Index : 32
      ...
      POLAR_INLINE_BITFIELD(
            IBWTO, PILNode, 64-NumPILNodeBits,
...
#define UIWTDOB_BITFIELD_EMPTY(T, U) \
   IBWTO_BITFIELD_EMPTY(T, U)
      ...
      IBWTO_BITFIELD(
            ObjectInst, SingleValueInstruction,
            32-NumSingleValueInstructionBits,
            NumBaseElements : 32-NumSingleValueInstructionBits
            );

      IBWTO_BITFIELD(
            SelectEnumInstBase, SingleValueInstruction, 1,
            HasDefault : 1
            );

      POLAR_INLINE_BITFIELD_FULL(
            IntegerLiteralInst, LiteralInst, 32,
            : NumPadBits,
            numBits : 32
            );

      POLAR_INLINE_BITFIELD_FULL(
            FloatLiteralInst, LiteralInst, 32,
            : NumPadBits,
            numBits : 32
            );

      POLAR_INLINE_BITFIELD_FULL(
            StringLiteralInst, LiteralInst, 2+32,
            TheEncoding : 2,
            : NumPadBits,
            Length : 32
            );
      ...
      POLAR_INLINE_BITFIELD(
            DeallocRefInst, DeallocationInst, 1,
            OnStack : 1
            );
      ...
      POLAR_INLINE_BITFIELD_FULL(
            AllocStackInst, AllocationInst,
            64-NumAllocationInstBits,
            NumOperands : 32-NumAllocationInstBits,
            VarInfo : 32
            );
      IBWTO_BITFIELD(
            AllocRefInstBase, AllocationInst, 32-NumAllocationInstBits,
            ObjC : 1,
            OnStack : 1,
      ...
      POLAR_INLINE_BITFIELD(
            CopyAddrInst, NonValueInstruction, 1+1,
      ...
      POLAR_INLINE_BITFIELD(
            LoadReferenceInstBaseT, NonValueInstruction, 1,
            IsTake : 1;
      ...
      POLAR_INLINE_BITFIELD(
            StoreReferenceInstBaseT, NonValueInstruction, 1,
            IsInitializationOfDest : 1;
      template<PILInstructionKind K>
      ...
      POLAR_INLINE_BITFIELD(
            BeginAccessInst, SingleValueInstruction,
      ...
      POLAR_INLINE_BITFIELD(
            BeginUnpairedAccessInst, NonValueInstruction,
            NumPILAccessKindBits + NumPILAccessEnforcementBits
            + 1 + 1,
            AccessKind : NumPILAccessKindBits,
            Enforcement : NumPILAccessEnforcementBits,
      ...
      POLAR_INLINE_BITFIELD(
            EndAccessInst, NonValueInstruction, 1,
            Aborting : 1
            );
      POLAR_INLINE_BITFIELD(
            EndUnpairedAccessInst, NonValueInstruction,
      ...
      POLAR_INLINE_BITFIELD(
            StoreInst, NonValueInstruction,
            NumStoreOwnershipQualifierBits,
            OwnershipQualifier : NumStoreOwnershipQualifierBits
            );
      POLAR_INLINE_BITFIELD(
            LoadInst, SingleValueInstruction,
            NumLoadOwnershipQualifierBits,
            OwnershipQualifier : NumLoadOwnershipQualifierBits
            );
      POLAR_INLINE_BITFIELD(
            AssignInst, NonValueInstruction,
            NumAssignOwnershipQualifierBits,
            OwnershipQualifier : NumAssignOwnershipQualifierBits
            );
      POLAR_INLINE_BITFIELD(
            AssignByWrapperInst, NonValueInstruction,
            NumAssignOwnershipQualifierBits,
            OwnershipQualifier : NumAssignOwnershipQualifierBits
            );

      POLAR_INLINE_BITFIELD(
            UncheckedOwnershipConversionInst,SingleValueInstruction,
            NumVOKindBits,
            Kind : NumVOKindBits
            );

      POLAR_INLINE_BITFIELD_FULL(
            TupleExtractInst, SingleValueInstruction, 32,
            : NumPadBits,
            FieldNo : 32
            );
      POLAR_INLINE_BITFIELD_FULL(
            TupleElementAddrInst, SingleValueInstruction, 32,
            : NumPadBits,
            FieldNo : 32
            );

      POLAR_INLINE_BITFIELD_FULL(
            FieldIndexCacheBase, SingleValueInstruction, 32,
            : NumPadBits,
            FieldIndex : 32);
      ...
      POLAR_INLINE_BITFIELD(
            PointerToAddressInst, ConversionInst, 1+1,
            IsStrict : 1,
            IsInvariant : 1
            );

      UIWTDOB_BITFIELD(
            ConvertFunctionInst, ConversionInst, 1,
      ...
      IBWTO_BITFIELD(
            CondBranchInst, TermInst, 32-NumTermInstBits,
            NumTrueArgs : 32-NumTermInstBits
            );
      IBWTO_BITFIELD(
            SwitchValueInst, TermInst, 1,
            HasDefault : 1
            );
      POLAR_INLINE_BITFIELD_FULL(
            SwitchEnumInstBase, TermInst, 1+32,
            HasDefault : 1,
            : NumPadBits,
            NumCases : 32
            );
...
private:
   ...
   {
      return PILNodeStorageLocation(m_bits.PILNode.StorageLoc);
   }
...
protected:
   ...
   {
      m_bits.OpaqueBits = 0;
      m_bits.PILNode.Kind = unsigned(kind);
      m_bits.PILNode.StorageLoc = unsigned(storageLoc);
      m_bits.PILNode.IsRepresentativeNode = unsigned(isRepresentative);
   }
...
public:
   ...
   {
      return m_bits.PILNode.IsRepresentativeNode;
   }
   ...
   {
      return PILNodeKind(m_bits.PILNode.Kind);
   }
...
struct cast_sil_node_is_unambiguous
{
...
{
public:
   static inline void *getAsVoidPointer(polar::pil::PILNode *node)
   {
      return reinterpret_cast<void*>(node);
   }

   static inline polar::pil::PILNode *getFromVoidPointer(void *pointer)
   {
      return reinterpret_cast<polar::pil::PILNode *>(pointer);
   }

//===--- patternmatch.h - PIL Pattern Matching Infrastructure ---*- C++ -*-===//
//
...
// tree-based pattern matches on PIL. The power of these routines is that it
...
#ifndef POLARPHP_PIL_PATTERNMATCH_H
#define POLARPHP_PIL_PATTERNMATCH_H
...
/// Explicit template instantiation for PILValue so we can access the value
/// inside.
template<typename Pattern>
bool match(PILValue V, const Pattern &P) {
   return const_cast<Pattern &>(P).match(&*V);
}
...
struct class_match {
   ...
   bool match(PILValue v) { return isa<Class>(&*v); }
};
...
struct bind_ty {
   ...
   bool match(PILValue v) { return match(&*v); }
};
...
struct silvalue_bind {
   ...
   silvalue_bind(PILValue &V) : Value(V) { }
   ...
   bool match(PILValue V) {
      Value = V;
      return true;
   }
};

/// Match an arbitrary PILValue, capturing the PILValue if the match succeeds.
inline silvalue_bind m_PILValue(PILValue &V) {
   return V;
}
...
/// Define class_match and bind_ty matchers for all VALUE PILNodes.
///
...
template <typename... MatcherTys> struct MatcherFunctor {
   ...
   template <typename MatcherTy> bool individual(MatcherTy matcher, PILValue v) {
...
// matchers.
template <PILInstructionKind Kind, typename... MatcherTys>
struct InstructionOperand_match {
   ...
   bool match(PILNode *node) {
      if (node->getKind() != PILNodeKind(Kind))
         return false;
      return match(cast<PILInstruction>(node));
   }
...
#define FULL_INST(ID, NAME, PARENT, MEMBEHAVIOR, MAYRELEASE)                   \
  ...
  InstructionOperand_match<PILInstructionKind::ID, MatcherTys...> m_##ID(      \
...
template <typename LTy> struct tupleextractoperation_ty {
   ...
   bool match(PILValue v) { return match(v->getDefiningInstruction()); }
...
template<>
struct Callee_match<PILFunction &> {
   const PILFunction &Fun;

   Callee_match(const PILFunction &F) : Fun(F) {}
...
///
/// We use explicit specialization of Callee_match to handle PILFunctions,

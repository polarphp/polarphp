...
#ifndef POLARPHP_PIL_MEMORY_LIFETIME_H
#define POLARPHP_PIL_MEMORY_LIFETIME_H
...
/// Specifically those instructions which are going to be included when PIL
...
public:
  ...
  struct Location {
    ...
    /// The PIL value of the memory location.
    ///
  ...
  private:
    ...
    Location(PILValue val, unsigned index, int parentIdx = -1);

    void updateFieldCounters(PILType ty, int increment);
  };
...
private:
  ...
  /// Mapping from PIL values (function arguments and alloc_stack) to location
  /// indices.
  ///
  /// In case there are multiple struct/tuple_element_addr for a single
  /// field, this map contains multiple entries mapping to the same location.
  llvm::DenseMap<PILValue, unsigned> addr2LocIdx;
...
public:
  ...
  int getLocationIdx(PILValue addr) const;

  /// Returns the location corresponding to a memory address or null, if
  /// \p addr is not associated with a location.
  const Location *getLocation(PILValue addr) const {
  ...
  /// with a location.
  void setBits(Bits &bits, PILValue addr) {
  ...
  /// with a location.
  void clearBits(Bits &bits, PILValue addr) {
    if (auto *loc = getLocation(addr))
  ...
  void analyzeLocations(PILFunction *function);
  ...
  void analyzeLocation(PILValue loc);
...
private:
  ...
  bool analyzeLocationUsesRecursively(PILValue V, unsigned locIdx,
...
public:
  ...
  struct BlockState {
    /// The backlink to the PILBasicBlock.
    PILBasicBlock *block;
    ...
    BlockState(PILBasicBlock *block = nullptr) : block(block) { }

    // Utility functions for setting and clearing gen- and kill-bits.

    void genBits(PILValue addr, const MemoryLocations &locs) {
    ...
    void killBits(PILValue addr, const MemoryLocations &locs) {
      if (auto *loc = locs.getLocation(addr)) {
...
private:
  ...
  /// Getting from PILBasicBlock to BlockState.
  llvm::DenseMap<PILBasicBlock *, BlockState *> block2State;

public:
  /// Sets up the BlockState datastructures and associates all basic blocks with
  /// a state.
  MemoryDataflow(PILFunction *function, unsigned numLocations);
  ...
  BlockState *getState(PILBasicBlock *block) {

//===--- PILConstants.h - PIL constant representation -----------*- C++ -*-===//
//
...
//===----------------------------------------------------------------------===//
//
// This defines an interface to represent PIL level structured constants in a
// memory efficient way.
//
//===----------------------------------------------------------------------===//

#ifndef POLARPHP_PIL_CONSTANTS_H
#define POLARPHP_PIL_CONSTANTS_H
...
/// SymbolicValue representation also includes a pointer to the PILNode in
...
public:
   enum UnknownKind {
      ...
      /// Attempted to load from/store into a PIL value that was not tracked by
      /// the interpreter.
         UntrackedPILValue,
      ...
      /// Attempted to determine the PIL function of a witness method  and failed.
         NoWitnesTableEntry,
...
private:
   ...
   union {
      PILFunction *function;   // For CalleeImplementationUnknown
...
public:
   ...
   static UnknownReason createCalleeImplementationUnknown(PILFunction *callee) {
      assert(callee);
      UnknownReason reason;
   ...
   PILFunction *getCalleeWithoutImplmentation() {
...
/// This is the symbolic value tracked for each PILValue in a scope.  We
...
private:
   ...
   union {
      ...
      /// This is always a PILType with an object category.  This is the value
      ...
      // semantics in Swift, it is not the case in PIL. In PIL, an array can be
      ...
      // this is how an array initialization is lowered in PIL. Therefore, the
...
public:
   ...
   enum Kind {
      ...
      /// This is a function, represented as a PILFunction.
         Function,

      /// This is an integer constant.
         Integer,

      /// String values may have PIL type of Builtin.RawPointer or Builtin.Word
      /// type.
         String,
   ...
   static SymbolicValue getUnknown(PILNode *node, UnknownReason reason,
   ...
   static SymbolicValue getFunction(PILFunction *fn) {
   ...
   PILFunction *getFunctionValue() const {
   ...
   /// \param target PILFunction corresponding the target of the closure.
   /// \param capturedArguments an array consisting of PILValues of captured
   ...
   static SymbolicValue makeClosure(
      PILFunction *target,
      ArrayRef<std::pair<PILValue, Optional<SymbolicValue>>> capturedArguments,
      SubstitutionMap substMap, PILType closureType,
      SymbolicValueAllocator &allocator);
   ...
   void emitUnknownDiagnosticNotes(PILLocation fallbackLoc);
...
using SymbolicClosureArgument = std::pair<PILValue, Optional<SymbolicValue>>;

/// Representation of a symbolic closure. A symbolic closure consists of a
/// PILFunction and an array of PIL values, corresponding to the captured
...
private:
   ...
   // The number of PIL values captured by the closure.
   ...
   SymbolicClosure(PILFunction *callee, unsigned numArguments,
                   SubstitutionMap substMap, PILType closureType,
...
public:
   static SymbolicClosure *create(PILFunction *callee,
   ...
   PILFunction *getTarget() {
      return target;
   }

   PILType getClosureType() { return closureType; }

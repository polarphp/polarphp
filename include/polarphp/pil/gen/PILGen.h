...
public:
  ...
  /// Mapping from ProtocolConformances to emitted PILWitnessTables.
  llvm::DenseMap<NormalProtocolConformance*, PILWitnessTable*> emittedWitnessTables;
...
public:
  ...
  PILWitnessTable *getWitnessTable(NormalProtocolConformance *conformance);

  /// Emit a protocol witness entry point.
  PILFunction *
  emitProtocolWitness(ProtocolConformanceRef conformance, PILLinkage linkage,
...
#include "polarphp/ast/KnownDecls.def"

  /// Retrieve the _ObjectiveCBridgeable protocol definition.
  ProtocolDecl *getObjectiveCBridgeable(PILLocation loc);
  ...
  ProtocolConformance *getConformanceToObjectiveCBridgeable(PILLocation loc,
                                                            Type type);

  /// Retrieve the _BridgedStoredNSError protocol definition.
  ProtocolDecl *getBridgedStoredNSError(PILLocation loc);
  ...
  ProtocolConformanceRef getConformanceToBridgedStoredNSError(PILLocation loc,
                                                              Type type);

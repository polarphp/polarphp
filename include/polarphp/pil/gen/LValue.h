...
//===----------------------------------------------------------------------===//
//
// A storage structure for keeping track of logical lvalues during PILGen.
//
// In general, only the routines in PILGenLValue.cpp should actually be
// accessing LValues and their components.  Everything else should just
// pass them around opaquely.
//
//===----------------------------------------------------------------------===//

#ifndef POLARPHP_PIL_GEN_LVALUE_H
#define POLARPHP_PIL_GEN_LVALUE_H
...
public:
   ...
   virtual ManagedValue project(PILGenFunction &SGF,
                                PILLocation loc,
                                ManagedValue base) && = 0;
   ...
   PILType getTypeOfRValue() const {
      return PILType::getPrimitiveObjectType(TypeData.TypeOfRValue);
   }
...
protected:
   ...
   ManagedValue projectForRead(PILGenFunction &SGF, PILLocation loc,
                               ManagedValue base, SGFAccessKind kind) &&;

public:
   /// Clone the path component onto the heap.
   virtual std::unique_ptr<LogicalPathComponent>
   clone(PILGenFunction &SGF, PILLocation l) const = 0;

   /// Set the property.
   ///
   /// \param base - always an address, but possibly an r-value
   virtual void set(PILGenFunction &SGF, PILLocation loc,
                    ArgumentSource &&value, ManagedValue base) && = 0;

   /// Get the property.
   ///
   /// \param base - always an address, but possibly an r-value
   virtual RValue get(PILGenFunction &SGF, PILLocation loc,
   ...
   ///
   /// \param base - always an address, but possibly an r-value
   virtual void writeback(PILGenFunction &SGF, PILLocation loc,
                          ManagedValue base,
...
public:
   ...
   virtual RValue translate(PILGenFunction &SGF, PILLocation loc,
                            RValue &&value,
                            SGFContext ctx = SGFContext()) && = 0;

   /// Transform into the original pattern.
   virtual RValue untranslate(PILGenFunction &SGF, PILLocation loc,
                              RValue &&value,
                              SGFContext ctx = SGFContext()) && = 0;
...
public:
   ...
   void addNonMemberVarComponent(PILGenFunction &SGF, PILLocation loc,
   ...
   void addMemberComponent(PILGenFunction &SGF, PILLocation loc,
                           AbstractStorageDecl *storage,
   ...
   void addMemberVarComponent(PILGenFunction &SGF, PILLocation loc,
                              VarDecl *var,
                              SubstitutionMap subs,
                              LValueOptions options,
                              bool isSuper,
   ...
   void addMemberSubscriptComponent(PILGenFunction &SGF, PILLocation loc,
   ...
   void addOrigToSubstComponent(PILType loweredResultType);
   ...
   /// type of l-value.
   PILType getTypeOfRValue() const {
      return PILType::getPrimitiveObjectType(getTypeData().TypeOfRValue);
   }
...
class InOutConversionScope {
   PILGenFunction &SGF;
public:
   InOutConversionScope(PILGenFunction &SGF);
...
struct LLVM_LIBRARY_VISIBILITY ExclusiveBorrowFormalAccess : FormalAccess {
   ...
   ExclusiveBorrowFormalAccess(PILLocation loc,
   ...
   void performWriteback(PILGenFunction &SGF, bool isFinal) {
...
struct LLVM_LIBRARY_VISIBILITY UnenforcedAccess {
   ...
   // given `address`.
   PILValue beginAccess(PILGenFunction &SGF, PILLocation loc, PILValue address,
                        PILAccessKind kind);

   // End the access and release beginAccessPtr.
   void endAccess(PILGenFunction &SGF);

   // Emit the end_access (on a branch) without marking this access as ended.
   void emitEndAccess(PILGenFunction &SGF);
};
...
/// at the PIL level. Adding the access markers in these cases gives PIL address
...
struct LLVM_LIBRARY_VISIBILITY UnenforcedFormalAccess : FormalAccess {
   ...
   static PILValue enter(PILGenFunction &SGF, PILLocation loc, PILValue address,
                         PILAccessKind kind);
   ...
   UnenforcedFormalAccess(PILLocation loc, UnenforcedAccess &&access,
                          CleanupHandle cleanup)

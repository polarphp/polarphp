...
#ifndef POLARPHP_PIL_GEN_SGFCONTEXT_H
#define POLARPHP_PIL_GEN_SGFCONTEXT_H
...
public:
  ...
  /// Otherwise, return an empty PILValue.
  ///
  ...
  bool finishInPlaceInitialization(PILGenFunction &SGF) const {
    if (auto *init = getEmitInto()) {
      if (init->canPerformInPlaceInitialization()) {

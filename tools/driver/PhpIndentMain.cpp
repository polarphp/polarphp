//===--- polarphp_indent_main.cpp - Swift code formatting tool ---------------===//
//
...
#include "polarphp/ide/Indenting.h"
...
private:
   ...
   class FormatterDiagConsumer : public polar::DiagnosticConsumer {
      void handleDiagnostic(SourceManager &SM,
                            const polar::DiagnosticInfo &Info) override {
...
class PolarphpIndentInvocation {
private:
...
public:
   PolarphpIndentInvocation(const std::string &ExecPath)
   ...
   int parseArgs(ArrayRef<const char *> Args, DiagnosticEngine &Diags) {
      ...
      llvm::opt::InputArgList ParsedArgs =
         Table->ParseArgs(Args, MissingIndex, MissingCount, PolarphpIndentOption);
      if (MissingCount) {
      ...
      if (ParsedArgs.getLastArg(OPT_help)) {
         ...
         Table->PrintHelp(llvm::outs(), ExecutableName.c_str(),
                          "polarphp Format Tool", options::PolarphpIndentOption, 0,
...
int polarphp_indent_main(ArrayRef<const char *> Args, const char *Argv0,
                         void *MainAddr) {
   CompilerInstance Instance;
   PrintingDiagnosticConsumer PDC;
   Instance.addDiagnosticConsumer(&PDC);

   PolarphpIndentInvocation Invocation(
